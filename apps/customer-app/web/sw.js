// Customer app service worker — the shop opens on a slow or absent connection (§31 customer row).
//
// **Prices are never served from a cache as if they were current.** The catalogue arrives with a
// pack version, the basket review is tied to that version, and paying against an older one is
// refused by the model rather than quietly repriced — a cached price shown as today's price is a
// customer charged a figure they never saw (P-02). The page is therefore fetched fresh whenever
// there is a connection, and a cached one says when it was taken.
//
// Ordering and payment need the network and say so. The basket does not — it lives in the device's
// own storage, which is why a dropped signal on a bus is a nuisance and not a lost afternoon.
//
...
const BUILT = ['./customer-app.bundle.js'];
