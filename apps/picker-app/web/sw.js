// Picker service worker — the handheld opens with no signal (§31).
//
// It caches the shell and the last page the box actually served. **Yesterday's wave must never be
// served as today's**, which is why the page is fetched fresh whenever there is a signal and a
// cached one announces itself. The queue of what has been picked lives in the device's own storage,
// where the app can reason about it, and is never in here.
//
...
const CACHE = 'sre-picker-shell-v2';
...
const BUILT = ['./picker-app.bundle.js'];
