// POS service worker — makes the lane's app shell load with no network (P-01, hard rule #1). A
// cashier must be able to open the till and keep billing through an outage.
//
// It caches the SHELL and the lane's last-served page. It does NOT cache business data of its own:
// the sale path keeps its local state and the sync outbox (`packages/sync`), which is the single
// place unsent work is tracked and made visible. The durable write still goes to this till's own
// edge on loopback and is refused if that is not answering — this cache never makes a sale look
