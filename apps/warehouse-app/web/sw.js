// Warehouse service worker — the handheld opens with no signal (§31, P-01).
//
// It caches the shell and the last page the box actually served. **Yesterday's assignment must never
// be served as today's**, which is why the page is fetched fresh whenever there is a signal and a
// cached one announces itself. The queue of what has been received and put away lives in the device's
// own storage, where the app can reason about it, and is never in here.
//
...
// page by the store box, so a cache-first page is a cache-first *payload* — yesterday's bins and
// goods-in handed over as today's, silently, which is the exact fault the rest of this codebase spends
// its effort refusing. Static files (the view, the bundle, the manifest) change only on a deploy, so
// those stay cache-first and load instantly.
//
// **A page served from the cache says so.** The copy that goes into the cache carries the time it was
// taken, as `window.shellCachedAt`, and the screen shows a strip saying this is what the shop last
// told it and when. The live copy handed to the browser is untouched, so online there is no strip and
// nothing changes. Without that stamp, turning this cache on would make the screen LESS safe than
// having no cache at all: a worker could put stock away against this morning's bins and nothing
// anywhere would say so (P-08).
//
...
const CACHE = 'sre-warehouse-shell-v1';
...
const BUILT = ['./warehouse-app.bundle.js'];
...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE).then(async (cache) => {
      ...
      // Take the page now as well, so the screen survives an outage that starts before it has been
      // opened a second time. Tolerant: a box that is already unreachable must not stop the service
