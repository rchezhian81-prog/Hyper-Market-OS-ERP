...
back to it when no policy set is supplied. It deliberately does NOT override the statute-accurate finance engine
(GST 72m / income-tax 72m / Companies Act 96m, longest wins) which already serves financial RECORDS. The
customer-personal-data DPDP clock (anonymise 24 months after last activity) is a **separate follow-on** on the
...
each month"). Wired: `services/api/src/main.ts` `PROPOSED_PLANS`, the marketing site (`apps/site/web/index.html`,
`subscribe.html`), and the billing test + honesty guardrail updated to match. OA-12's commercial pricing decision
is now made; metered-ceiling pricing and suspend-for-non-payment remain a later (R8) commercialisation concern,
not needed for the single-tenant pilot.
Earlier: **OA-14 ANSWERED** — the owner chose to **keep the repository PRIVATE and
