...
| **OA-10** | **Native-Tamil review of the Warehouse handheld's on-screen wording (OA-9 bilingual requirement).** The Warehouse PWA shows every scan result — received, put away, and every refusal (unknown barcode, over-delivery needs approval, wrong bin, bin full, recalled/expired into a pickable bin, etc.) — in English and Tamil. The English is authoritative; the **Tamil was written by the build and needs a native speaker to confirm it reads correctly to a warehouse worker** before go-live. A completeness check already fails the build if any word is missing in either language, but it cannot judge whether the Tamil is *right*. | Have a Tamil-speaking staff member or the owner read the warehouse scan-feedback words (in `apps/warehouse-app/web/app.js`, the `ta:` block) and correct any that are unclear; we apply the corrections. | Accept the current Tamil for the pilot and refine from worker feedback on the floor. | **Low** — the words are present and complete, the English is correct, and every refusal also carries a colour and a sound; the risk is only that a Tamil phrase reads awkwardly. Not trading-blocking. | Before warehouse go-live (pilot can run and gather feedback) | Confident Tamil wording on the warehouse handheld | The whole Warehouse PWA (engine, feed, shell, offline, scan feedback) and its English wording — all shipped and tested | 
