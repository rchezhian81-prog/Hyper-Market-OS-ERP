...
Last updated: 12 August 2026 (**★ D-1 inc3 — THE FORECAST CAN NOW COLD-START A BRAND-NEW PRODUCT ★** (M09): a product too new to have its own pattern would forecast near zero (useless — you'd never order it); this seeds it from a **peer / category** rate. `coldStartBaseline` (`packages/demand/src/forecast.ts`) blends the item's own demand with the peer rate, weighted by how much own history exists (credibility shrinkage: `(totalQty + priorDays×peer)/(observedDays + priorDays)`) — a brand-new SKU leans on the peer, and its own sales take over as they accumulate. `forecastDemand`'s optional `coldStart` seeds the baseline this way and forecasts **flat** (no noisy weekly shape) until real history exists; `baselineSource` says `history` vs `cold_start`; the peer rate is DATA the caller supplies (`?peerBaselinePerDay=&priorDays=`). **This completes the full D-1 decomposition** — baseline + day-of-week seasonality + promo/festival/weather signals + new-item cold-start, all judged by a fixture back-test. **8 new tests** (peer-only/own-dominant/weighted blend, cold-start forecast rescues a new item, signals-on-top of cold-start, bad-input reject; route `?peerBaselinePerDay=` end-to-end + 400). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). The whole D-series demand vein (D-3 · D-1 · D-4 + the reorder loop) is now complete; no named demand follow-ons remain. Prior: D-1 signals (#170), D-4 markdown (#169), D-1 forecast (#168), replenishment uses real demand (#167), sales-history read (#166), C4 (#165), D-3 (#164).)
