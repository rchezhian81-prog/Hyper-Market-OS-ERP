...
Last updated: 12 August 2026 (**★ C3 — AUTOMATED RETENTION CLOCK + PRE-ERASURE NOTICE (DPDP s.8(7)) ★** (M16·M35): personal data is not kept forever — once its purpose is served or consent is withdrawn, the clock starts and it is erased. `assessRetention` (`packages/customer/src/retention-clock.ts`) runs that clock over the categories held for a person **without waiting for a request** (unlike the request-driven `planErasure`), and does the two things the Act insists on: **a pre-erasure notice** — a triggered erasable category sits in a `notice` window (erasure scheduled at trigger + `noticeLeadDays`, default 30) and only erases once the window elapses, with a customer-facing notice of what goes and when — and it **honours the same law** as `planErasure`: a legally-retained category is **minimised** (identity stripped, record kept) or, for audit evidence / a legal hold, **retained in full** (never erased, hard rule #6). The clock starts at the **earlier** of purpose-served / consent-withdrawn. Runs on `POST /v1/privacy/retention/sweep`, gated `customer.consent.write` (owner + store-manager, not the till). **9 new tests** (retain-when-live, erase-after-window, notice window + scheduled date + text, minimise/retain-in-full, earlier-of-two-triggers, malformed reject, RBAC). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). **The self-contained DPDP C-series is now COMPLETE (C1·C2·C3·C4)** — no named self-contained data-protection follow-ons remain. Prior: C1 consent-notice (#173), C2 breach-notification (#172), D-1 cold-start (#171), D-4 markdown (#169), D-3 (#164).)
