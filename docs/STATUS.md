...
Last updated: 12 August 2026 (**★ GST RETURNS-NETTING — THE RETURN NOW FILES SALES NET OF RETURNS (CGST s.34) ★** (M23·M13): the other half of a correct GSTR-1. The from-sales return reported outward supplies but not the credit notes for returns, so it OVER-stated the tax owed. Now a return issued in the period nets against the supplies: `netTable12` (`packages/finance/src/outward-from-sales.ts`) subtracts a returns Table-12 from the sales one per HSN/rate, and the route folds the period's banked returns through the SAME `salesToOutwardSupplies` engine (a returned line is an outward supply reversed). `gstReturnsAdapter.returnedTaxLines` reads `STREAM.returns` by processed day, finds each return's ORIGINAL sale, allocates the ACTUAL refund across the return's lines by original value, and reverses the tax **at the rate it was sold** (off the original sale's frozen HSN/rate — a rate change since must not rewrite the reversal). The route response carries `table12` (sales), `returns`, and `net = sales − returns`. A return whose original sale is not on file surfaces as `unmapped` on the returns side (never dropped); a return processed outside the period is excluded. Read-only fold; the offline path is untouched. **5 net-new tests** (3 `netTable12` — net per HSN/rate, independent HSN rows incl. a return with no matching sale netting negative, no-returns no-op; 2 integration — a return nets end to end at the rate it was sold, and an out-of-period return is excluded). Full gate green (typecheck, lint, secret-scan, traceability-integrity, pos-offline, full suite — 5170 passed). **The GSTR-1-from-sales return is now correct on BOTH sides — sales and returns.** Prior: promotion allocation (#180), A5 till-capture (#179), A5 freeze-at-supply (#178), A5 Option A (#177), A5 from-sales (#176).)
