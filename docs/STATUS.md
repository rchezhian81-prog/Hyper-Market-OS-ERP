...
Last updated: 12 August 2026 (**★ C1 — ITEMISED CONSENT-NOTICE VALIDATOR (DPDP s.5–6) ★** (M16): consent is only real if the person was told WHAT data is taken and WHY, and can walk it back as easily as they gave it. `checkConsentNotice` (`packages/customer/src/consent-notice.ts`) validates a notice against exactly that: it must **itemise each (dataCategory, purpose)** (a category with a blank purpose is flagged; an empty notice too), and carry — IN the notice — the ways to **withdraw**, raise a **grievance**, and complain to the **Data Protection Board**, with **withdrawal no harder than giving** (s.6(6), `withdrawSteps ≤ giveSteps` — the commonest dark pattern, checked structurally). It names **every** defect at once and commits nothing. Runs on `POST /v1/privacy/consent-notice/check`, gated `customer.consent.read` (the counter staff who present a notice may check it). **8 new tests** (complete passes; each defect — empty / blank-purpose / missing withdraw+grievance+board / withdrawal-harder; both-counts-only symmetry; name-all-at-once; RBAC + malformed). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). Two DPDP C-series pieces now done (C1 + C2); the last self-contained one is C3 (automated retention clock + pre-erasure notice, honouring the existing planErasure). Prior: C2 breach-notification (#172), D-1 cold-start (#171), D-1 signals (#170), D-4 markdown (#169), D-3 (#164).)
