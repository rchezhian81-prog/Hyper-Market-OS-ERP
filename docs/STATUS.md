...
Last updated: 12 August 2026 (**★ B8 — THE TILL NOW READS THE USE-BY OFF A DATE-CODED BARCODE (AUTO-CAPTURE) ★** (M10·M12): the last core piece of B8 — no manual date entry. An **`expiry` embedded-barcode rule** lets the lane read a batch's use-by straight from a date-coded perishable barcode: `parseGs1Date` (`packages/catalogue/src/catalogue.ts`) turns a GS1-style **YYMMDD** into a real date (day `00` → last day of month; impossible dates like 31-April rejected), and a date-coded scan is **refused offline if past** — the cashier just scans, nothing to type. Backward-compatible (existing weight/price embedded barcodes unchanged); never guesses (today unknown → no block). **6 new unit tests** (parse valid / last-day / leap / bad-date; auto-capture expired-refused, in-date-rings-up, no-asOf-no-block). **B8 is now COMPLETE as core logic** — the lane refuses an expired item, reads the date automatically where the label encodes it, and head office backstops the rest; the only thing left is a per-tenant label/device choice for labels that don't encode the date. Full gate green (typecheck, lint, secret-scan, recall + pos-offline guardrails, traceability-integrity, full suite). Prior: B8 capture through the view (#162), B8 lane block (#161), cloud backstop + critical-breach guardrail (#160).)
