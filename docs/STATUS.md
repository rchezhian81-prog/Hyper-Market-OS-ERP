...
Last updated: 12 August 2026 (**★ A5 GSTR-1 FROM SALES — THE RETURN FOLDED FROM REAL TILL SALES ★** (M23·M12): the write-path foundation the owner asked for. GSTR-1 already had a write path (record an outward-supply document → fold the return) and the full engine + GSTN export — but a hypermarket's real supplies are the thousands of **B2C counter sales the till already banks**, and nobody re-keys each one. This closes that loop: `salesToOutwardSupplies` (`packages/finance/src/outward-from-sales.ts`) turns banked sale lines INTO the return. The obstacle it solves: a banked sale line carries only the **MRP-inclusive total** the till charged, not the HSN or the tax split (those live on the M03 master, not the sale) — so it takes a caller-supplied **product→{HSN, rate} table** (the house idiom; the freeze-safe filing mapping), pulls the GST **back out** of each inclusive total with the tested `extractInclusiveGst` (A9, reconciles to the paisa), splits by place of supply (A8 — a counter sale is intra-State CGST+SGST), and aggregates by `gstr1Table12`. A sold product with no mapping / a bad HSN comes back in `unmapped` — counted and named, **never silently off the return** (P-08). Runs on `POST /v1/finance/gstr1/from-sales/table-12`, gated `finance.gstr.read` (owner + store-manager, not the till), as a **read-only fold** of the sales stream filtered to the period by trading day — the sale-commit path is UNTOUCHED (hard rule #1), the events ARE the record (hard rule #2). **11 new tests** (7 unit, 4 integration end-to-end over the real sale-intake → sales stream → finance fold). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). **OPEN OWNER DECISION named openly:** the production source of the HSN table — read from the M03 master at fold time, or freeze it onto the sale at cloud intake — and the mid-period rate-change case where the stored-document path stays authoritative. Prior: D-2 order-proposal (#175), C3 retention-clock (#174), C1 consent-notice (#173), C2 breach-notification (#172), D-1 cold-start (#171).)

**R2 SELF-CONTAINED SCOPE STATUS (for the owner — a decision point):** 16 ratified-R2 requirements are now built and merged as self-contained, cloud-side, fully-tested increments — A1/A2/A4/A6/A8/A9/A10/A11/A12/A27/A28 (invoice numbering, the GST tax-invoice suite, statutory retention, non-disableable audit) + B2/B3/B4/B6/B9/B24 (dual-MRP, unit price, pack declarations, verified-scale gate, allergen/veg mark, label heights). Each is a pure, tested engine or guardrail joined to a running route, no offline-lane risk, reusing existing permissions. **The clean self-contained vein is now essentially mined out.** The remaining ratified R2 items fall into three buckets the owner should choose between: (1) ALREADY SERVED by existing generic engines — B7/B10 licence/scale-cert expiry alerts → `packages/compliance/src/obligations.ts` (`expiryAlerts`), B1 above-MRP → `packages/price-list/src/price-change.ts` + `packages/catalogue/src/snapshot-builder.ts`, B25/D-7 price integrity → `packages/self-checkout/src/price-integrity.ts`, A29 append-only → `tests/guardrails/ledger-append-only.test.ts` (these need at most a thin cross-reference, not a fresh build); (2) NEED A WRITE PATH or stored data first — **A5 GSTR-1-from-sales is now DONE** (`salesToOutwardSupplies` folds the return straight from banked till sales; the one open piece is the owner decision on the production HSN source — M03-master-at-fold vs frozen-at-intake); what remains here is any requirement whose enforcement lives at the offline till (B5 weighed-price billing, B8 expired-batch block) which we have deliberately not touched (hard rule #1); (3) NEED AN OWNER DECISION — A20/A21 turnover-triggered e-invoicing, B14/B19 tobacco/plastic gates, and the ten owner decisions (entity type, chart of accounts, product lines, payroll). **Recommended next move:** pause net-new R2 feature work and get the owner's call — take on a write-path increment (e.g. persist invoice lines so A5 Table-12 and price-integrity reconciliation can run on real data), or hold R2 for the pre-deployment owner-decision session. No thin/forced increments will be manufactured in the meantime. Context unchanged: CORE-04 pure-read seam complete; work advances the owner-ratified R2 scope (#106).)
