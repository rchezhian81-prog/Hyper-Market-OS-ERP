...
Last updated: 11 August 2026 (**★ BATCH-ON-SALE inc1 — THE SALE LINE CAN NOW CARRY ITS BATCH ★** (owner-authorised core-selling-path work, M10-FR-03): the first, deliberately **cloud-side-only** slice of the change that will let a recall trace real sales. A product now carries a **`batchTracked`** flag from the master through `buildCatalogueSnapshot` onto the lane snapshot; an arriving sale line (`IncomingSaleLine`) carries an optional **`batchId`/`batchExpiry`**; and `acceptSale` raises a **material `batch_tracked_sold_without_batch`** exception when a batch-tracked good (perishable / pharma) is banked with **no batch** — the sale still STANDS (never refused — the money is in the drawer, hard rule #1/#10), but the untraceable unit is surfaced for the day-end review (P-08). **The offline commit path is untouched** — the `pos-offline` guardrail (no network call in the sale path) stays green, which was the whole point of starting cloud-side. **4 route/engine tests + snapshot passthrough test**; full gate green (typecheck, lint, secret-scan, the recall guardrail, one-surface, full suite). NEXT increments (each its own PR): (2) capture the batch at the till + thread it through the **local commit + sync** so a real batchId arrives; (3) fold stored sale lines into the **lot-trace outbound** so B11's trace pulls live sales, and auto-feed A5 GST returns. Prior: B11 (#153), B19 (#152), A7 (#151), B14 (#150), A5 end-to-end.)
