...
Last updated: 11 August 2026 (**★ CORE-05 inc4 — OWNER ALERTS INBOX, ON THE CLOUD ★**: continuing the owner-chosen 'close the thin-services gap' workstream. Wires `packages/owner-control` alerts-inbox — a complete, tested control-by-exception engine nothing on the cloud fed — onto the live API. The owner is the PRIMARY user of this system and cannot read a hundred notifications, so `POST /v1/reporting/owner-alerts` (API-10, on `services/reporting/src/owner-alerts.ts`) runs `raiseOwnerAlerts`: raw exception events (large discounts, voided bills, price overrides, after-hours logins, till shortfalls) are GROUPED by kind, branch and person into the few alerts a phone screen holds — 'six voids by one cashier' is one conversation, not six pings — each keeping every underlying transaction id for the M29-FR-02 drill-through, each severity-ranked (notice/attention/urgent), every threshold the owner's own and an unset kind raising nothing (P-03 control by exception). Stateless and read-only — nothing commits here (hard rule #5); the caller supplies the period's exceptions + the owner's thresholds and this returns the grouping. Gated new permission `owner.alert.read`. `tests/integration/owner-alerts.test.ts` proves the grouping (3 discounts→1 alert keeping all 3 txn ids), the below-threshold-is-silent and unset-kind-is-silent rules, malformed-400 and RBAC-403. Full gate green (typecheck, lint, secret-scan, **4,882 tests / 353 files**). **Metric moved: test-only packages 12 → 11 of 77** (down from 35 at the audit). Prior CORE-05: inc1 `compliance` (#135), inc2 `quality` cold-chain (#136), inc3 `self-checkout` price-integrity/B25 (#137) — all merged. Remaining unused engines: reversal, suspended-sales, notifications, ops, org, documents.)
