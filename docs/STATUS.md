...
Last updated: 11 August 2026 (**★ EINV inc2 — GST E-INVOICING DURABLE LIFECYCLE STORE (A20) ★**: the owner asked to start inc2 groundwork. Built the event-sourced e-invoice lifecycle on the live API (`services/finance/src/e-invoice-register.ts` + `eInvoiceAdapter`, per-invoice stream folded by the tested `packages/e-invoice` engine). The honest architecture: the API is the durable STORE OF RECORD for each invoice's e-invoice state, and a credentialed GSP connector (the remaining deployment step) reads submitted invoices, calls the IRP, and posts the answer back — so the invoice is never held hostage to a synchronous network call and a retry collapses. Routes (API-09): `POST /v1/finance/e-invoice/invoices/:id/submit` records the intent + canonical IRP request (idempotent per invoice; refuses B2C/not-required and a Rule-46-invalid invoice BEFORE storing); `…/record-response` applies the IRP's answer — a `registered` IRN is FINAL (a later response can never change or un-set it), an `unknown` stays retryable, a re-post collapses, and a forged/malformed IRN is still never stored as registered (engine's never-fabricate rule); `…/cancel` cancels a registered IRN ONLY within the 24-hour window (`assessCancellation`) else directs to a credit note; `GET …/invoices/:id` reads the folded state. Gated `finance.einvoice.generate` (writes) / `finance.einvoice.read` (read). Engine additions: `foldEInvoice` (registered/cancelled are final; response-without-submit ignored) + `assessCancellation` (24h window) + `IRN_CANCEL_WINDOW_HOURS`. **12 unit + 9 integration** assertions across e-invoice. Full gate green (typecheck, lint, secret-scan, **4,921 tests / 362 files**). **THE ONLY REMAINING PIECE for live e-invoicing is the certified-GSP connector** (network call to the IRP + credentials from a vault + posting the answer to `/record-response`) — a deployment step that needs the owner's GSP account (ClearTax/Zoho/TallyPrime connector/NIC). A21 30-day IRN N/A (< ₹10 cr). Prior: EINV inc1 deterministic core (#145 merged), CORE-05 complete, owner decisions ratified.)
