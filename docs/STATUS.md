...
Last updated: 12 August 2026 (**★ A5 FROM-SALES — TILL-CAPTURE: THE OFFLINE LANE NOW SELF-FREEZES ITS GST FACTS ★** (M23·M12): the owner-authorised final piece of GSTR-1-from-sales, done as its own careful stage because it touches the offline hot path (hard rule #1). Prior increments made the return honour tax facts frozen on a sale line, but only imports could set them; now the **offline till stamps them onto every sale line at commit**. `apps/pos/src/session.ts` builds the CLOUD sale record with rich lines — `unitPriceMinor`, the tax-inclusive `lineTotalMinor`, `taxRateBps`, and `hsnCode` — the HSN/rate read off the pack the lane priced from (`view-adapter.ts` `scanBarcode` freezes it from `CatalogueProduct.hsnCode`, carried on `ScanInput`/`BasketEntry`). So future live till sales carry their own frozen tax facts and a mid-period rate change files correctly with no import needed. **A record, never a control** — read off the pack already in memory, adds NO network call and NO new way for a sale to fail (proved by the `pos-offline` guardrail staying green); the local-first commit ordering (durable disk → ledger → outbox) is untouched. Discovery that made this its own stage: the offline session previously sent only minimal lines (productId/qty/uom) to the cloud — no per-line price either — so this also **closes that latent gap** (the cloud's line-sum / price-difference checks now run on real till sales). Named boundary: a basket-level promotion discount is a bill-level figure not allocated to lines here (the sale total still carries it). **3 net-new tests** (session records the rich lines + frozen HSN/rate; HSN omitted when the pack carried none; view-adapter freezes the HSN off the pack). Full gate green (typecheck, lint, secret-scan, traceability-integrity, pos-offline guardrail, full suite — 5155 passed). **GSTR-1-from-sales is now COMPLETE end to end and self-serving** — folds from real sales, auto-sources HSN from the catalogue, files a mid-period rate change, and the live till freezes the facts itself. Prior: A5 freeze-at-supply (#178), A5 Option A (#177), A5 from-sales (#176), D-2 order-proposal (#175), C3 retention-clock (#174).)
