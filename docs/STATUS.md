...
Last updated: 12 August 2026 (**★ B8 (cloud backstop) — A SALE OF EXPIRED STOCK IS NOW CAUGHT ★**: selling past-use-by food is an FSSAI breach, not a stock note. A sale line already carries its `batchExpiry` (batch-on-sale inc1), so `acceptSale` (`services/pos/src/sale-intake.ts`) now raises a **CRITICAL `sold_expired_batch`** exception when a line's captured batch expired **strictly before** the sale's trading day (the expiry is the LAST sellable day, so a sale ON it is fine). Ranked with a recall, it tells the manager to pull the batch, check whether the receipt names the customer, and find why the lane did not block it. The sale still banks (money's in the drawer, hard rule #1). **This is the CLOUD BACKSTOP** — the PRIMARY defence, the offline till hard-BLOCKING an expired scan at the lane (FEFO-at-scan), is an offline-path change and is deliberately deferred (same posture as the batch-on-sale offline capture). **4 new unit tests** (critical + last-sellable-day boundary + still-in-date/no-expiry non-flags + ranked-first). Full gate green (typecheck, lint, secret-scan, traceability-integrity, one-surface, full suite). The till exception list now catches the three critical breaches together: a recalled product, an above-MRP charge, and an expired batch. Prior: M16 customer-in-trace (#159), B1 (#158), batch-on-sale COMPLETE (#154-#157).)
