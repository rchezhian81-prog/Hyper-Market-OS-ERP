...
Last updated: 12 August 2026 (**★ PROMOTION ALLOCATION — A PROMO SALE NOW FILES THE CORRECT PER-LINE TAXABLE VALUE ★** (M05·M23): the named boundary from the till-capture increment, closed. A basket promotion is one figure for the whole basket, but a tax invoice and the GST return are per line — so a discount had to be attributed to the lines it actually reduced, or a promo sale's line totals summed to MORE than the customer paid (a cloud line-sum exception) and a targeted promo across mixed GST rates **under-declared tax** by mis-spreading the discount. Now `bestPrice` returns `PromotionResult.perLine` (`packages/promotions/src/promotions.ts`): each chosen promotion's discount is split across ITS eligible lines by gross — a **targeted** promo comes off its own lines, a **basket-wide** one is spread by value — via the reusable `allocateDiscount` (`packages/contracts/src/allocate.ts`; largest-remainder rounding, sums to the discount EXACTLY, never below a line's gross). The till subtracts each line's share from its recorded total (`apps/pos/src/session.ts`), so a promo sale's line totals **sum to what was paid** and the GST return files the **correct per-HSN taxable value** (CGST s.15(3)). Pure & deterministic; the offline commit path is untouched (`pos-offline` guardrail green). **13 net-new tests** (6 `allocateDiscount`; 3 engine perLine — targeted/basket-wide/no-promo; 1 session — targeted promo comes off its own line + lines sum to paid; plus the allocation reused across the from-sales chain). Full gate green (typecheck, lint, secret-scan, traceability-integrity, pos-offline, full suite — 5165 passed). This finishes the GST-from-sales chain including the promotion edge. Prior: A5 till-capture (#179), A5 freeze-at-supply (#178), A5 Option A (#177), A5 from-sales (#176), D-2 order-proposal (#175).)
