...
Last updated: 11 August 2026 (**★ CORE-05 inc5 — REFUND EXCEPTIONS & DAY TOTALS, ON THE CLOUD ★**: continuing the owner-chosen 'close the thin-services gap' workstream. Wires `packages/reversal`'s read/compute side — a complete, tested engine nothing on the cloud fed — onto the live API (M14-FR-03/04). A refund is where the shop pays money OUT, so the failures are the expensive ones: `POST /v1/finance/refunds/exceptions` (API-09, on `services/finance/src/refund-exceptions.ts`) runs `refundExceptions` — a REVERSAL the provider REFUSED (the customer is owed money now), one that came back UNCERTAIN (refund again and the shop pays twice), one confirmed but never on a statement (unreconciled), and OVER-REFUNDED (more reversed against a tender than was ever charged) — worst-first, each valued AND owned (P-03: an exception nobody owns is one nobody clears). And `POST /v1/finance/refunds/day-totals` runs `refundDayTotals` — the day close states confirmed/uncertain/failed SEPARATELY rather than pretending an uncertain reversal is settled, so a manager closing at 10pm sees exactly how much money is in an unknown state. Stateless and read-only — the reversals live in settlement/POS, this is the reading. Gated new permission `refund.exception.read`. `tests/integration/refund-exceptions.test.ts` proves worst-first ordering (failed above over-refund), the over-refund value, the three-way day totals (uncertain+pending grouped), malformed-400 and RBAC-403. `requestReversal`'s payment-provider adapter is a deployment step and stays out of scope. Full gate green (typecheck, lint, secret-scan, **4,885 tests / 354 files**). **Metric moved: test-only packages 11 → 10 of 77** (down from 35 at the audit). Prior CORE-05: inc1 `compliance` (#135), inc2 `quality` (#136), inc3 `self-checkout`/B25 (#137), inc4 `owner-control` (#138) — all merged. Remaining unused engines: suspended-sales, notifications, ops, org, documents.)
