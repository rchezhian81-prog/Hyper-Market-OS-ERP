...
done for Store-Core (R2); Stage 5 has built 17 tested foundation units (138 tests). **D3/D4/D5/D8 were answered on 2 Aug 2026** (see
...
- **Stage 5 (foundation) — BEGUN** (`packages/contracts/`, all with tests; full `pnpm
  ...
  - `pnpm check` green: typecheck + lint + secret-scan + **138 tests**. Value-object
...
## In progress
- **Stage 5 foundation build** — 17 tested units done (`packages/` contracts, ledger,
  approvals, rbac, sync, numbering, calendar, pricing, tender, config, sale, tenant; 138 tests, `pnpm check` green). The
