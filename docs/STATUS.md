...
Last updated: 10 August 2026 (**★ R2 v2.1 A28 — STATUTORY RETENTION CLOCKS + LEGAL HOLD, ON THE CLOUD ★**: the fifteenth ratified-R2 increment (a priority-10 item), extending the audit/retention area. A record is kept for the LONGEST period any binding statute demands, not the shortest that happens to apply: a sales invoice is a GST record (72 mo), an income-tax record (6 yr) AND a book of account under the Companies Act (8 yr), so it is kept EIGHT years — destroying it at six to satisfy one statute breaches another. `longestStatutoryRetention` resolves the applicable statutes to the governing (maximum) period (the minimums are named constants — a legal fact, a one-line tested edit), and `statutoryRetentionDecision` computes the retain-until date and whether the record may be reviewed for deletion. A LEGAL HOLD blocks deletion REGARDLESS of the clock — the hold beats the retention date, never the reverse — and nothing is ever deleted here (it reports when a human MAY review; deletion stays a separate authorised, audited act, hard rule #6). This closes the real gap in the existing `packages/audit/src/retention.ts` (which used one policy per type and had no longest-of-many resolution). Runs on `GET /v1/finance/retention?recordDate=&statutes=&asOf=&onLegalHold=` (a new stateless finance route), gated `finance.period.read` (owner/accountant/manager). `tests/unit/audit-statutory-retention.test.ts` proves longest-wins, within-period block, past-period allow, and the hold override; `tests/integration/finance-retention.test.ts` proves the route + RBAC (a cashier is refused 403). Full gate green (typecheck, lint, secret-scan, **4,862 tests / 348 files**). No new dependency, no new permission. R2 progress: A1/A2/A4/A6/A8/A9/A10/A11/A12/A28 (invoice+tax+retention) + B2/B3/B4/B6/B9/B24 (product/labelling/weights) = 15 ratified requirements built. Note: process fix applied after #132 — validate docs (esp. the traceability-integrity guardrail's backticked-*.ts path resolution) BEFORE the final suite run, not after. Context unchanged: CORE-04 pure-read seam complete; several remaining items already served by generic engines or need a write path / owner decision; work advances the owner-ratified R2 scope (#106), holding the gated items for pre-deployment.)
