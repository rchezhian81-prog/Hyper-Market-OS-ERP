...
Last updated: 10 August 2026 (**★ CORE-04 inc2 — THE RETURN REGISTER IS NOW READABLE ON THE CLOUD (M21), + a latent branding write-drop fixed ★**: the second CORE-04 increment, same method as the first — a tested `packages/returns` engine (`returnableLines`, `overReturned`, `alreadyRefundedMinor`) that ran only in its own unit tests and inside the Web-ERP service-desk session, now READABLE on the cloud. Two NEW read-only routes in `services/pos/src/returns.ts` (API-05): `GET /v1/sales/:saleId/returnable` shows what may still come back **per product** + how much money is left to refund (the money cap, never below zero), and `GET /v1/sales/:saleId/over-returns` is a **detect-only LOSS surface** naming a bill that had MORE come back than went out — impossible through the POST guard, but real when a return is migrated in or synced from another branch's box (money already gone twice; hard rule #10 — it names the exposure, reverses nothing, a human decides). Decisively (the winning trait again): the data was ALREADY on the cloud ledger — both fold from `SaleCommitted` (the wired `POST /v1/sales`) and `ReturnRecorded` (the wired returns guard, already projected to a tenant-wide read model). No new events, no write-path change, no edge data, no owner decision. **RBAC least-privilege (P-04), the same two-tier split as the stored-value reads:** the returnable/refundable desk read is `pos.sale.read` (owner / manager / cashier), but the over-refund report is a loss surface gated one rung up on `lp.case.read` (owner / manager / accountant, NOT cashier). `tests/integration/returns-guard-the-refund.test.ts` proves both routes over the real pipeline (returnable + refundable arithmetic, an over-returned bill surfaced from a foreign/migrated return, the RBAC gate — cashier reads returnable, is 403 on over-returns — 404 for an unbanked bill, per-tenant isolation) AND end to end against real PostgreSQL (a `describe.skipIf(!DATABASE_URL)` e2e in the Stage-gate CI job). M21 stays **PARTIALLY WIRED** (exchanges + no-receipt returns still pending), so the completion-ladder guardrail is untouched and no rung is over-claimed. **Drive-by fix (P-08, discovered while validating):** the M36-FR-02 branding versioned-append was keyed on the wall-clock millisecond alone, so two DIFFERENT brand sets inside the same millisecond collided on one idempotency key and the second was silently DROPPED as if it were a retry — a rebrand vanishing with no error, and a flaky-CI source that surfaced when the new tests shifted scheduling. Fixed by keying the version on the COUNT of prior sets (`services/api/src/adapters.ts` `setBranding`), with a fixed-clock regression test in `tenant-branding.test.ts` that fails on the old key and passes on the new. `typecheck` + `lint` + `secret-scan` clean; full default suite green twice, 327 files / 4729 tests, incl. the new reads + branding regression. Previous increment ↓ · **★ CORE-04 — STORED-VALUE HOUSEHOLD POOLING + CROSS-CHANNEL DOUBLE-SPEND ON THE CLOUD API (M17-FR-04) ★**: first CORE-04 increment — a tested `packages/loyalty` engine (`householdBalance`, `findDoubleSpends`) that ran nowhere on the cloud, now wired onto API-06. Two NEW read-only routes in `services/customer/src/stored-value.ts`: `GET /v1/stored-value/households/:ownerRef/balance` pools ONE balance across every gift card / store-credit a family holds (a family's value is one purse; showing it apart tells a customer they have less than they do), and `GET …/double-spends` surfaces an instrument that went negative once its offline channels synced — with **both** redemptions kept and both channels named, never last-write-wins (hard rule #10 / §31.1). Decisively, the data was ALREADY on the cloud ledger: both fold from the shared issue index (`StoredValueIssued`, carrying `ownerRef`) and the per-instrument movement streams (`StoredValueMovement`, carrying `channel`) that the wired FR-03 already populates — no new events, no write-path change, no edge data, no owner decision. **RBAC is least-privilege (P-04):** the pooled balance is a cashier-level customer read (`loyalty.value.read`), but the double-spend report names money the shop gave away twice — a LOSS surface — so it is gated one rung up on `lp.case.read` (owner / manager / accountant, NOT cashier). `tests/integration/household-pooling.test.ts` proves both routes over the real pipeline (pooled balance, a synced cross-channel double-spend with both channels named, no false positive within balance, per-tenant isolation, the RBAC gate) AND end to end against real PostgreSQL (a `describe.skipIf(!DATABASE_URL)` e2e that runs in the Stage-gate CI job — it caught a test bug: redeeming the gift card before overdrawing it left three spends on it, so the store credit is redeemed instead). M17 stays PARTIALLY WIRED (coupons FR-02 still engine-only), so the completion-ladder guardrail is untouched. Validated: typecheck + lint clean, **full suite 4983 tests / 346 files green** (incl. the real-PG e2e). Previous headline — **★ CORE-03 inc2 — THE SNAPSHOT FACILITY IS NOW DURABLE (survives a restart) ★**: the projection-snapshot facility (inc1) gains a durable backing store. NEW `SqlSnapshotStore` + migration `0011_projection_snapshot` (a `projection_snapshot` table: the whole `Snapshot` as one JSONB row keyed by tenant/stream/projection, UPSERTed so the newest fold wins; no append-only guard or audit chain, because unlike the ledger a snapshot row is MEANT to be overwritten and safely truncated — it is derived and disposable). `main()` wires it into the finance credited-per-invoice read (`financeNotesAdapter`), so that bounded read now resumes from the last persisted fold across a restart instead of re-folding the whole credit-note history on a cold start; a store-less/test wiring still falls back to the process-local in-memory store. Its one added constraint: a projection's state must be JSON-serialisable (the credited-per-invoice state is a plain `Record<string, number>`). Validated: typecheck + lint clean; a SnapshotStore contract runs against BOTH the in-memory and the SQL (fake-client) stores; migration 0011 applies/re-applies against real PostgreSQL; full suite green. Previous headline — **★ CORE-03 inc1 — A GENERAL PROJECTION-SNAPSHOT FACILITY, AND THE FIRST READ IT BOUNDS ★**: a read model is a fold of the append-only ledger, and a fold that always starts at seq 0 grows without bound as the shop trades. The hot paths were already bounded per-adapter; what was missing was a REUSABLE way for any projection to resume from a persisted fold. NEW `packages/persistence/src/snapshot.ts`: a `SnapshotStore` port + `InMemorySnapshotStore` + `projectFromSnapshot`, which loads a `Snapshot<S>` (folded state at a watermark, keyed by tenant/stream/**versioned** projection name), reads only the tail AFTER it (`sinceSeq` + `type`, narrowed at the store), folds it on, and writes a fresh snapshot once enough events accrue so the next read's tail stays bounded. A snapshot is derived and **disposable** — equal to a full fold by construction, and delete-every-snapshot rebuilds the identical model from the ledger (both pinned by test); it changes the cost of reading the truth, never the truth (hard rule #2). To avoid shipping a facility that runs nowhere, it is wired into a real consumer THIS increment: the CGST s.34 cumulative-credit read (`financeNotesAdapter.alreadyCredited`) now folds credited-per-invoice through the facility with a process-local disposable cache, instead of re-summing all credit-note history on each issuance — with a test proving it never goes stale. `docs/adr/ADR-0005-projection-snapshots.md` records the decision (incl. why inventory's bespoke in-ledger snapshot is deliberately left as-is for now, and the SQL snapshot store deferred). Validated: typecheck + lint clean, full suite green. Previous headline — **★ CORE-02 inc2 — GST CREDIT NOTES RUN ON THE TESTED ENGINE (M23-FR-02 on the cloud API) ★**: second CORE-02 increment, chosen by the owner. The tested `packages/finance` credit-note engine (CGST s.34) had lived only in its own unit tests; it now runs on the API. NEW route `POST /v1/finance/credit-notes` (`services/finance/src/credit-notes.ts`, a new route module alongside settlement.ts/b2b-*.ts) issues a credit/debit note through `issueCreditNote`: the credit can never exceed what the invoice charged **cumulatively across every note** (the cap is folded from the issued-note events in `financeNotesAdapter.alreadyCredited`, not a stored total), tax reverses in the proportion it was charged, a note is declared in the period it was ISSUED, and a note after the s.34(2) 30-November window still issues commercially while stating plainly the tax is not reclaimable. The invoice is never edited (hard rule #2). New permission `finance.creditnote.issue` granted to the senior finance role and the Accountant; idempotent on the note id so a resend is never a second credit. New `tests/unit/finance-credit-note-route.test.ts` proves the route delegates to the engine (issues, persists, enforces the cap, refuses non-proportional tax / wrong customer / missing invoice / missing reason, and marks a late note commercial-only). `docs/audit/wired-via.md` + `docs/traceability.md` (M23-FR-02 → wired on API; ladder note) updated. Validated: typecheck + lint clean, **full suite 4960 tests / 342 files green** (incl. real-PostgreSQL, the surface-contract permission-coverage check). Next CORE-02: the returns report + reconcile routes, then the load-bearing period-close/control-totals question (owner decision on the independent second source). Previous headline — **★ CORE-02 inc1 — OWNER DASHBOARD KPIs RUN ON THE TESTED ENGINE (M29 on the cloud API) ★**: first Phase-3 CORE-02 increment. The cloud API's reporting figures (`GET /v1/reports/dashboard`) are now aggregated by the tested `packages/reporting` `salesSummary` engine (M29-FR-01) instead of an inline `reduce` in the composition-root adapter. "Sales today" (gross takings) and "Sales today — receipts" (basket count) are unchanged, now sourced from the engine; NEW: an honest **tender-mix** breakdown ("Sales today — cash / upi / card / …"), each an exact Σ the engine computes, with split baskets attributed to their largest tender and tenderless sales shown as "unrecorded" rather than hidden (P-08). The path deliberately surfaces **no margin/net/tax** figure — the cloud `SaleCommitted` event carries takings and tenders but not the pre-tax split or cost of goods (those are joined at the edge in `costTheDay`), and a fabricated zero margin is a number somebody acts on. `docs/audit/wired-via.md` + `docs/traceability.md` (M29 ladder → WIRED) updated. Validated: typecheck + lint clean, **full suite 4954 tests / 341 files green** (incl. real-PostgreSQL). Next CORE-02 increments: finance journal posting onto `packages/finance`, then the load-bearing period-close/control-totals question (needs an owner decision on the independent second source — see the finance adapter note). Previous headline — **★ CORE-01 inc3 — AI SERVICE RUNS ON ITS TESTED ENGINE; wired_via PENDING NOW EMPTY (GAP-ARCH-01 closed for services) ★**: the third and final Phase-3 service collapse. `services/ai` now takes its `AgentId` from the tested `packages/ai` authority engine (one definition, not a second copy) and serves a NEW read-only route `GET /v1/ai/agents` straight from the engine's `AGENTS` registry + `FORBIDDEN_TOOLS` — the AI-NFR-01/02/03 governance catalogue (each agent, its allowed tools, its approver, read-only flag, and the tools NO agent may ever be granted, AI-NFR-12), previously reachable only from the engine's own tests. Existing `mayRun`/`acceptProposal`/kill-switch/budget routes unchanged. The `wired_via` guardrail's PENDING set is **now empty** — every domain service (reporting inc1, fulfilment inc2, ai inc3, plus the 11 already-wired) provably runs on its tested engine, and a new re-implementing service fails CI rather than being quietly added back. `docs/audit/wired-via.md` updated. Validated: typecheck + lint clean, **full suite 4949 tests / 340 files green** (incl. real-PostgreSQL, the kill-switch guardrail, AI authority + governance suites). Previous headline — **★ CORE-01 inc2 — FULFILMENT RUNS ON ITS TESTED ENGINE (GAP-ARCH-01) ★**: the second Phase-3 collapse. `services/fulfilment` now decides a recorded delivery attempt's resulting state through the tested `packages/fulfilment` state machine (`transitionDelivery`/`isTerminalDelivery`), not a second copy: `POST /v1/delivery/attempts` returns the engine-computed `deliveryState` (a `delivered` outcome → terminal `delivered`; any failure → non-terminal `failed`, reattemptable, never a silence) and `final`. The `wired_via` guardrail (`tests/guardrails/services-run-on-their-tested-engine.test.ts`) drops `fulfilment` from PENDING — **only `services/ai` remains** — and pins fulfilment→`packages/fulfilment`; `docs/audit/wired-via.md` updated. Existing `checkAttempt`/`reconcileRun` refusals unchanged. Validated: typecheck + lint clean, **full suite 4945 tests / 339 files green** (incl. real-PostgreSQL). Previous headline — **★ CORE-01 inc1 — REPORTING RUNS ON ITS TESTED ENGINE + wired_via GUARDRAIL (GAP-ARCH-01 / RTM-01) ★**: first Phase-3 collapse. The audit's one uncaught drift was that a green "built" row can certify a `packages/*` engine **no running path executes** (~half the packages were imported only by their own tests). NEW `tests/guardrails/services-run-on-their-tested-engine.test.ts` makes "each domain service imports at least one tested engine" a machine-checked fact, with `services/fulfilment` and `services/ai` listed as the remaining PENDING backlog (a ratchet that can only shrink) and a tripwire that FIRES if a wired service drops its engine import. COLLAPSED `services/reporting` onto `packages/reporting`: a new read-only route `GET /v1/reports/catalogue` now serves the tested `reportCatalogue`/`whatWouldUnlockMost` engine (the owner's "which reports can I run, and what should I record next to unlock more" surface — D13/M29/M30 — previously reachable only from its own unit test), fed honest `records`/`produced` config declared in the composition root (`services/api/src/main.ts`). `docs/audit/wired-via.md` is the human-readable companion. Validated: typecheck + lint + secret-scan clean, **full suite 4941 tests / 338 files green** (incl. real-PostgreSQL integration). Previous headline — **★ R2 SCOPE RATIFIED + CORE-01 RESUMED ★**: the owner ratified the recommended Adopt-now (R2) shortlist — **52 requirements** now recorded with testable acceptance criteria in `docs/roadmap/roadmap-v2.1-addendum.md` (closing open item OA-1); items gated on the ten owner decisions are held, not dropped; every other finding keeps its deferred/target-release disposition. Each adopted item earns its `docs/traceability.md` row when built. **Phase-3 CORE-01 has resumed** — the proven-engine ↔ running-service wiring (GAP-ARCH-01 / RTM-01). Previous headline — **★ WORLDWIDE REQUIREMENTS-COMPLETENESS GAP ANALYSIS delivered for OWNER RATIFICATION ★**: at the owner's instruction not to depend on him to author/correct requirements, five parallel worldwide research sweeps produced **132 cited candidate requirements** — India fiscal/tax (A1–A30), product/trade/consumer/environmental (B1–B25), data-protection/payments/labour (C1–C21), industry-standard grocery ERP/POS capability depth (D-1–D-30), and competitive/emerging practice (E1–E26) — each tagged Legally-mandatory / Industry-standard / Competitive / Innovation, cited to the exact rule/notification/benchmark, cross-referenced to the existing 36 roadmap modules, with a recommended disposition. Written to `docs/requirements/WORLDWIDE_REQUIREMENTS_GAP_ANALYSIS.md` as PROPOSALS (not RTM rows — `CLAUDE.md`: never invent a requirement). Headline: roadmap breadth is strong (~90% of areas named); the gaps are (1) legal sub-obligations named-but-not-enforceable, (2) depth shortfalls (fresh forecasting/markdown/waste), (3) genuinely absent duties (breach-notification, children's-data guard, payment-data residency, price-integrity audit, store task-execution + digital HACCP). **Phase-3 CORE-01 remains PAUSED per the owner's "research first" decision** until the report is ratified line-by-line. Previous headline — **★ SYNC-06 — HEADLESS-OFFLINE E2E: every screen opens with the network cut — GATE G2 MET (audit TEST-02) ★**: the second Gate-G2 item (with SYNC-01). The static guardrail (`every-screen-opens-without-a-network`) proved each shell REGISTERS a service worker; this proves the property the shop depends on in a REAL headless browser. NEW `tests/e2e/screens-open-offline.e2e.ts`: for one screen per distinct app/service-worker (pos, manager/web-erp, owner, picker, driver, customer, warehouse), it loads the screen, lets the worker install (which caches the shell AND a stamped copy of the page), waits until the worker CONTROLS the page, then **cuts the network entirely (`context.setOffline(true)`) and reloads** — and asserts the screen STILL OPENS: genuinely offline (`navigator.onLine === false`), a real page rendered (non-empty title + body, NOT the browser's "no internet" error), and served FROM the service-worker cache (`window.shellCachedAt` is a stamped string). The till gets an extra check it boots its OWN shell (title "SRE POS", "Scan an item to begin"). Driven by `playwright-core` against the environment's PRE-INSTALLED Chromium via `executablePath` (never a download — per the run notes); it lives in its OWN suite (`pnpm test:e2e`, `vitest.e2e.config.ts`) OUT of the default `pnpm test` (the CI `verify` runner has no browser) and SELF-SKIPS when no browser is present rather than failing (the same posture the real-PostgreSQL suites take without a database). RAN here: **8 checks green in ~3.2s against real Chromium**; skips cleanly (8 skipped, no error) when the browser path is absent. Added `playwright-core` devDep (SBOM regenerated, `pnpm audit --audit-level=high` clean) and a `test:e2e` script, wired into `pnpm check`. Full default suite still green (336 files, 4933) and unchanged (the e2e is excluded by its `.e2e.ts` extension); typecheck + lint + secret-scan + perf clean. **HONEST SCOPE:** a browser is not provisioned on the CI runner, so this does not yet GATE CI — wiring a browser-provisioning CI job (mirroring how the integration job provisions PostgreSQL) is a DevOps follow-on, the same non-blocking posture STAB-04 set for the perf suite. **With SYNC-01 (increments 1–3) and SYNC-06 both green, GATE G2 (offline/hybrid hardening) is MET.** Per the critical path the next phase is Phase 3 — Core product gap closure (CORE-01: collapse thin services onto the tested engines; CORE-02: Finance M23 + Reporting M29 end to end), gated behind G1 (done) + G3's own gate. Earlier today: **★ SYNC-01 increment 3 — PACK-AGE BADGE ON EVERY SCREEN — SYNC-01 now WIRED end to end (audit GAP-SYNC-01) ★**: increments 1–2 made the box PULL a signed catalogue and hold it across a reboot; this makes that freshness VISIBLE. Every served screen now carries a `catalogueFreshness` global — HOW FAR BEHIND THE CLOUD this shop's prices/recalls have fallen — computed on the edge from the pack the box is actually trading on (`node.pack()`) via the SAME `packFreshness` the cloud serves, so "3 hours old" means three hours behind the cloud, NOT "three hours since this box rebooted" (the misleading `receivedAt`=boot-time reading the deep recon flagged). The honest state is explicit (P-08): a box that has pulled no pack says `{ known: false }` — never a misleading "0 hours old" that reads as fresh — and only says `{ known: true, version, builtAt, ageHours, visibleToStaff }` when it truly holds one. Surfaced at the one seam every screen passes through: `catalogueFreshness(input)` off the `ScreenInput` (new `cataloguePack?: SignedPack`, set in `edge/store-edge/src/main.ts`'s per-request `snapshot()` from the live held pack), injected by `injectPayload` (`edge/store-edge/src/screen-server.ts`) as its OWN `<script>window.catalogueFreshness = …</script>` tag on EVERY screen — including one whose own payload is null (a screen with nothing else to show STILL shows the badge) — WITHOUT polluting any screen's data (the POS catalogue object stays a clean `CatalogueSnapshot`). Tested — unit (`tests/unit/edge-feeds-the-screens.test.ts` +6) and integration through the REAL screen server + real shells (`tests/integration/the-screens-are-fed.test.ts` +2: the badge is in the served HTML of every screen with the right version/age, and `{ known: false }` when the box holds no pack; the existing payload-extractor's invariant was corrected to "the screen's own global is absent" now that the badge legitimately replaces the marker). Full suite green (336 files, 4933); typecheck + lint + secret-scan + perf clean; no control bytes. **HONEST SCOPE:** this surfaces the DATA on every screen (`window.catalogueFreshness`); each screen's shell RENDERING the badge visually is a small per-screen UI follow-on. What the POS PRICES from is still the richer `StorePack` catalogue (Tamil names, age-restriction, productMaster recall cross-check) — feeding the till from the pulled signed slice would DROP that information, so unifying the two onto one signed full-`StorePack` refresh is the SYNC-01b follow-on. **SYNC-01 (inbound pack sync: signed pull + atomic swap + pack-age on screens) is now WIRED END TO END** across increments 1–3. Gate G2 additionally needs SYNC-06 (headless-offline Playwright e2e, network cut). Earlier today: **★ SYNC-01 increment 2 — INBOUND PULL WIRED INTO THE EDGE LOOP + atomic pack swap (audit GAP-SYNC-01) ★**: increment 1 built the pull MECHANISM; this joins it to the RUNNING box. The store edge's poll loop (`pass()` in `edge/store-edge/src/main.ts`) already drained the outbox to the cloud on a backoff timer, well off the sale path (hard rule #1); this rides the SAME loop, the OTHER DIRECTION — after each drain it calls a new `refreshPack()` which runs `pullPack` (fetch signed pack → `acceptPack` verify → adopt-if-newer-and-verified) and, on a real update, PERSISTS the accepted pack ATOMICALLY to disk. NEW `edge/store-edge/src/signed-pack-file.ts`: `writeSignedPack` writes a temp file, fsyncs it, RENAMES it over the live file (rename is atomic on POSIX) and fsyncs the directory — so a power cut mid-write leaves either the whole old pack or the whole new pack, never a torn one (the same durability the sales log and sync cursor already have); `readSignedPack` restores it at BOOT and re-verifies it the SAME way a lane verifies one over the wire (signature + tenant via `acceptPack`), so the box starts on the last pack it trusted and a TAMPERED on-disk file is rejected (start from no pack, let the first pull re-establish trust) rather than trusted as a baseline. Persisted to `EDGE_DATA_DIR` beside the cursor (NO new config; the pack's version IS the durable last-accepted marker, and `acceptPack`'s version-not-backward rule prevents regressions). Change-only logging (P-08): a real update / a rejected pack / going offline is one line; a quiet "still newest" pass says nothing. `refreshPack()` is exposed on `EdgeProcess` (null with no cloud) — the same code the timer calls — so a test drives one pull without waiting the 15s interval, exactly as `agent.drain` is. Rules kept: no pull without a cloud (same branch as no-sync-without-a-cloud), the pull never blocks or touches the sale path, and whatever happens the lane keeps trading on the last pack it trusted (P-01). Tested — unit (`tests/unit/signed-pack-file.test.ts`, 7: round-trip, no temp file left, atomic overwrite, missing/tampered/wrong-tenant/garbage→undefined) and INTEGRATION against a real local HTTP cloud (`tests/integration/the-box-pulls-its-catalogue.test.ts`, 5: fetch→adopt→persist→REBOOT RESTORES IT; a forged pack refused with nothing adopted; a 404 none_published; an unreachable cloud keeps the restored pack and reports offline; a tampered on-disk file starts the box from no pack). Full suite green (336 files, 4926); typecheck + lint + secret-scan + perf clean; no control bytes. **HONEST SCOPE:** the pull now runs LIVE and the accepted catalogue SURVIVES REBOOT. What the SCREENS/POS consume is still the unsigned `StorePack` file — feeding the POS from this freshly-pulled signed catalogue and surfacing pack-age on every screen is increment 3. Re-downloading the full pack each pass (no conditional GET/ETag yet) is a documented bandwidth follow-on. Gate G2 = SYNC-01 + SYNC-06 (headless-offline Playwright e2e). Earlier today: **★ SYNC-01 increment 1 — INBOUND SIGNED PACK PULL mechanism (audit GAP-SYNC-01) — Phase 2 begins ★**: Gate G1 done, so per the audit roadmap's critical path Phase 2 (hybrid/offline hardening) begins with SYNC-01 (inbound pack sync). PROBLEM: the outbox drain (`edge/sync-agent`) carries the shop's sales UP to the cloud, but NOTHING carried prices, promotions or — the one that matters — RECALLS back DOWN. The store box read its catalogue pack from a file ONCE at boot and never again, so "one commerce truth" (P-02) and the offline recall-block were only ever as fresh as somebody carrying a file to the box (there is no inbound pull at all today — verified by a deep recon of the sync/pack architecture). This increment builds the **inbound MIRROR** of the outbound `SyncTransport`/`httpTransport`/`SyncAgent.drain`, the same way they were built: a **`PackSource` port** with a thin **`httpPackSource`** adapter that GETs the ALREADY-EXISTING cloud endpoint `GET /v1/catalogue/pack`, and a pure **`pullPack`** engine (`edge/sync-agent/src/pack-source.ts` + `pack-puller.ts`) that runs the fetched pack through the SAME `acceptPack` the lane already trusts (via `EdgeNode.takePack` — which was fully built + unit-tested but DEAD in production, no caller) and reports what the lane is now trading on and how stale it is (reusing `packFreshness`). It REIMPLEMENTS NOTHING: the fetch is the port, the trust decision (signature + tenant + version-not-backward) is the one shared function, staleness is `packFreshness`. The rule that carries: whatever happens — offline, bad signature, older pack, none published — **the lane keeps trading on the last pack it trusted** (P-01), and a pull can only ever move it FORWARD to a newer verified pack, never blank the catalogue or stop the till. Shares two rules with the outbound transport: the token is NEVER put in a message (#4), and **unreachable is not rejected** (timeout / 5xx / expired token keeps the last good pack and retries; only a clean 200 carrying snapshot+signature is a candidate). Tested (`tests/unit/pack-pull.test.ts`, 14 cases, against a REAL `hmacSigner`/`publishPack`/`createEdgeNode` so `acceptPack`'s check is genuinely exercised — not a fake): adopt a newer verified pack with age reported; adopt the first pack from genesis; keep-last-good on a tampered signature / older version / wrong-tenant pack; offline keeps the held pack, never offers it to the lane, and STILL shows its age (P-08); offline-with-no-pack says so; none-published; and the full `httpPackSource` status mapping with the token never leaked. Full suite green (334 files, 4914); typecheck + lint + secret-scan + perf clean; no control bytes. **HONEST SCOPE:** this is the verified, reusable pull MECHANISM (increment 1). The next increments are: (2) wire `pullPack` into the edge poll loop (`pass()` in `edge/store-edge/src/main.ts`) with an ATOMIC temp-file→rename pack swap + a durable last-accepted-version marker, and (3) surface pack-age on every screen / feed the POS from the freshly-pulled catalogue. A full `StorePack` (all ~60 screen sections) inbound refresh beyond the catalogue slice is the SYNC-01b follow-on. Gate G2 = SYNC-01 + SYNC-06 (headless-offline Playwright e2e). Earlier today: **★ FND-03 — RATE LIMITING + AUTH-ATTEMPT LOCKOUT (audit GAP-SEC-04, SEC-03) ★**: the kernel had EXACTLY ONE 429 in the whole product (the AI budget gate), so nothing capped how fast a caller could hammer the API and nothing slowed a script guessing tokens against the sign-in path. Two protections, kept apart because they answer different threats. **RATE LIMITING** smooths VOLUME: a token bucket per key refills at a sustained rate and allows a burst to a capacity; over-rate calls are refused 429 with a **`Retry-After`** so a retry is paced, not a hot loop. The kernel pipeline checks it **per source IP BEFORE authentication** (a flood — including of the sign-in path — is capped whether or not it ever presents a valid token) and **per tenant AFTER** (one tenant's burst cannot starve another's lane, §35, keyed on the signed tenant, never anything the caller sent). **AUTH-ATTEMPT LOCKOUT** stops GUESSING: consecutive failed sign-ins from one source trip an EXPONENTIAL BACKOFF — below a threshold nothing locks (a mistyped password is not an attack), at/beyond it the source is locked for `base·2^n` seconds capped at a max, and a locked source is refused 429 WITHOUT the token even being examined; a SINGLE SUCCESS clears the count. NEW `services/kernel/src/rate-limit.ts` holds both as PORTS (`RateLimiter`, `AuthThrottle`) with in-process token-bucket / backoff implementations — REAL components, not test doubles (ephemeral state is correct here, the way `RequestMetrics` counters are; named `TokenBucketRateLimiter`/`BackoffAuthThrottle`, driven by an injected clock so a test proves the refusal AND the recovery with no waiting). Wired into `handle()` (`services/kernel/src/pipeline.ts`); the source IP is set by the HTTP server (`services/kernel/src/http-server.ts`) from the leftmost `x-forwarded-for` or the socket peer; the 429 is a complete three-part error (`services/kernel/src/errors.ts` — `ApiError` gained an optional `retryAfterSeconds` the pipeline surfaces as the header); wired in production (`services/api/src/main.ts`, generous burst for a busy till, strict auth lockout). Tested in `tests/unit/rate-limit.test.ts` (9: token-bucket burst/refill/per-key-isolation/cap; backoff below-threshold/lock/doubling-capped/success-clears/source-isolation) and `tests/unit/service-kernel.test.ts` (+4: the pipeline returns 429 + Retry-After on a per-IP flood, isolates sources, locks a source then refuses even a VALID token, and lets a genuine sign-in clear the count). Full suite green (333 files, 4900); typecheck + lint + secret-scan + perf clean; the no-test-doubles-in-production guardrail green (the components are correctly named for their algorithm, not `InMemory*`). **HONEST SCOPE:** the in-process limiter is correct for the single-store box and a single API instance; a MULTI-INSTANCE cloud swaps the port for a shared Redis-backed limiter (technology baseline) so the limit is global and a lockout cannot be evaded by hitting another pod — the same port / deployment-adapter split as idempotency, the follow-on for horizontal scale. **Gate G1 (data-integrity + security foundations) is now COMPLETE: FND-01 atomic append + FND-02 audit hash-chain + FND-03 rate limiting/lockout, all merged.** Earlier today: **★ FND-02 — TAMPER-EVIDENT AUDIT HASH-CHAIN (audit GAP-SEC-03, hard rule #6) ★**: the kernel writes an `audit_log` row for every write and every refusal — WHO did WHAT, WHEN, and what they were told, including "no". Those rows were append-only AT THE DATABASE (migrations 0004/0008 refuse UPDATE and DELETE) but UNSEALED: nothing bound a row to the one before it, so a row inserted, removed or reordered BEHIND the database — a restored backup with a line quietly dropped, a direct write by whoever holds the credentials — left no trace. Append-only stops edits THROUGH THE APPLICATION; it does not make an out-of-band change DETECTABLE. Now each row carries the SHA-256 hash of its predecessor within the SAME TENANT (§35) plus a hash over its own sealed contents — actor, path, outcome status, and the TIMESTAMP (sealed as a writer-supplied ISO string, not the DB's `now()`, so a back-dated row breaks the seal). Change one byte of one row, or drop one, and the verifier names the exact row where the chain breaks (P-08 — tamper-EVIDENT, not tamper-proof). NEW `services/kernel/src/audit-chain.ts` holds the ONE definition of the seal (`sha256Hex` via `node:crypto` — a real cryptographic digest, not the pure-TS FNV-1a that `packages/audit` carries as its dependency-free default — plus `auditChainHash` and `verifyAuditChain`), used by BOTH the writer (`SqlAuditSink`, rewritten to chain) and the verify tool, so the two cannot drift. Writes seal under a PER-TENANT ADVISORY LOCK inside one transaction (built on FND-01's `pgPoolClient`, now wired into the audit sink in `services/api/src/main.ts`), so two concurrent audit writes cannot fork the chain; a UNIQUE INDEX on `(tenant_id, prev_hash)` (migration `db/migrations/0010_audit_log_hash_chain.sql`) is the belt-and-suspenders fork guard — even a bug cannot silently branch a chain, it fails loudly. The migration is additive + idempotent (two nullable columns + the index; pre-migration rows are an unsealed prefix the verifier skips). SHIPS A RUNNABLE VERIFIER: `pnpm verify:audit` (`scripts/verify-audit-chain.mts`) reads the whole `audit_log`, checks every tenant's chain, and EXITS NON-ZERO on any break — for CI, a cron, or an auditor's terminal. Proved against REAL PostgreSQL (`tests/integration/the-trail-is-kept.test.ts` +2: rows written through the real sink link genesis→…→n and the whole chain verifies via the shared `verifyAuditChain`; a hand-crafted FORKED row is refused by the database; edit/delete still refused) and the verifier's tamper-detection pinned in `tests/unit/audit-chain.test.ts` (11 cases: SHA-256 known vectors, every sealed field changes the hash, and the verifier catches an EDITED row, a REMOVED row, a FORGED row, with per-tenant isolation). Also RAN the tool against the live DB: 3 sealed records across 2 tenants intact, 252 unsealed legacy rows correctly skipped, exit 0. Full suite green (332 files, 4887); typecheck + lint + secret-scan clean; migration idempotency (QG-08) green; no control bytes. **HONEST SCOPE:** this seals the PRODUCTION kernel `audit_log` (the request-level access trail — who called what and what they were told). The richer domain-state audit in `packages/audit` (before/after reconstruction) is a SEPARATE, currently-unwired trail whose `Hasher` port already accepts SHA-256; wiring it is not in this increment. Second data-integrity foundation of Gate G1; FND-03 (rate limiting + auth-attempt lockout) is the last of the three. Earlier today: **★ FND-01 — ATOMIC MULTI-EVENT APPEND (audit GAP-DATA-01, hard rule #2) ★**: a money-critical command writes MORE THAN ONE event — a banked sale plus its receipt-number index, a return plus its tenant-wide reporting projection — and before this they were two SEPARATE appends, so a crash between them left a PARTIAL set: a sale with no receipt, a report missing a return. The database's own answer to "all-or-none" is a transaction, and the store had no way to open one — the `SqlClient` port was `query()`-only (the audit's GAP-DATA-01). Added: (1) an OPTIONAL `transaction<T>(fn)` on the `SqlClient` port whose callback runs every query on the SAME pinned connection (so a read inside the batch sees the batch's own uncommitted writes — the subtle correctness point, and it is tested), implemented over a real `pg.Pool` by a NEW `pgPoolClient` adapter (BEGIN → run → COMMIT, ROLLBACK-and-rethrow on any throw, the connection always released) — `pgClient` STAYS query-only, so the ~dozen single-write stores and the integration harness's `pg.Client` (whose `connect()` is incompatible with a pool's) are untouched; (2) `EventStore.appendBatch(tenant, entries)` that commits a whole set atomically — the SQL store wraps a batch of TWO-OR-MORE in one transaction (a lone append needs none: a single INSERT is atomic, so the common path pays no BEGIN/COMMIT overhead), the in-memory reference STAGES then applies in one mutation pass; each event keeps its own idempotency key so a whole-batch RESEND dedups every event and the results line up one-to-one, in order. Wired the two money-critical sites (`bankSale`, `recordReturn` in `services/api/src/adapters.ts`) to `appendBatch`; `services/api/src/main.ts` now gives the PRODUCTION event store the transactional adapter (`pgPoolClient`). **Proved against REAL PostgreSQL** (`tests/integration/appends-are-atomic.test.ts`, 2 cases: a good batch commits both events in order + a whole-batch replay dedups; a batch whose second event violates `event_ledger_id_uq` mid-flight ROLLS THE FIRST ONE BACK TOO — the stream is left empty and neither idempotency key resolves, so a crash mid-command leaves NOTHING). Plus the store contract on both implementations (`tests/unit/persistence-event-store.test.ts` +5: batch order/seq, whole-batch replay dedup, cross-stream, in-batch duplicate-key collapse, and a transactional fake proving rollback-leaves-no-partial-set + tx-scoped conflict read-back) and the connector envelope (`tests/unit/persistence-pg-connector.test.ts` +4: BEGIN/COMMIT on success, ROLLBACK+rethrow+release on throw, queries on the pinned connection, and no transaction primitive over a query-only client). Full suite green (331 files, 4874); typecheck + lint + secret-scan clean; no control bytes. **HONEST SCOPE:** this draws the atomic boundary and wires the two multi-event money sites; the other ~90 single-append call sites are already atomic on their own and were deliberately not touched. First data-integrity foundation of the audit roadmap's Phase-1 (Gate G1); FND-02 (SHA-256 audit hash-chain) and FND-03 (rate limiting + auth lockout) follow as separate PRs. Earlier today: **★ STAB-02 — OFFLINE RECEIPT NUMBERING wired to the POS (audit GAP-SYNC-02) ★**: the served till used to mint `R-${Date.now()}`, so two lanes ringing offline could collide on a receipt number — a customer/statutory document that must be gap-free and unique (M01-FR-02). Now each lane draws from a per-lane RESERVED RANGE via the already-unit-tested `ReservedRangeAllocator` (`packages/numbering`), wired into `bootPos` (`apps/pos/src/browser-entry.ts` — new `receipt` config + `nextReceipt()` / `receiptsRemaining()`), and the served shell (`apps/pos/web/app.js`) now draws its receipt number from `session.nextReceipt()` at both tender sites and derives a lane-unique `saleId` (`S-${receiptNumber}`) from it — which also closes the parallel `saleId` idempotency-collision risk (two lanes minting the same `S-${timestamp}` would have deduped to one sale in the cloud). An **exhausted range STOPS the till** (the allocator throws → the shell shows a bilingual "receipt numbers used up — take no money, tell the manager" and takes nothing) rather than reusing a number; an unprovisioned standalone/demo shell falls back to a timestamp (explicitly not collision-safe). Tested (`tests/unit/pos-receipt-numbering.test.ts`, 3 cases: two lanes with distinct ranges mint 100 unique gap-free numbers with zero overlap; an exhausted range throws; the demo fallback). Full suite green (4861); typecheck + all POS guardrails (pos-offline, till-usable, screen-opens-offline) clean. **HONEST SCOPE:** this wires and proves the reserved-range MECHANISM. The remaining piece for end-to-end multi-lane collision-safety in the *deployed* shell is provisioning a DISTINCT range per lane into the box's injected `posReceiptSeries` (the served shell currently boots default lane config; per-lane config injection depends on the box-per-store-vs-per-lane topology, ADR-0004) — and automatic range refresh/reconciliation rides the inbound-pack path (SYNC-01). Recorded as the SYNC-02b follow-on. First stabilization pair of the audit roadmap done (STAB-01 pool + STAB-02 numbering mechanism). Earlier today: **★ STAB-01 — DB CONNECTION POOL (audit GAP-DATA-09 resolved) ★**: the cloud API now opens PostgreSQL through a `pg.Pool` (`max: 10`) instead of a single shared `pg.Client`. The single client serialised every query across all thirteen APIs over one TCP connection and a dropped connection disabled all persistence — a throughput ceiling and a single point of failure the deep architecture audit flagged as GAP-DATA-09. The `pgClient` adapter (`packages/persistence/src/pg-client.ts`) was written for a `Pool` from the start (its own header shows `new Pool(...)`), so this is the wire it was waiting for; the pool reconnects a dropped member transparently, so a brief DB blip no longer takes the process down. Boot still fails fast if the DB is unreachable (`await db.query('SELECT 1')` through the pool, the analog of the old eager `connect()`), and `db.end()` drains the pool cleanly on shutdown. **Verified end-to-end locally** by booting the built API against real PostgreSQL: 184 routes served, `/livez` + `/readyz` both green (the readiness probe queries PG through the pool), and SIGTERM drained cleanly (exit 0) — the same path the CI deploy job checks. Full suite green (4858); typecheck + lint clean. First item of the audit's roadmap (Phase 0 immediate stabilization). Earlier today: **★ M08-FR-04 RETURNS-NETTING WIRED (API-04) — the margin now reflects what came back ★**: `GET /v1/inventory/performance` now nets customer returns out of sales and margin, store-wide and per product. The obstacle found and solved: returns were stored PER-SALE (`forSaleReturns(saleId)`) with no time-windowed source, and a return records a refund + a per-line DISPOSITION (resell/quarantine/damaged/scrap) but does NOT move stock. Solution: `recordReturn` now also appends the return to a tenant-wide `STREAM.returns` PROJECTION — an additive, idempotent second append, exactly the pattern `bankSale` already uses for its receipt-number index (the per-sale stream stays the book of record for the refund cap + at-most-once register; the money path is unchanged, and a projection append that ever failed would only cost a report one return, never money). The performance adapter reads the projection for the window and reverses each return: **net sales fall by the ex-tax refund** (allocated across the return's lines by their original sale value, so a multi-line refund splits by what each line was worth; single-line is exact), and **COGS falls ONLY for `resell` lines** at the closing weighted-average unit cost — a `damaged`/`scrap` return is a genuine loss by its own recorded disposition, so its cost stays in COGS. Honest edges (P-08): a return of a product with an unknown tax rate marks that product unmeasurable just as an unknown-tax sale does; a `resell` return of a since-sold-out product has no WAC unit to reverse at, so no cost is reversed. The report exposes a `returns` figure (ex-tax returns deducted) alongside net sales for transparency; revenue basis is now `net_of_tax; net_of_returns`. Integration-tested (`tests/integration/inventory-performance.test.ts` +2, now 6: a RESELL return nets both sales AND cost — margin ₹75, GMROI 0.25×; the SAME refund on a DAMAGED return nets sales but NOT cost — margin −₹25, the loss made visible). Full suite green (4858). Typecheck clean. **The M08-FR-04 analytics are now complete** (valuation, ageing, turns/GMROI store-wide + per-product, returns-netted). The one remaining follow-on is a returns↔stock integration: a `resell` return does not yet re-enter the inventory ledger, so returned stock does not lift average inventory (a bigger cross-module change that would also let valuation/ageing reflect returns). Earlier today: **★ M08-FR-04 PER-PRODUCT TURNS/GMROI BREAKDOWN WIRED (API-04) — where the range decisions live ★**: `GET /v1/inventory/performance` now returns, alongside the store-wide totals, a `byProduct[]` array — every product that held stock or sold in the period with its OWN cogs, average inventory, net sales, gross margin, turns, days-of-cover and GMROI. The store total is the exact sum of the product rows. Computed by grouping the weighted-average valuation fold (at both period cut points) BY PRODUCT and grouping POS net sales by product line; a product whose tax rate is unknown keeps its own margin/GMROI absent (`not_meaningful`) while every other product still computes — more granular honesty than the all-or-nothing store figure (P-08). Backward-compatible: the store-wide fields stay at the top level, `byProduct[]` is additive; the metric maths still reuses the unit-tested `packages/stock/metrics.ts`. `services/inventory` `StockPerformanceInputs` now carries `{ total, byProduct }` and the route maps a shared `figuresOf(row)` over both; the adapter (`services/api/src/adapters.ts`) folds per-product cogs/value and per-product net sales. Integration-tested (`tests/integration/inventory-performance.test.ts` +1, now 4: a two-product basket — P1 GMROI 0.33×, P2 0.125×, store total 0.214× as their exact sum — plus the single-product rows equalling the total and the tax-unknown row staying absent). Full suite green (4856). Typecheck clean. This completes the per-product follow-on flagged at the turns/GMROI wire; the one remaining M08-FR-04 revenue-basis follow-on is returns-netting. Earlier today: **★ M08-FR-04 STOCK TURNS + GMROI WIRED (API-04) — is the money in stock working? ★**: stock productivity over a period, live on `GET /v1/inventory/performance?from=&to=` (default trailing 365 days). TWO kinds of number. (a) TURNS, annualised turns and days-of-cover are pure inventory — COGS ÷ average inventory, both folded from the weighted-average movement ledger at the two period cut points (period COGS = cumulative WAC COGS at `to` minus at `from`; average inventory = the two-point (opening+closing)/2). (b) GMROI (rupees of margin per rupee of stock) is cross-domain: net sales come from the POS `SaleCommitted` ledger de-grossed by the catalogue's per-product `taxBps` (MRP-style prices are tax-inclusive, so net = gross × 10000/(10000+taxBps)), gross margin = net sales − period COGS, GMROI = gross margin ÷ average inventory. **Honest by construction (P-08):** when a product sold in the period has no known tax rate, net sales/gross margin are ABSENT and GMROI is `not_meaningful` — never a guessed margin; when no stock was held, turns/GMROI are `not_meaningful` — never a fabricated 0.00×. The metric maths reuses the already-unit-tested `packages/stock/metrics.ts` (`inventoryTurns`/`gmroi`, exact BigInt basis points). Wired through `services/inventory` (new `InventoryDeps.performance` port + `GET /v1/inventory/performance` route; permission `inventory.availability.read`, owner/store_manager — cashier refused) and the live adapter (`services/api/src/adapters.ts` folds the `InventoryMoved` stream at both cut points + reads the `SaleCommitted` window + the current catalogue pack's tax rates; the inventory SERVICE stays clean, the cross-domain read lives in the adapter — ports & adapters). Integration-tested through the real API + RBAC + per-tenant (`tests/integration/inventory-performance.test.ts`, 3 cases: 100 units @ ₹10 received Mar, 40 sold Jun → COGS ₹400, avg inventory ₹300, turns 1.3333×, days-of-cover 273, and GMROI not_meaningful WITHOUT a tax rate; with an 18% GST catalogue, ₹590 gross → ₹500 net sales, ₹100 margin, GMROI 0.33×; cashier 403 + an empty tenant honest not_meaningful not 0.00×). **Revenue basis: net of tax, gross of returns** — returns-netting and a per-product breakdown are the documented follow-ons. Full suite green (4855). Typecheck clean. This completes the M08-FR-04 analytics that were engine-only. Earlier today: **★ M08-FR-04 STOCK AGEING REPORT WIRED (API-04) — reconciles to the weighted-average valuation ★**: the "how long has the money been asleep" report, live on `GET /v1/inventory/ageing`. A new pure engine (`packages/stock/src/ageing-source.ts` `agedStockLots`, 7 unit tests) folds the same append-only movement ledger into the CURRENT remaining stock — issues drawn down FIFO (oldest receipt first, so remaining stock skews newer; a return/transfer-in re-ages as of when it came back) — and values it at weighted-average, then the route buckets it by receipt age through the existing `stockAgeing` presenter (`packages/stock/src/metrics.ts`). Two honesty properties, both tested: the ageing total RECONCILES exactly to the valuation endpoint's stock value (two reports about one shelf that cannot disagree — value is the WAC pool apportioned across on-hand units, carried with an exact integer remainder so nothing drifts), and uncosted on-hand is stated as an `unvalued` quantity, never priced at a guess (P-08). An out-of-order/negative-stock overdraw is trimmed to the authoritative on-hand so quantity never overstates. Wired through `services/inventory` (new `InventoryDeps.ageing` port + `GET /v1/inventory/ageing` returning the bucketed report + `unvaluedMinor` + `method:'weighted_average'` + `asAt`; permission `inventory.availability.read`, owner/store_manager — a cashier is refused) and the live adapter (`services/api/src/adapters.ts` folds the `InventoryMoved` stream, filtered by product before folding so a product's ageing is its own). Integration-tested through the real API + RBAC + per-tenant (`tests/integration/inventory-ageing.test.ts`, 4 cases: 100 units received ~100 days ago + 100 today bucket into over-90 and 0-30 at ₹1,025/unit and the ageing total equals the valuation total; a 60-unit sale draws the oldest down FIFO leaving 40 old + 100 new; an uncosted receipt surfaced as unvalued; cashier 403 + tenant isolation). **The remaining M08-FR-04 analytics — turns, GMROI — additionally need sales-revenue correlation over a reporting period (a larger cross-domain wire), noted as the next package.** Full suite green (4852). Typecheck clean. Earlier today: **★ M08-FR-04 WEIGHTED-AVERAGE STOCK VALUATION WIRED (API-04) — owner accounting-policy decision answered ★**: the owner chose **weighted-average** as the stock-valuation method, and it is now live on `GET /v1/inventory/valuation`. A new pure engine (`packages/stock/src/valuation.ts` `weightedAverageValuation`, 7 unit tests) FOLDS the tenant's whole append-only inventory-movement ledger in occurrence order into a per-product/location valuation — value at weighted-average cost, cost of goods sold, the derived average unit cost, and any UNVALUED quantity — never a stored or editable figure (hard rule #2, same one-truth ledger as on-hand). A costed `received` movement (now carrying an optional `unitCostMinor`) re-averages `newValue = oldValue + qty×unitCost`; an issue (sale/waste/transfer-out) leaves at the running average as COGS; a return/positive-count/transfer-in re-enters at the current average so it carries no new purchase cost; and a receipt with NO cost adds the units but is reported as `unvaluedMinor` rather than folded in at zero (P-08 — a zero cost would drag the average down and understate every margin after it). Exact integer minor units throughout, half-up integer `share()`, the average derived (value÷valuedQty) never rounded-and-stored so it cannot drift. Wired through `services/inventory` (new `InventoryDeps.valuation` port + `GET /v1/inventory/valuation` returning rows, `totalValueMinor`, `method:'weighted_average'`, `asAt`; permission `inventory.availability.read`) and the live adapter (`services/api/src/adapters.ts` folds the `InventoryMoved` stream sorted by `occurredAt`, currency INR). Integration-tested through the real API + RBAC + per-tenant (`tests/integration/inventory-valuation.test.ts`, 3 cases: two receipts at ₹10.00 then ₹10.50 average to ₹10.25 and a sale leaves 50×1025=₹512.50 as COGS with value holding at the average; an uncosted receipt surfaced as unvalued not folded at zero; a cashier refused 403 + another tenant holds nothing). **M08-FR-04 valuation method → WIRED** (analytics — ageing/turns/GMROI in `packages/stock/metrics.ts`, M29 margin, M23 books — build on this one authoritative cost basis); the **Valuation-method owner decision is now ANSWERED (weighted-average)** in the register + one-page summary. Typecheck clean; affected suites green. Earlier today: **★ M20-FR-03 CUSTOMER DELIVERY WIRED END TO END (test-mode payment) ★**: the last piece — capturing the CUSTOMER's own location for the 10 km check. Added an injectable device-geolocation seam to the customer app (`apps/customer-app/src/browser-entry.ts` `useMyLocation(provider)` + `hasLocation()`): in the browser it reads `navigator.geolocation` (no external provider, no cost); in a test a fake position is injected. A refused/denied/timed-out/NaN capture leaves the location UNSET and says so plainly — never a guessed {0,0} that would read as "9,000 km away". `send` measures serviceability from the captured location: a nearby address CONFIRMS (test payment), one beyond 10 km is REFUSED — unit-tested end to end via `bootShop` (`tests/unit/customer-app.test.ts` +4). The served screen (`apps/customer-app/web/app.js` + `index.html`) has a bilingual (en+ta) "use my location" control and gates pay on `hasLocation()` so an un-located delivery gets a clear "share your location" rather than a puzzling distance refusal; honesty-guarded + offline-guarded + bundle rebuilt. Combined with the store-side feed (previous increment), the full delivery order now runs: browse → basket → review → slot → share location → pay(test) → confirmed/refused-if-out-of-area. **M20-FR-03 → WIRED** (real card/UPI is a deployment provider adapter, OA-4/EX-03; test mode covers the pilot); M20 now FR-01+FR-02+FR-03 wired, only privacy-centre FR-04 partial (ties to M16). Counts: built 139→140, partial 5→4; M20 3 built/1 partial; family M16–M17,M20–M21 → Built (15/16, 1 partial). Full suite 4831 green. Earlier: **◆ OWNER GAVE STORE COORDINATES (11.0168, 76.9558 — Coimbatore) — CUSTOMER DELIVERY STORE-SIDE WIRED ◆**: `customerPayload` (`edge/store-edge/src/screen-data.ts`) now feeds the served customer app (1) the store location + radius from the pack's routing policy — the SAME location/radius the driver dispatch uses (§6.2, one commerce truth) — so the 10 km serviceability check distinguishes a 9 km address from an 11 km one instead of the app defaulting its store to {0,0} and refusing everyone; and (2) the day's **8 delivery slots GENERATED** from the store's delivery policy via `generateDeliverySlots` (9am–9pm IST, 90-min windows, 10 orders each), or the concrete slots the pack carried, or none — never an invented slot. Delivery-slot config co-located on `PackRoutingPolicy` (optional fields; dispatch ignores them). A malformed window yields no slots, never a thrown Invalid Date. Integration-tested (`tests/unit/edge-feeds-the-screens.test.ts` +3: store location/radius fed, 8 slots generated at the right IST instants, none-when-unconfigured). **SRE provisioning values** (for the box's `routingPolicy`): storeLocation {11.0168, 76.9558}, radiusMetres 10000, deliverySlotsPerDay 8, deliveryWindowOpen '09:00', deliveryWindowClose '21:00', deliverySlotCapacity 10, deliveryUtcOffsetMinutes 330. **REMAINING for a complete customer delivery order (M20-FR-03 stays PARTIALLY WIRED):** the app must capture the CUSTOMER's OWN delivery location (device geolocation — no external provider needed; a focused app-UI piece, deliberately not shipped as untested browser code here), and real card/UPI payment awaits a provider (OA-4; test mode covers the pilot). Full suite 4827 green. Earlier: **◆ OWNER ANSWERED OA-11 (main store, 10 km, 8 slots/day) — DELIVERY-SLOT ENGINE BUILT ◆**: added `generateDeliverySlots` (`packages/storefront/src/checkout.ts`) — a pure, tested engine that divides a delivery window into N equal contiguous slots (the last absorbing the rounding remainder so the day is covered exactly), feeding straight into the existing `availableSlots`/`bookSlot`. It INVENTS NOTHING: no slots / a backwards window / no capacity → an empty list, never a guessed slot a shop cannot staff. Unit-tested (+3 cases, `tests/unit/storefront-checkout.test.ts` now 22). The owner declined the follow-up detail questions and said continue, so per that delegation the operational values are DOCUMENTED, OVERRIDABLE DEFAULTS: delivery hours **9 am–9 pm** (eight 90-min slots), capacity **10 orders/slot**; radius **10 km** (the owner's figure, the existing `DELIVERY_RADIUS_KM` setting). **The ONE fact with no safe default is the store's MAP COORDINATES** (lat/lon for the distance check) — a wrong coordinate delivers to the wrong address, so delivery serviceability stays FAIL-SAFE (switched off, never a guessed 0,0) until the owner sends the two numbers. Recorded OA-11 answered (partly) in the register + owner summary; M20-FR-03 RTM note updated (stays PARTIALLY WIRED). NEXT increment (needs the coordinates): feed the slot config to the served customer app so it offers the slots, and enable serviceability once coordinates are set. Earlier: **◆ OWNER ANSWERED OA-13 (guest browse) + OA-4 (keep payment test mode) — M20 RECORD CORRECTED ◆**: investigating the owner's guest-browse choice corrected a WRONG earlier premise. The customer app (M20) is NOT engine-only and was NOT blocked on customer-auth: it is already a served, offline-first, **guest-by-design** storefront — `edge/store-edge` `customerPayload` feeds it the published catalogue (products, prices, availability, pack version, slots, consent purposes) and it boots the real `apps/customer-app` shopping session over the socket. Integration-proven (`tests/integration/the-screens-are-fed.test.ts` — "the customer app opens a real catalogue, tied to the pack version": search returns the right product, the pack version travels so a stale price is refused not repriced, P-02), offline-guarded (`every-screen-opens-without-a-network` includes 'the customer'), honesty-guarded (`the-customer-app-is-honest` — consent-withdraw symmetry/DPDP, "prepared is not placed"/the inverse of the till, no card data ever), bilingual. **No login — guest by design, exactly the model the owner chose.** So **M20-FR-01 (guest catalogue browse/search) and M20-FR-02 (cart review before checkout) are WIRED today**; FR-03 (checkout) is PARTIALLY WIRED — the served screen wires slot-choice/place-order/payment and enforces the refusals (uncertain payment pending §4.3, PAN refused #3, out-of-area refused) in payment TEST MODE per the owner's OA-4 choice, but real delivery slots need OA-11 and real payment needs a provider (OA-4/EX-03); FR-04 (privacy centre) is PARTIALLY WIRED (served, ties to M16 consent). **M20 module corrected ENGINE ONLY → PARTIALLY WIRED; stale domain label corrected "Promotions" → "Customer app/storefront".** RTM/backlog/family counts updated (built 141→139, partial 3→5; M20 2 built/2 partial; family M16–M17,M20–M21 → Built (14/16, 2 partial)). OA-13 CLOSED (answered); OA-4 pilot posture (test mode) confirmed, open for production. Owner register + one-page decisions summary updated. **The ONE remaining decision to unlock a FULL online order for the pilot is OA-11 (delivery areas — slots).** Two numbers: requirements/design ≈85%; wired & verified ≈36% (M20 gained 2 wired FRs). Earlier: **◆ CLEAN STOP: unblocked cloud wires done; owner decisions consolidated ◆**: after this session's run (M11 completed, M18 lifecycle/reservation/cancel-release wired, M36-FR-03 export + FR-02 branding + FR-01 entitlement control plane wired, M15 completed), an honest completeness sweep + engine-only FR enumeration finds that the remaining unwired work is NOT freely unblocked: it is gated by an owner decision (OA-4 payments/IdP, OA-11 delivery serviceability, OA-12 plans/pricing, OA-13 customer-auth), an external provider, or the **stock-valuation-method accounting-policy** decision (which gates cost/GMROI/margin analytics — M08-FR-04, M29 margin, M23 valuation — the same authoritative-cost-source gap M11-FR-02 navigated). Rather than manufacture marginal work or force a hollow wire, the open decisions are consolidated for the owner in **`docs/OWNER-DECISIONS-SUMMARY.md`** (one plain-English page: each decision, a recommended first step, and what it switches on), with full detail in `docs/OWNER-ACTION-REGISTER.md`. Two numbers: requirements/design ≈85%; wired & verified ≈35% (6 modules fully WIRED, 26 PARTIALLY WIRED, 1 ENGINE ONLY=M20-storefront which is OA-13-blocked). The build resumes the moment any decision is answered. Earlier today: **★ M15 LOSS-PREVENTION COMPLETE — M15-FR-03 DUPLICATE-BANK DETECTION WIRED (API-05) ★**: the last M15 FR, now all four are live. `GET /v1/fraud-signals/duplicate-bank` runs the authoritative `detectDuplicateBankAccounts` over each supplier's CURRENT bank account — folded from the already-captured, append-only `SupplierBankChanged` ledger (latest change per supplier wins), reusing the wired M06 supplier-bank capture, NOT data the caller supplies — and returns the flags plus the blocked-holder set. Two DISTINCT suppliers sharing one account (the classic invoice-redirection fraud — a real supplier's real address saying "the account changed", money leaving on the next payment run) is flagged and BLOCKED PENDING REVIEW; the same holder listing an account twice is not a duplicate; a supplier moving to its own account clears the flag. Detect-and-hold, released by a person, never silently reversed (hard rule #5, §28); masked/tokenised refs (PRV); per-tenant §35; RBAC `lp.case.read` (owner/store_manager/accountant, already granted — no new permission). Integration-tested (`tests/integration/duplicate-bank.test.ts`, 3 cases: flag+block two suppliers on one account, latest-account fold clears a moved-away flag, per-tenant isolation + cashier 403). **M15-FR-03 → WIRED; M15 module → WIRED (all four FRs live: anomaly rules FR-01, cross-domain fraud signals FR-02, investigation cases FR-04, duplicate-bank FR-03) — added to the completion-ladder EVIDENCE map.** Also recorded **OA-13** (customer identity/auth model) — M20 storefront is the last engine-only module but the API authenticates every request against STAFF roles and a shopper is not staff; there is deliberately no public route class, so the customer app cannot be wired until the owner decides guest-browse vs OTP account (OTP needs an external SMS provider). This is a genuine owner decision surfaced, not guessed. NEXT: the clearly-unblocked cloud wires are largely done; remaining big items (M20 storefront, M18-FR-03 routing, M36 paid-plan/partner tiers, payments/settlement) are owner-blocked (OA-4/11/12/13) or need external providers — the next step is to deepen a PARTIALLY-WIRED module toward WIRED where unblocked, or consolidate the open owner decisions for the owner. Earlier today: **★ M36-FR-01 ENTITLEMENT CONTROL PLANE WIRED (API-11) ★**: per-tenant optional-feature entitlements, DEFAULT-OFF (a tenant gets only what it enables). `PUT /v1/platform/entitlements/:feature` {enabled} turns an optional module (delivery, b2b, loyalty, customer_app, the conditional departments — the `OPTIONAL_FEATURES` catalogue) on/off for a tenant, append-only + audited (who turned it on, when), refusing a feature the product does not offer (404 unknown_feature — never invented) and a change with no state (400). `GET /v1/platform/entitlements` returns entitled-vs-catalogue so a tenant SEES ONLY ITS ENTITLED MODULES; `GET /v1/platform/entitlements/:feature` returns the authoritative DEFAULT-DENY decision with its SOURCE ('not_entitled' = a sales conversation, 'suspended' = a billing one) through the real `checkEntitlement` engine (enabled features are explicit grants against an empty base plan until paid plans exist). This tenant only (§35); RBAC `platform.entitlement.read`/`manage`, owner-held. **HONEST SCOPE (per completeness-sweep discipline — do not force a hollow wire): this is the CONTROL PLANE. Hard per-route enforcement (refusing an actual optional-feature call) is a deliberate FOLLOW-ON — every optional-feature route family (delivery/b2b/loyalty/customer) has integration tests that call it without enabling the feature, so gating them is a broad retrofit; the decision is exposed for consumers but not yet enforced at those routes. The PAID-PLAN tier (priced plans, metered ceilings, suspend-for-non-payment) is owner-blocked on the commercial plans/pricing model (recorded as OA-12).** Integration-tested (`tests/integration/tenant-entitlements.test.ts`, 4 cases: default-off→enable→revoke with source, unknown-feature + no-state refused, cross-tenant isolation, cashier 403). M36-FR-01 → **PARTIALLY WIRED**; M36 module now has FR-01 control plane + FR-02 branding + FR-03 export live (partner FR-04 still engine-only). NEXT: M36 remaining pieces are follow-on/owner-blocked; the honest next module is **M20 storefront** (the last ENGINE-ONLY module) — but it needs a customer-auth realm decision (guest-browse vs OTP account) which must be surfaced to the owner via the register before wiring, NOT guessed. Earlier today: **★ M36-FR-02 WHITE-LABEL BRANDING WIRED (API-11) ★**: one codebase, one deployment, many brands — no code fork. `PUT /v1/platform/branding` sets a tenant's brand as CONFIGURATION, validated first through the authoritative `validateBranding`: a renamed statutory term (a "tax invoice" called a "bill" is not a tax invoice — protected list incl. GST/HSN/credit note/MRP), an unreadable colour pair (WCAG AA contrast, blocking — a cashier who can't read the total at 8pm is a support call every day for a year), or an invalid hex is REFUSED 422 with nothing stored. The brand is versioned append-only, and the tenant is ALWAYS the caller's (set server-side, never a client field), so a rebrand can never be aimed at another retailer's paperwork (§35). `GET /v1/platform/branding` resolves via `resolveBrand` — every unset field falls back to a NEUTRAL default, NEVER to another tenant's (a missing logo showing the previous tenant's mark is a retailer invoicing under a competitor's name), rebuilt fresh each call so no shared object leaks. New RBAC `platform.branding.read`/`platform.branding.write` (owner-held, grantable). Integration-tested through the real pipeline + per-tenant RBAC (`tests/integration/tenant-branding.test.ts`, 4 cases: neutral-before/brand-after, protected-term + low-contrast refused with nothing stored, cross-tenant isolation incl. a client that cannot aim a rebrand at another tenant, cashier 403). M36-FR-02 → **WIRED**; M36 module stays PARTIALLY WIRED (now FR-02 branding + FR-03 export live; plans/entitlements FR-01 and partner FR-04 still engine-only). NEXT: M36-FR-01 entitlement enforcement is the remaining wireable platform FR, but its 'denied outside plan' needs a real feature-gate that won't break existing optional-route tests — investigate a safe gate first; else M20 storefront (needs a customer-auth realm decision). Earlier today: **★ M36-FR-03 TENANT DATA EXPORT WIRED (API-11) ★**: a tenant owns and can export its data (P-06/OD-09). `GET /v1/platform/export` reads the caller's WHOLE ledger via a new tenant-scoped `EventStore.exportTenant` (added to the interface + both InMemory and Sql implementations; `WHERE tenant_id = $1` is the isolation boundary, §35), groups it into the product's declared data domains (the STREAM roots) and runs it through the authoritative `buildTenantExport` engine — so the export is CERTIFIED complete-or-refused: every declared domain PRESENT (with zero rows where empty, because absence and emptiness are different facts), each carrying a sha256 checksum (an export nobody can verify is a file, not evidence), and no foreign-tenant data. Deriving the domain list from the same STREAM constant the writes use is what stops the exporter falling behind the product. The route returns the certified manifest AND the data grouped by domain; it is a READ that removes nothing (#6). New RBAC `platform.tenant.export` (owner-held, grantable). Integration-tested through the real pipeline + per-tenant RBAC with TWO tenants side by side proving isolation (A's export contains A's product and never B's), the export is idempotent/stable (checksums identical across two exports), and a cashier is refused 403 (`tests/integration/tenant-export.test.ts`, 4 cases); the new store method is contract-tested on both InMemory and Sql (`tests/unit/persistence-event-store.test.ts`). M36-FR-03 → **WIRED**; M36 module → PARTIALLY WIRED (plans/entitlements FR-01, branding FR-02, partner FR-04 still engine-only; closure/retention & upgrade-compat within the FR-03 engine also remain engine-only). Also recorded **OA-11** (AVR-13 delivery serviceability model) as the owner blocker for M18-FR-03 routing: the routing engine is built+tested but cannot become a usable API feature until the owner supplies per-location radius/slots/dark-store/express figures — genuine business decisions the build may not invent; noted and moved on per the completeness-sweep discipline. NEXT: continue the platform/unblocked modules (M36-FR-01 entitlement enforcement or FR-02 branding next; M20 storefront needs a customer-auth realm + OTP). Earlier today: **★ M18 ORDER LIFECYCLE + RESERVATION/ATP + CANCEL-RELEASE WIRED (API-07) ★**: the next module after M11. The order-promise route already reserved stock in the same breath (no oversell, §6.2); this increment wires the **order lifecycle** and the **cancellation-releases-reservation** half of FR-04 to the live cloud API, connecting the `packages/orders/lifecycle` engine that was engine-only. `POST /v1/orders/:orderId/promise` now also records the order (idempotent on the order id) so it exists to be read and moved; `GET /v1/orders/:orderId` reads its lifecycle end-to-end (state, location, lines, this order's still-holding reservations); `POST /v1/orders/:orderId/transition` moves it by ALLOWED steps only through the authoritative `transitionOrder` — an illegal step is refused 409 and never applied, an unknown order 404, a missing event 400. A `cancel` transition gives EVERY one of the order's reservations back in the SAME step — a compensating `ReservationReleased` per hold on the location stream (append-only, hard rule #2), and `outstanding` now folds those releases so the stock is immediately visible to the shop floor again and re-promisable (a cancel that forgets the release is the commonest phantom out-of-stock, P-08). New RBAC `order.read` + `order.lifecycle.manage` (owner + store_manager, grantable). Integration-tested through the real pipeline + real per-tenant RBAC + stock projected from the real inventory ledger (`tests/integration/orders-lifecycle.test.ts`, 5 cases: reserve-and-never-oversell, lifecycle end-to-end, illegal/unknown/empty refused, cancel-returns-the-shelf, authz+tenant-isolation+idempotent). M18-FR-01 and FR-02 now **WIRED**; FR-04 **PARTIALLY WIRED** (cancel-release live; substitution/returns/refunds/channel-reconciliation remain engine-only, awaiting the finance/refund surface M23); FR-03 routing/slots engine-only. M18 module → **PARTIALLY WIRED**. RTM ladder + backlog counts + M18–M19 family status updated. NEXT: completeness sweep → next module (M18-FR-03 routing needs slot/capacity config; substitution/refund needs M23; consider M20 storefront non-payment FRs or M36 platform). Earlier today: **★ M11 FRESH-FOOD/PRODUCTION COMPLETE — ALL FOUR FRs WIRED ★**: M11-FR-02 authoritative costing is the last piece. `POST /v1/production/costs/:productId` registers a governed ingredient cost; run commit values inputs at the REGISTERED cost, never a per-run body figure (a run cannot value its own margin — a request cost of 9999 is ignored). An uncosted ingredient is NOT faked as zero (100% margin lie) — the physical run still commits (a missing cost must not stop the cafe) but its cost is marked `costKnown:false` with the uncosted ingredient named (P-08). production.test.ts now 15 assertions. M11 module marked **WIRED** in the RTM ladder + added to the completion-ladder-has-evidence EVIDENCE map. Full M11 = FR-01 recipes/raw-material-issue/finished-batch + FR-02 authoritative costing + FR-03 quality release + FR-04 departments/scale-labels, all on the live API with real RBAC + per-tenant. NEXT: completeness sweep → next module. Earlier today: **M11-FR-04 DEPARTMENTS + SCALE LABELS WIRED**: `POST /v1/production/departments/:departmentId` enables a supported department per-tenant (unsupported → 400 unknown_department; SRE enables the cafe per OB-04), `GET /v1/production/departments` lists operated vs the full catalogue; producing for a department the store does NOT operate is refused (422 department_not_operated, naming what it does — enforced at run-commit); and `POST /v1/production/runs/:runId/label` issues a pack/scale label via the authoritative buildPackLabel/renderLabel, refusing an incomplete label (missing net qty / packer / allergens / weight) per the department's Legal-Metrology + food-safety flags (§9.3). production.test.ts now 12 assertions. **M11 now has FR-01 + FR-03 + FR-04 wired; only FR-02 (yield/costing — source ingredient costs from the catalogue) remains, then M11 → WIRED.** Earlier today: **M11-FR-03 QUALITY RELEASE WIRED** (the gate that makes produced stock sellable): `POST /v1/production/runs/:runId/release` moves a run's finished batch out of quarantine to sellable through the authoritative `releaseForSale` engine — refused for a failed check (422 qc_failed), an already-released batch (409), or (engine-covered) an unnamed releaser / expired batch; the batch stays in quarantine until a NAMED person passes it; `GET /v1/production/runs` shows each run's `released` state. New `production.release` permission (owner + store_manager, grantable so QC sign-off can be granted apart from production). Integration-tested (cashier refused + per-tenant); production.test.ts now 9 assertions. M11 now PARTIALLY WIRED with FR-01 + FR-03; FR-02 yield/costing (source ingredient costs from the catalogue) + FR-04 departments (SRE enables the cafe per OB-04; the department-scoped scale-label issue rides here) remain. Earlier today: **M11-FR-01 IN-STORE PRODUCTION WIRED**: the next module after OA-9. `POST /v1/production/recipes/:recipeId` registers a validated recipe/BOM; `POST /v1/production/runs/:runId` commits a run through the authoritative `produceBatch` engine — ingredients checked against M08 on-hand MINUS what prior runs at that location consumed (production is state-aware and keeps its own append-only stream layered on M08, as counts/warehouse do), a short run refused before anything is consumed (422), the finished batch created with its own id + expiry into quarantine (never sellable until quality releases it), cost & yield measured; `GET /v1/production/runs` reads them. Idempotent on run id (409), recipe_not_found→404, real RBAC (production.recipe.manage / production.plan.commit / production.read on owner+store_manager) + per-tenant. Integration-tested (`tests/integration/production.test.ts`, 5 assertions). Also corrected a stale RTM ladder label (M11 was mislabelled "Loss-prevention"; it is Fresh food/production). M11 now PARTIALLY WIRED (FR-01); FR-02 yield/costing, FR-03 packing/scale-labels + quality release, FR-04 departments next. Earlier today: **★ OA-9 COMPLETE END TO END ★**: the dual-interface warehouse the owner approved is fully delivered. (1) The offline scanner-first **Warehouse PWA** (handheld): receiving/GRN via the authoritative receiveScan (§28 over-delivery/DSD approval), scan-to-bin put-away via applyMovement (unknown-bin queued not invented, full-bin/over-draw refused, recalled/expired kept out of pickable bins via FEFO), felt scan feedback (colour+sound+vibration), offline queue + idempotent sync, English+Tamil with a tripwire — served over the store box's socket. (2) The **Web ERP supervisor** screen: bin configuration + occupancy, stock visibility, and the exception queue (P-03: negative/over-capacity/recalled-in-pickable), plus the supervisor ACTIONS — §28 approvals (reason codes, self-approval refused, queued), transfer planning (advisory; dispatch stays a separate server-side §28 step), and task assignment — all English+Tamil with tripwires, served over the socket. BOTH interfaces read the SAME authoritative warehouse/inventory services with NO duplicated logic. The final slice added the plan-transfer/assign-task BUTTONS (`apps/web-erp/web/warehouse.js`), proved over the socket. Also fixed a flaky test helper (tamperSignature) that had intermittently failed the auth suite ~1-in-16 — a test-support bug, not an auth flaw; the fix makes the security test stricter. NEXT: honest completeness sweep to pick the next unblocked roadmap module. OA-10 (non-blocking) tracks a native-Tamil review of all warehouse wording. Earlier today: **OA-9 SUPERVISOR TRANSFER PLANNING + TASK ASSIGNMENT — write-paths built**: the last two supervisory write actions (data-path first). `proposeTransfer()` builds a PROPOSED transfer on the authoritative `Transfer` aggregate and queues `WarehouseTransferProposed` — advisory, moves no stock; dispatch (SEPARATE approver §28, recalled/quarantined never sent) stays on the cloud service unchanged. `assignTask()` assigns a put-away/count/bin-to-bin/replenish task to a worker (recording the assigner) and queues `WarehouseTaskAssigned`. Both idempotent on id (append-only #2), queued to the ERP outbox like approvals, refusing invalid input and refusing at all when the box hasn't said who the supervisor is. Integration-tested (`tests/integration/warehouse-supervisor-fed.test.ts`, 13 assertions total). REMAINING for OA-9: the plan-transfer / assign-task BUTTONS on the served screen (bilingual + tripwire) — then M09/OA-9 is COMPLETE end to end. Earlier today: **OA-9 SUPERVISOR APPROVE/REJECT BUTTONS on the served screen**: the approvals queue is now on the supervisor's warehouse screen with working Approve/Reject buttons — actionable rows carry both; a row the supervisor cannot decide shows the reason (own request / out of scope / above limit) not a dead button (§28 visible). A decision picks a reason CODE from an on-screen list (never prompt/confirm/alert, never free text), calling the tested `decide()` which queues the decision through the authoritative engine to the ERP outbox (same in-memory outbox as the manager screen); a row is not offered twice. English+Tamil for every reason code / not-actionable reason / heading, with the completeness tripwire now bound to packages/approvals APPROVE_REASONS/REJECT_REASONS. Proved over the real socket (`the-screens-are-fed`): an approval served to the screen is decided and queues a `WarehouseApprovalDecided` event. REMAINING for OA-9: task assignment + transfer planning — then M09/OA-9 is COMPLETE end to end. Earlier today: **OA-9 SUPERVISOR APPROVALS — write-path built (§28)**: the first supervisory WRITE action. The box's `warehouse` section now also carries the pending §28 approvals the floor raised (stock-adjustment / count-variance / over-delivery) and who the supervisor is (id, branch scope, approval limit). The supervisor session's `approvalQueue()` presents them via the AUTHORITATIVE `buildQueue` (the supervisor's own request is shown-but-not-actionable, §28; out-of-scope/over-limit shown as escalate) and `decide()` goes through the AUTHORITATIVE `submitDecision`→`decide` (§28 self-approval forbidden, mandatory reason CODE from the shared vocab checked before the engine, scope + authority) — no rule re-implemented. A decided request is queued as `WarehouseApprovalDecided` for idempotent sync (keyed on id+outcome, append-only #2); a refusal queues nothing; queue answers NOT KNOWN when approvals/supervisor absent (P-08). Integration-tested (`tests/integration/warehouse-supervisor-fed.test.ts`, 9 assertions total). REMAINING for OA-9: the approve/reject BUTTONS on the served screen (bilingual reason vocab + tripwire), then task assignment + transfer planning — then M09/OA-9 is COMPLETE. Earlier today: **OA-9 WEB ERP SUPERVISOR IS A SERVED SCREEN**: the supervisor's oversight is now the eighteenth served screen (a web-erp page sharing the ERP bundle) — `apps/web-erp/web/warehouse.html` + `warehouse.js`, registered across SCREENS/GLOBAL_FOR/BUILDERS/APP_SHELL, served by the store box with its data injected above the bundle, driven by the tested `WarehouseSupervisorSession` (view holds no rule; renders bins()/stock()/exceptions()). Meets the same offline contract as every screen (SW registered+guarded, network-first, stale-stamp in local time, versioned cache). Three oversight sections — bin configuration+occupancy, stock visibility, exception queue (P-03) — showing NOT KNOWN rather than an empty/healthy warehouse when contents absent (P-08). English+Tamil keyed to `WAREHOUSE_EXCEPTION_KINDS` with a completeness tripwire (Tamil pending review, OA-10 extended). Proved over the real socket (`the-screens-are-fed`): occupancy/stock/over-capacity exception all show. **The OA-9 supervisory READ surface is now complete; the LAST OA-9 piece is the supervisory WRITE actions (approvals, task assignment, transfer planning) queued to the cloud — after that M09/OA-9 is COMPLETE end to end.** Earlier today: **OA-9 WEB ERP SUPERVISORY SURFACE — data-path built**: the supervisory half of the dual-interface warehouse. The box's `warehouse` pack section now also carries current bin **contents**, and `warehouseSupervisorPayload` presents the SAME section to a tested `WarehouseSupervisorSession` (`apps/web-erp/src/warehouse-supervisor-session.ts`) — bin configuration (always), occupancy + stock visibility (reusing `packages/warehouse` `binOccupancy`, no duplicated rule), and the EXCEPTION QUEUE (P-03): negative bins, over-capacity bins, and recalled stock in a pickable bin. Holds the box's one rule: contents never sent are `known: false` (not an empty/healthy warehouse). Integration-tested (`tests/integration/warehouse-supervisor-fed.test.ts`, 4 assertions). Handheld & supervisor read one authoritative source, so they cannot disagree. REMAINING for OA-9: the supervisor's served screen shell (web-erp `warehouse.html`) + supervisory WRITE actions (approvals, task assignment, transfer planning) queued to the cloud. Earlier today: **OA-9 WAREHOUSE PWA IS A SERVED OFFLINE SCREEN (the execution surface is complete)**: the Warehouse handheld is now the seventeenth served screen — registered in SCREENS/GLOBAL_FOR/BUILDERS/APP_SHELL, served by the store box's screen socket with its assignment injected above the bundle, driven by the tested offline `WarehouseSession` (same authoritative engines, no logic in the view). It meets the same offline contract as every other screen (service worker registered + guarded, page network-first so a cached page is never a cached payload, cached copy stamped with when it was taken in local time, per-app versioned cache, tolerant build-artefact caching, missing-script-never-a-page). Scanner-first with NO text box; every scan result is FELT — colour + word + sound + vibration — so a refusal isn't missed in a cold store. Words are English AND Tamil keyed to the session's `FEEDBACK_CODES` with a completeness tripwire (`tests/guardrails/the-warehouse-screen-speaks-both-languages.test.ts`); the Tamil is pending native-speaker review (OA-10, non-blocking). Proved over the real socket (`tests/integration/the-screens-are-fed.test.ts`): the box serves the assignment, the session boots and puts stock away into a real bin. **The OA-9 offline Warehouse PWA is now complete end to end (engine → feed → served screen → offline). REMAINING for OA-9: the Web ERP supervisory surface** (bin config, stock visibility, transfer planning, task assignment, approvals, exception queues, discrepancy investigation, reports/KPIs/audit) reading the same authoritative cloud warehouse endpoints. Earlier today: **the box FEEDS the handheld from the cloud pack**: the store box carries a `warehouse` section in its pack (bins, scanning catalogue, what is on order, goods-in awaiting put-away, recalls) and `warehousePayload` (`edge/store-edge/src/screen-data.ts`) turns it into the assignment the offline `WarehouseSession` boots on — holding the box's one rule (a pack that never mentioned warehouse work serves NOTHING, not an empty assignment; `emptyPack` starts the section not-known; the `?? []`/`?? {}` idiom stays banned). Proved end to end (`tests/integration/warehouse-fed-from-pack.test.ts`): readPack → warehousePayload → the real session boots, receives and puts away into a real bin; no-warehouse pack → null. STILL TO COMPLETE THE SURFACE: register `warehouse` as a served screen (shell + service worker + bundle + SCREENS/GLOBAL_FOR/APP_SHELL wiring) with the bilingual English+Tamil scan-feedback vocabulary + completeness tripwire, then the Web ERP supervisory surface. Earlier today: **OA-9 WAREHOUSE PWA — execution engine + assignment feed built (data-path first)**: the offline, scanner-first `WarehouseSession` (`apps/warehouse-app/`) now receives at the back door and puts stock away into bins, ORCHESTRATING the authoritative engines with NO duplicated logic — `packages/receiving` (`receiveScan`: unknown-barcode→resolution queue, exact case conversion, price-edit refused, over-delivery/DSD needing a SEPARATE approver §28), `packages/warehouse` (`applyMovement`/`suggestPutAway`: unknown-bin queued not invented, full-bin/over-draw refused, bad stock kept out of pickable bins), and `packages/fefo` (recall/expiry folded into the stock state so the one gate keeps them out of pickable bins). Every accepted action queues for idempotent sync (a put-away keyed on the same command id the cloud ledger uses — hard rule #1), a refusal queues nothing (#2), and every scan returns a feedback signal (accept/warn/reject + sound + vibration length) so a rejected scan is felt in a cold store; boots on the served assignment via `bootWarehouse`; integration-tested (`tests/integration/warehouse-screen-fed.test.ts`, 10 assertions). Its visual shell (bilingual English+Tamil strings, service worker, screen-server socket wiring) + the Web ERP supervisory surface are the next work packages. Earlier today: **M09 BACKEND COMPLETE: M09-FR-04 blind cycle counts WIRED**, so all four M09 FRs (bins/put-away, replenishment, transfers, counts) are now on the live API and M09 is marked **WIRED**. The count is BLIND — the counter never sends the expected quantity; the server computes it (the authoritative M08 position **plus** the sum of prior count corrections, since M08's kind+positive movement model cannot express a signed correction, so the count keeps its OWN append-only correction ledger layered on M08); a match reconciles with no adjustment; a variance commits a reason-coded compensating adjustment through the real engine; a MATERIAL variance is refused (422) until a SEPARATE approver clears it — the counter can never approve their own variance (§28); append-only (#2), idempotent on the count id; integration-tested through the real API + RBAC + per-tenant isolation. **Next per OA-9: the offline-first scanner Warehouse PWA (execution) + Web ERP supervisory surfaces, both on these same authoritative warehouse/inventory services (no duplicated logic).** Earlier today: **M09-FR-02 replenishment WIRED** (advisory reorder proposals up to max level, pack/MOQ rounding, demand×lead reorder points, blocked suppressed, never auto-committing a PO). Earlier today: **M09-FR-03 warehouse & inter-store transfers WIRED** (in-transit-at-destination, separate-approver dispatch, recalled/quarantine never sent, valued shortfall on receipt, days-of-cover allocation); and **OA-9 CLOSED** by the owner — the dual-interface warehouse (offline-first scanner Warehouse PWA + Web ERP supervisory, both on the same authoritative services); next: finish the M09 backend (FR-02 replenishment, FR-04 counts) then build the two surfaces. Earlier today: **M09-FR-01 warehouse put-away & bin movements WIRED** (opened up the warehouse module): bins registered with capacity/pickability; scanned movements applied against projected contents with a double-scan no-op and unknown-bin/full-bin/over-draw/quarantine-into-pickable refusals; a put-away suggestion that keeps a product together and never sends bad stock to a pickable bin; on API-04, integration-tested through the real API + RBAC. Earlier today: **M15-FR-02 cross-domain fraud signals WIRED**: per-tenant thresholds stored, and a detect-only evaluate over coupons/loyalty/COD/suppliers returns one prioritised list (strong→weak), every signal actionTaken:false and linking its evidence — a signal is a reason to look, never a verdict (A07/AI-NFR-12); on API-05, integration-tested through the real API + RBAC. M15 now has FR-01 + FR-02 + FR-04 wired; only FR-03 duplicate-bank-account detection remains engine-only. Earlier today: **M15-FR-01 loss-prevention anomaly rules WIRED**: a store configures its own void/refund/discount/no-sale/cash thresholds as DATA (no code) and already-synced activity is evaluated into linked exceptions, detect-only, spike→escalate; on API-05, integration-tested through the real API + RBAC. Earlier today: **M15-FR-04 loss-prevention investigation cases WIRED**: open a case from a raised signal (never self-investigated), append SEALED evidence with a mandatory chain of custody (append-only — no edit/delete, the event stream IS the chain), close with an outcome where proven needs a second signer (§28) + evidence + a verifying chain, and a rule-feedback loop that retires an always-unfounded rule; on API-05, integration-tested through the real API + RBAC. Earlier today: **M05 fully WIRED**: FR-03 the promotion catalogue & best-price basket evaluation now on the live API (define→activate→stop a promotion, then evaluate a basket deterministically with `bestPrice` — only active+in-window apply, exclusive-group best-only + stacking, member/coupon gates, never below zero), joining FR-01 price list + FR-02 governed change + FR-04 promotion governance; integration-tested through the real API + RBAC. Earlier today: **M05-FR-01 the effective-dated, scoped price list WIRED**: publish a scoped (customer/channel/zone/store), effective-dated, append-only price entry with a version per (scope, ref); resolve the one price that applies at a moment by precedence and lock the sale to it (a future price does not activate early); read the chronological history; above MRP refused at any scope (legal ceiling) and back-dating refused; integration-tested through the real API + RBAC. Earlier today: **M22-FR-02 the FULL B2B document chain WIRED end to end** (quote → order → proforma → challan → tax-invoice + chain check): each document on its own gap-free series drawn ONCE and only on success; conversion at the QUOTED price through the real credit gate; a proforma never tax-claimable; a challan carrying what LEFT with over-delivery refused; the tax invoice built from the CHALLANS so partial delivery bills partially and cannot exceed delivered; a chain check naming delivered-but-not-invoiced; all integration-tested through the real API + RBAC. All four INTERNAL M22 FRs (credit, commission, collections, document chain) are now on the live API — the only M22 gap left is the customer-facing collections portal (external B2B-customer auth). Earlier today: **M22-FR-03 salesperson commission WIRED** — earned not stated, computed exactly by the engine on the server, the total projected from the accruals never stored, a re-send collapsing on the accrual id, with a record-vs-read SoD split. Also **OA-9 recorded** in the Owner Action Register — the genuine design decision for the first on-screen increment (offline in-store pack vs a separate cloud office screen for pure back-office data), with a recommended default and two alternatives, surfaced rather than guessed. Earlier: PROJECT RECOVERY KICKOFF — read-only audit accepted as the implementation baseline; Phase 0 baseline/status reconciliation + Phase 1 CRITICAL security repair done: per-tenant RBAC now genuinely enforced on the live API surface (it was inert — `AccessControl([],[])`), with a guarded genesis-owner bootstrap and an authenticated+authorized E2E; completion-status vocabulary adopted, 5 dropped D-FR rows restored, ASSEMBLY/WIRING workstream opened, and `docs/OWNER-ACTION-REGISTER.md` created. See the PROJECT RECOVERY section at the top. Earlier: durable tenant settings — setup answers now persist through the append-only config_versions store and survive a restart, PR #19; see the durable-settings milestone below. Just before it, self-service store setup shipped COMPLETE across PRs #15–#18 — built-in thermal receipt templates, the setup engine with defaults and block-until-given, the API-11 read/answer endpoints with optimistic concurrency and an audit trail, and the bilingual Store setup page with FULL inline editing; see the store-setup milestone below. Earlier the same day: the pilot-preparation pack across PRs #11–#13 — the store go-live checklist, the set-up workbook in document and Excel form, and the day-by-day pilot run-sheet, all CI-green; see the pilot-preparation milestone below. Earlier still, PR #10 merged the eleven ERP screens and six apps with CI green — see the 7 August milestone below; the session that built it: the buyer's screen, offline shells switched on for real, products and prices, shelf addresses, the pick zone order, merchandising and space, reporting and analytics, the day boundary the store box did not have, the service desk, expiry and recall — including the recall block that never reached a till — finance, which lets a month close for the first time, admin and security — including a security control that existed twice and enforced less where it counted, AI control, including the kill switch that stopped nothing — and migration, including the cutover gate that had only ever been ticked by hand)
