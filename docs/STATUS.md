...
Last updated: 11 August 2026 (**★ CORE-05 inc1 — COMPLIANCE OBLIGATION REGISTER, ON THE CLOUD ★**: the owner chose to close the 'thin services' gap so the % toward a runnable shop climbs. This wires `packages/compliance` — a complete, tested licence/certificate/obligation register that NOTHING on the cloud fed — onto the live API as a new persisting service (`services/compliance`, API-11). `POST /v1/compliance/obligations/:id` registers a licence/scale-stamping/food-licence with its NAMED responsible person and expiry (append-only — a change is a new fact, never an overwrite; hard rules #2/#6), `GET /v1/compliance/alerts` folds `expiryAlerts` (worst-first, escalating, an expired one keeps shouting), and `GET /v1/compliance/status` folds `isCompliant`. Event-sourced (obligations projected from an `ObligationRegistered` stream, later-registration-wins), gated new permissions `compliance.obligation.manage`/`.read` (owner/store-manager). **This lands M34-FR-03 on the running surface AND delivers the two ratified R2 items it subsumes — B7 (weighing re-verification alerts) and B10 (FSSAI-licence expiry), since a scale stamping and a food licence are both obligations.** `tests/integration/compliance-obligations.test.ts` proves register + worst-first alerts + compliance status + the named-person/kind validation + RBAC. Full gate green (typecheck, lint, secret-scan, **4,870 tests / 350 files**). **Metric moved: test-only packages 15 → 14 of 77** (down from 35 at the audit) — the honest 'thin services' number in the assembly-state doc, updated to match. This is the first of the CORE-05 workstream (wire the remaining ~10 unused domain engines: quality, self-checkout, reversal, suspended-sales, notifications, owner-control, ops, org, documents). R2 self-contained scope: 16 requirements built + B7/B10 now via this wiring. Context: the clean scalar-calculator vein is done; this write-path wiring is the owner-chosen next move to raise the runnable-shop %.)
