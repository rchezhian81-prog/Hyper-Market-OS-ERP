...
Last updated: 12 August 2026 (**★ A5 FROM-SALES — FREEZE-AT-SUPPLY: A MID-PERIOD RATE CHANGE NOW FILES CORRECTLY ★** (M23·M12): the owner-chosen follow-on that closes the last named GST-return boundary. Until now the return applied one (current/catalogue) rate across the whole period, so a product whose GST rate changed **mid-period** could not file each sale under the rate that actually applied. Now a sold line may carry its **own** tax facts, **frozen at the time of supply**: `IncomingSaleLine.hsnCode`/`taxRateBps` on the cloud intake, carried by `soldTaxLines`, and PREFERRED by `salesToOutwardSupplies` over the period table (both must be present and well-formed). So the two rates stay as **separate HSN/rate rows, never blended**; `frozenLineCount` reports how many lines used them. It is a **record, never a control** — a wrong/absent value can never refuse the sale (hard rule #1); at worst the line falls back to the catalogue mapping (Option A) or surfaces as `unmapped`. **5 net-new tests** (engine: frozen-wins, mid-period two-rate, partial-fallback; integration: frozen-only end to end, mid-period end to end). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite — 5152 passed). **This is the cloud-carry + engine-prefer half** — an import of historical sales can set these fields TODAY; the **named remaining follow-on** is the offline till stamping the pack's HSN/rate onto each line at commit (the pack already carries them since Option A), which makes future till sales self-freezing. Prior: A5 Option A (#177), A5 from-sales (#176), D-2 order-proposal (#175), C3 retention-clock (#174), C1 consent-notice (#173).)

**R2 SELF-CONTAINED SCOPE STATUS (for the owner — a decision point):** 16 ratified-R2 requirements are now built and merged as self-contained, cloud-side, fully-tested increments — A1/A2/A4/A6/A8/A9/A10/A11/A12/A27/A28 (invoice numbering, the GST tax-invoice suite, statutory retention, non-disableable audit) + B2/B3/B4/B6/B9/B24 (dual-MRP, unit price, pack declarations, verified-scale gate, allergen/veg mark, label heights). Each is a pure, tested engine or guardrail joined to a running route, no offline-lane risk, reusing existing permissions. **The clean self-contained vein is now essentially mined out.** The remaining ratified R2 items fall into three buckets the owner should choose between: (1) ALREADY SERVED by existing generic engines — B7/B10 licence/scale-cert expiry alerts → `packages/compliance/src/obligations.ts` (`expiryAlerts`), B1 above-MRP → `packages/price-list/src/price-change.ts` + `packages/catalogue/src/snapshot-builder.ts`, B25/D-7 price integrity → `packages/self-checkout/src/price-integrity.ts`, A29 append-only → `tests/guardrails/ledger-append-only.test.ts` (these need at most a thin cross-reference, not a fresh build); (2) NEED A WRITE PATH or stored data first — **A5 GSTR-1-from-sales is now DONE end to end** (`salesToOutwardSupplies` folds the return straight from banked till sales; the HSN table auto-sources from the catalogue (Option A); and a mid-period rate change files correctly via tax facts frozen on the sale line — the remaining follow-on is only the offline till stamping those facts at commit, so future till sales self-freeze); what else remains here is any requirement whose enforcement lives at the offline till (B5 weighed-price billing, B8 expired-batch block) which we have deliberately not touched (hard rule #1); (3) NEED AN OWNER DECISION — A20/A21 turnover-triggered e-invoicing, B14/B19 tobacco/plastic gates, and the ten owner decisions (entity type, chart of accounts, product lines, payroll). **Recommended next move:** pause net-new R2 feature work and get the owner's call — take on a write-path increment (e.g. persist invoice lines so A5 Table-12 and price-integrity reconciliation can run on real data), or hold R2 for the pre-deployment owner-decision session. No thin/forced increments will be manufactured in the meantime. Context unchanged: CORE-04 pure-read seam complete; work advances the owner-ratified R2 scope (#106).)
