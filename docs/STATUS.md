...
Last updated: 10 August 2026 (**★ R2 v2.1 B24 — STATUTORY LABEL CHARACTER HEIGHTS, ON THE CLOUD ★**: the ninth ratified-R2 increment, extending the labelling work (B3). Legal Metrology Rule 9 sets a MINIMUM character height for a pack declaration by the size of the principal display panel — a declaration printed too small is a non-compliant label even with every word present. `minLabelHeightMm` gives the minimum by band (≤100 cm² → 1 mm, ≤500 → 2 mm, ≤2500 → 4 mm, above → 6 mm; the bands are named constants — a legal fact, so a rule change is a one-line edit with a failing test, not a silent drift) and `validateLabelHeight` FAILS a self-printed template shorter than the minimum. Runs on `GET /v1/catalogue/label-height?principalPanelAreaCm2=&declaredHeightMm=` (the labelling route in the catalogue service), gated `catalogue.pack.read`. `tests/unit/product-label-height.test.ts` proves the band edges + the too-small-fails rule; `tests/integration/catalogue-unit-price.test.ts` proves the route + RBAC. Full gate green (typecheck, lint, secret-scan, **4,818 tests / 340 files**). No new dependency, no new permission. R2 progress: A1/A4/A6/A8/A9/A10/A11/A12 (tax-invoice suite) + B3/B24 (labelling) = 9 ratified requirements built. Context unchanged: CORE-04 pure-read seam complete (5 increments) and exhausted; work advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
