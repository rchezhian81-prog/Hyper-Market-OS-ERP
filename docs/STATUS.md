...
Last updated: 11 August 2026 (**★ WRITE-PATH inc2 — FULL GSTR-1 RETURN ASSEMBLER (A5) ★**: continuing the write-path work. Added the full GSTR-1 return over the stored outward-supply lines: `GET /v1/finance/gstr1/return?from=&to=` (API-09, `gstr1Return` in `packages/finance/src/gstr1.ts`) assembles **B2B invoice-by-invoice** (per recipient GSTIN, rate-wise per invoice — a B2B document with no GSTIN files as B2C because it cannot be B2B), **B2C rate-wise in aggregate**, and the **HSN Table 12**, ALL from the same stored lines so the sections can never disagree about the same sale. Complements the Table-12 read from inc1. Reuses `finance.gstr.read` (no new permission), the existing outward-supply store and adapter (no schema change). **+2 unit (5 total in `tests/unit/gstr1.test.ts`) + 1 integration (5 total in `tests/integration/gst-returns.test.ts`)**. Full gate green (typecheck, lint, secret-scan, full suite). This is the report side of A5 complete (Table 12 + full return over stored data). Follow-ups: the exact GSTN portal JSON serialization (needs the tenant's GSTIN + filing period config), and auto-feeding sale-commit lines into outward-supplies so the return fills itself from real trading. Prior: write-path inc1 (#147 merged), EINV inc1+inc2, CORE-05 complete, owner decisions ratified.)
