...
Last updated: 10 August 2026 (**★ R2 v2.1 A11 — CGST s.15(3) DISCOUNT ELIGIBILITY, ON THE CLOUD ★**: the third ratified-R2 increment (after A9/A8 #119 and A10 #120). `assessDiscountEligibility` decides whether a discount reduces the GST TAXABLE VALUE — a common, expensive error to get wrong: a discount reduces the tax only when it is ON the invoice (s.15(3)(a)), or — post-supply — **pre-agreed AND invoice-linked AND the recipient's ITC reversed** (all three together, s.15(3)(b)); anything else is a commercial discount only (real money off the price, but the GST stays on the pre-discount value, because charging tax on the net would under-declare output tax the recipient's GSTR-2B exposes). It runs on `GET /v1/finance/tax/discount-eligibility` (the same back-office finance calculator as the MRP extraction, gated `finance.period.read`; not the offline till). `tests/unit/finance-discount-eligibility.test.ts` proves the full s.15(3) truth table — on-invoice wins; the post-supply case needs all three conditions; each dropped in turn makes it commercial-only with a zero eligible reduction; `tests/integration/tax-from-mrp.test.ts` proves the route + the RBAC gate (cashier 403). Full gate green (typecheck, lint, secret-scan, **4,772 tests / 333 files**). No new dependency, no new permission. Context unchanged: the CORE-04 pure-read seam is complete (5 increments, #114-#118) and exhausted; work advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
