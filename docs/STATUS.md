...
Last updated: 11 August 2026 (**★ WRITE-PATH inc3 — GSTN GSTR-1 PORTAL JSON EXPORT (A5 FINISH) ★**: the report/export side of A5 is now END-TO-END. `GET /v1/finance/gstr1/export?from=&to=&gstin=&fp=` (API-09, `toGstnGstr1` in `packages/finance/src/gstr1.ts`) serialises the assembled return into the GSTN GSTR-1 JSON the portal ingests: **paise→rupees** (÷100), **YYYY-MM-DD→DD-MM-YYYY**, B2B grouped by counterparty GSTIN (`ctin` with `inv`/`itms`/`itm_det`), B2C rate-wise (`b2cs`), the HSN summary (`hsn.data`); the supplier `gstin` and filing period `fp=MMYYYY` are supplied per export (validated: GSTIN regex + 6-digit MMYYYY) rather than stored, so any period re-runs. Reuses `finance.gstr.read` (no new permission), the existing store/adapter (no schema change). **6 unit + 6 integration** across gstr1. Full gate green (typecheck, lint, secret-scan, full suite). **GST filing is now computable end-to-end from stored data — record tax lines → HSN Table 12 → full B2B/B2C return → GSTN portal JSON.** OWNER CHECK before first live filing: round-trip the exported JSON through the GSTN offline validation tool (the schema here is faithful to the documented spec but the portal tool is the authority). Remaining A5-adjacent follow-ups: auto-feed sale-commit lines into outward-supplies (needs HSN+tax on the POS line — larger). Prior: write-path inc1+inc2 (#147/#148), EINV inc1+inc2, CORE-05 complete, owner decisions ratified.)
