...
Last updated: 11 August 2026 (**★ R2-A7 — GST COMPENSATION CESS ON TOBACCO / PAN-MASALA ★**: the direct tax follow-on to B14 — now that the store stocks tobacco, a tobacco sale must carry the **compensation cess over and above the GST**, not GST alone. `GET /v1/finance/tax/cess` (API-09, `assessCompensationCess` in `packages/finance/src/cess.ts`) computes it for a demerit HSN: an **ad-valorem** part (bps of the taxable value) and/or a **specific** part (₹ per block of units, e.g. per 1000 cigarette sticks, charged **pro-rata** on quantity), summed to the paisa. A good with no cess spec is **not liable** — cess 0 — so every ordinary grocery line is untouched (the data gate). `checkDemeritHoldover` enforces the **GST-2.0 rule**: a cess good keeps **28% GST + cess** and must NOT also land on the **40% demerit** slab (that would tax it twice — refused). Rates are DATA, never hard-coded; back-office finance gate `finance.period.read`, the offline till keeps its own copy (hard rule #1). **10 unit + the cess cases in the tax integration suite**. Full gate green (typecheck, lint, secret-scan, traceability-integrity, one-surface, full suite). This completes the tobacco tax picture opened by B14. OWNER CHECK before first live tobacco billing: confirm the cess rates loaded for your cigarette/pan-masala HSNs match the current CBIC notification (they are configuration, not code). Prior: B14 (#150), WRITE-PATH inc1-3 (A5 end-to-end), EINV inc1+inc2, CORE-05 complete.)
