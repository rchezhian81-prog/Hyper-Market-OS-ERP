...
Last updated: 12 August 2026 (**★ D-4 — EXPIRY MARKDOWN LADDER (HUMAN-APPROVED) ★** (M05·M10): the commercial finisher of the perishables story — B8 stops an expired item selling, D-3 stops the shop over-buying, D-1 forecasts demand, and D-4 clears what is already on the shelf and running out of time. `proposeMarkdown` (`packages/demand/src/markdown.ts`) uses the **two inputs the roadmap names**: **sell-through** decides WHETHER (only the surplus that will not clear before the use-by at the current rate needs a cut — a will-clear batch is left alone, no margin given away), **remaining shelf life** decides HOW DEEP (a **ladder** that deepens as the use-by nears: default 10% within a week, 25% within 3 days, 50% on the last day — the ladder is DATA, a config edit). **A person commits (hard rule #5):** every proposal is advisory-only, there is NO price-changing function (a test reads the exports to prove no `apply`/`commit` shortcut), and the change itself rides the price-change approval path. Runs on `POST /v1/pricing/markdown/propose`, gated `price.change.propose` (owner + store-manager, not the till), deriving `avgDailyDemand` from the store's own sales when not supplied. **10 new tests** (will-clear no-cut, surplus markdown, ladder rungs, too-early hold, supplied policy/demand overrides, derived-demand end to end, the no-commit invariant, RBAC). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). This completes the sales-history/demand vein (D-3 · D-1 · D-4 + the reorder loop). Deferred & named: D-1's promo/festival/weather/cold-start signals. Prior: D-1 forecast (#168), replenishment uses real demand (#167), sales-history read (#166), C4 (#165), D-3 (#164).)
