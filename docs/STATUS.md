...
done for Store-Core (R2); Stage 5 has built 26 tested foundation units (212 tests). **D3/D4/D5/D8 were answered on 2 Aug 2026** (see
...
- **Stage 5 (foundation) — BEGUN** (`packages/contracts/`, all with tests; full `pnpm
  ...
  - `pnpm check` green: typecheck + lint + secret-scan + **212 tests**. Value-object
  ...
  - **Foundation engines now cover the core invariants** (exact money/quantity, append-only
    ...
    rule, loss-prevention anomaly rules) plus compositions (line/bill pricing, the deterministic
    ...
    controlled reopen) — 26 tested units, 212 tests.
...
## In progress
- **Stage 5 foundation build** — 26 tested units done (`packages/` contracts, ledger,
  approvals, rbac, sync, numbering, calendar, pricing, promotions, tender, config, sale, tenant,
  receiving, adjustment, counts, returns, cash, till, day-close, loss-prevention; 212 tests,
