...
Last updated: 11 August 2026 (**★ CORE-05 inc10 — SUSPENDED (PARKED) BILLS, ON THE CLOUD — CORE-05 COMPLETE ★**: the final increment of the owner-chosen 'close the thin-services gap' workstream. Wires `packages/suspended-sales` (park/resume/abandon a bill) onto the live API as an EVENT-SOURCED service (M15-FR-01/M12-FR-02). `POST /v1/pos/suspended-bills/:id` (API-05, on `services/pos/src/suspended-bills.ts`) parks a basket as durable state (a lane restart cannot lose it, hard rule #1); `…/resume` recalls it as a CLAIM that succeeds ONCE and refuses every attempt afterwards naming who holds it — two lanes resuming one bill is a double charge both cashiers believe is correct; `…/abandon` keeps the record with who and why (hard rule #6 — repeated park-and-abandon is a loss-prevention pattern); `GET …/stale` is the manager's oldest-first parked-bill report at close. A cross-lane/cross-store recall is refused by default; a bill parked past the price window still resumes (refusing strands the customer) but the lane is TOLD to re-price. Event-sourced: each transition an append-only `SuspendedBillStateChanged` fact, latest-wins fold; the engine's synchronous offline store is hydrated from those facts per request. Gated new permissions `pos.suspend.write`/`.read`. `tests/integration/suspended-bills.test.ts` proves park→resume-once→double-resume-blocked, cross-lane refused, abandon-needs-reason + the stale report, empty-basket 422, malformed-400, RBAC-403. Full gate green (typecheck, lint, secret-scan, **4,902 tests / 359 files**). **Metric moved: test-only packages 6 → 5 of 77** (down from 35 at the audit). **★ CORE-05 IS COMPLETE: all TEN domain engines (compliance, quality, self-checkout, owner-control, reversal, notifications, ops, org, documents, suspended-sales) are now wired onto the live API. The remaining 5 test-only packages (types, utils, config, ui, api-client) are shared libs/frontend with no endpoint to serve — not wireable by design. ★** The 'thin services' gap the 7-Aug audit named is closed. Prior CORE-05: inc1–inc9 all merged (#135–#143). Next natural work needs an owner decision — the write-path increment or the ten pending business decisions (entity type, turnover band, tobacco/liquor, chart of accounts) — so this is a clean stopping point pending owner direction.)

**R2 SELF-CONTAINED SCOPE STATUS (for the owner — a decision point):** 16 ratified-R2 requirements are now built and merged as self-contained, cloud-side, fully-tested increments — A1/A2/A4/A6/A8/A9/A10/A11/A12/A27/A28 (invoice numbering, the GST tax-invoice suite, statutory retention, non-disableable audit) + B2/B3/B4/B6/B9/B24 (dual-MRP, unit price, pack declarations, verified-scale gate, allergen/veg mark, label heights). Each is a pure, tested engine or guardrail joined to a running route, no offline-lane risk, reusing existing permissions. **The clean self-contained vein is now essentially mined out.** The remaining ratified R2 items fall into three buckets the owner should choose between: (1) ALREADY SERVED by existing generic engines — B7/B10 licence/scale-cert expiry alerts → `packages/compliance/src/obligations.ts` (`expiryAlerts`), B1 above-MRP → `packages/price-list/src/price-change.ts` + `packages/catalogue/src/snapshot-builder.ts`, B25/D-7 price integrity → `packages/self-checkout/src/price-integrity.ts`, A29 append-only → `tests/guardrails/ledger-append-only.test.ts` (these need at most a thin cross-reference, not a fresh build); (2) NEED A WRITE PATH or stored data first — A5 GSTR-1 Table-12 (aggregates invoice lines that are not yet on the ledger), and any requirement whose enforcement lives at the offline till (B5 weighed-price billing, B8 expired-batch block) which we have deliberately not touched (hard rule #1); (3) NEED AN OWNER DECISION — A20/A21 turnover-triggered e-invoicing, B14/B19 tobacco/plastic gates, and the ten owner decisions (entity type, chart of accounts, product lines, payroll). **Recommended next move:** pause net-new R2 feature work and get the owner's call — take on a write-path increment (e.g. persist invoice lines so A5 Table-12 and price-integrity reconciliation can run on real data), or hold R2 for the pre-deployment owner-decision session. No thin/forced increments will be manufactured in the meantime. Context unchanged: CORE-04 pure-read seam complete; work advances the owner-ratified R2 scope (#106).)
