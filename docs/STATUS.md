...
Last updated: 11 August 2026 (**★ BATCH-ON-SALE inc3b DECISION RATIFIED — ADR-0006 ★**: the owner ratified (11 Aug) *where* a sale gets its batch (M10-FR-03). **OD-BATCH-01: head office assigns the batch, NOT the till** — the offline sale path and checkout software are untouched (hard rule #1); the batch is attribution metadata added ALONGSIDE the banked sale, never an edit (hard rule #2). **OD-BATCH-02: a "good-enough" best-estimate is accepted**, not forensic exactness (a single promptly-syncing store). FINDING captured in the ADR: the cloud's inventory ledger carries `batchId` + receipt date but **NOT batch expiry**, so head-office attribution is **FIFO-by-receipt-date** as the proxy for FEFO (they coincide when oldest-expiry stock is received first — the normal case), **labelled `fifo_receipt_estimate`** so an estimate is never mistaken for a till-captured batch. The reject/defer alternatives (till-side offline; carrying expiry onto cloud events for true FEFO) are recorded as the upgrade path. `docs/adr/ADR-0006-batch-on-sale-attribution.md`. NEXT (inc3b implementation): at bank time, attribute the FIFO best-estimate batch to a batch-tracked sale line that arrived with none (reusing `assignBatchesFefo` fed cloud on-hand batches), as an append-only derived record folded into the lot-trace outbound alongside the captured batches. Prior: batch-on-sale inc3a (#156, trace pulls live sales), inc2 (#155), inc1 (#154), B11 (#153).)
