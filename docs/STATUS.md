...
Last updated: 12 August 2026 (**★ A5 FROM-SALES — OPTION A: THE HSN TABLE NOW AUTO-SOURCES FROM THE CATALOGUE ★** (M23·M12·M03): the owner-chosen follow-on to GSTR-1-from-sales. Last increment folded the return from real till sales but needed the filer to supply the product→{HSN, rate} table by hand; this makes that table **DEFAULT from the published catalogue** so the return builds with nothing to key. The snapshot builder now carries each product's HSN/tax-class code (`CatalogueProduct.hsnCode`, from `MasterProduct.taxClassId`) alongside its rate — an additive, backward-compatible field — and `gstReturnsAdapter.productTaxTable` reads it from the latest pack; a caller-supplied `products[]` still **overrides** per product (an exception the filer is handling), reported back as `taxTableSize`/`overrideCount`. A tax class that is not a real HSN (an internal alias) safely surfaces the product as `unmapped` rather than filing a wrong HSN. Runs on the same `POST /v1/finance/gstr1/from-sales/table-12` (products now optional). **3 net-new tests** (snapshot carries hsnCode; catalogue-default with no products[]; supplied override wins) on top of the prior 11. Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite — 5147 passed). **Remaining named boundary (still an open owner decision):** the return uses the current/supplied rate for the whole period, so a product whose GST rate changed **mid-period** is where the stored-document path (frozen per supply) stays authoritative; the alternative (freeze the HSN/tax onto the sale at cloud intake) is unbuilt by choice. Prior: A5 from-sales (#176), D-2 order-proposal (#175), C3 retention-clock (#174), C1 consent-notice (#173), C2 breach-notification (#172).)
