...
Last updated: 12 August 2026 (**★ THE REORDER TOOL NOW USES REAL DEMAND — THE SALES-HISTORY LOOP IS CLOSED ★** (M09): `POST /v1/replenishment/propose` (`services/inventory/src/replenishment.ts`) now **derives** a missing `avgDailyDemand` per item from the store's own banked sales over a trailing window (default 28 days, `?demandWindowDays=`), via the sales-history read merged just before. So REAL demand now drives **both** the reorder point (`safety + ceil(demand × lead)`) **and** the D-3 shelf-life cap (`demand × remaining-shelf-life`) — the buyer no longer types a demand figure in. Fully backward-compatible: a **supplied** `avgDailyDemand` still wins (a pure what-if is unchanged), and with no sales banked nothing changes. The response states the demand window it used (P-08). **4 new integration tests** (derived demand drives reorder point + cap; supplied-wins; custom window + bad-window 400; no-sales-unchanged) atop the 24 existing replenishment tests, all green. Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). NEXT on this vein: D-1 (forecast) and D-4 (markdown ladder), which build on the same sales-history read. Prior: sales-history demand read (#166), C4 children's-data guard (#165), D-3 shelf-life ordering (#164), B8 auto-capture (#163).)
