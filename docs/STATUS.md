...
Last updated: 10 August 2026 (**★ CORE-04 PURE-READ SEAM COMPLETE (5 increments) → FIRST R2 INCREMENT: GST PULLED OUT OF A TAX-INCLUSIVE MRP (v2.1 A9 + A8) ★**: CORE-04 wired five tested-but-unrun engines onto the cloud as pure reads — M17-FR-04 household pooling/double-spend (#114), M21 returns register (#115), M22-FR-04 `reconcileAr` (#116), M17-FR-03 `reconcileLiability`+`flagVelocity` (#117), M23-FR-02 `reconcileNotes` (#118). **Two definitive investigations then confirmed the pure-read seam is EXHAUSTED** (every remaining tested engine needs a new write path, a new event, product-master persistence, external ingestion, edge cost data, or one of the ten held owner decisions — the one data-viable leftover, `reconcile`, already runs inside the shipped settlement review). Rather than force a hollow sixth pure-read, work now advances the **owner-ratified R2 scope** (roadmap v2.1 addendum, #106) — genuine approved work, holding the gated items for pre-deployment. First R2 increment (**A9 + A8**): `extractInclusiveGst` (`packages/finance/src/inclusive-tax.ts`) pulls GST BACK OUT of a tax-inclusive MRP — `taxable = MRP × 10000/(10000+rateBps)` half-up, `tax = the REMAINDER` — so taxable + tax reconcile to the MRP **to the paisa by construction** (A9), never charging above the MRP; place of supply decides the split (A8): intra-State → CGST + SGST at half the rate each (halves split from the WHOLE tax, odd paisa deterministically on SGST), inter-State → a single IGST. It runs on `GET /v1/finance/tax/from-mrp` — a stateless back-office calculator so the same extraction is computed ONCE authoritatively (gated `finance.period.read`: owner/manager/accountant; the offline till keeps its own copy per hard rule #1, and does NOT call the cloud for a sale). Not left an orphan engine — a running route executes it, integration-tested. `tests/unit/finance-inclusive-tax.test.ts` proves the paisa-exact reconciliation across every slab × a wide amount sweep + the CGST/SGST/IGST split + refusals; `tests/integration/tax-from-mrp.test.ts` proves the route + the RBAC gate (cashier 403). Full gate green (typecheck, lint, secret-scan, **4,760 tests / 332 files**). No new dependency, no new permission. Held for pre-deployment as before: finance period-close second source, chart of accounts, real provider credentials, the ten owner decisions.)
