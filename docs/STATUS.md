...
Last updated: 10 August 2026 (**★ CORE-04 inc3 — THE B2B AR SUB-LEDGER↔GL RECONCILIATION IS NOW READABLE ON THE CLOUD (M22-FR-04) ★**: the third CORE-04 increment, same method as the last two — a tested `packages/b2b` engine (`reconcileAr`) that ran nowhere on the cloud, now wired onto API-09 as a pure read. One NEW route in `services/finance/src/b2b-collections.ts`: `GET /v1/b2b/collections/:cust/reconciliation?asOf=…` folds the collections ageing total and the finance AR balance from the **SAME customer stream** (the sub-ledger's `B2BInvoiceRecorded`/`B2BPaymentAllocated` and the credit surface's `B2BReceivableMovement`) and reports the difference **EXACTLY and with its sign** — the customer is being asked for money the books do not record, or shown a smaller debt than they owe and will pay the smaller one. The WORST case — the sub-ledger empty while finance records a debt — is SURFACED (a −difference), never 404'd; it 404s only when NEITHER ledger knows the customer. Decisively, the data was ALREADY on the cloud ledger (both sides fold the wired M22-FR-01 + M22-FR-04 events) — no new events, no write-path change, no edge data, no owner decision; the collections adapter gains only an `outstandingMinor` projection copied from the credit adapter. **RBAC is least-privilege (P-04):** the ageing read is a manager's (`b2b.account.read`), but reconciling the sub-ledger to the GL is a finance-controllership act, so the reconciliation is gated ONE RUNG UP on `b2b.receivable.record` (owner + accountant, NOT the store_manager) — the same split as the settlement day-review. **Correcting the record:** earlier snapshots grouped `reconcileAr` with the external-auth customer portal as 'needs external B2B-customer auth' — that was inaccurate; `reconcileAr` is an INTERNAL reconciliation with no session/scope dependency, and it needed none. `tests/integration/b2b-reconcile-ar.test.ts` proves it over the real pipeline + real per-tenant RBAC (agree; a signed drift; the empty-sub-ledger-but-finance-owes case surfaced not 404'd; the one-rung-up RBAC gate; 404 only when neither ledger knows the customer; per-tenant isolation) AND end to end against real PostgreSQL (a `describe.skipIf(!DATABASE_URL)` e2e in the Stage-gate CI job). M22 stays PARTIALLY WIRED — the only remaining gap is the customer-facing collections PORTAL (`scopeToCustomer`), which genuinely does need external B2B-customer auth. Full gate green (typecheck, lint, secret-scan, **4,734 tests / 328 files**, plus the real-PG e2e). Held for pre-deployment as before: finance period-close second source, chart of accounts, real provider credentials, the ten owner decisions.)
