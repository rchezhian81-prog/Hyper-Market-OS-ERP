...
Last updated: 10 August 2026 (**★ R2 v2.1 B6 — VERIFIED-SCALE GATE, ON THE CLOUD ★**: the fourteenth ratified-R2 increment, opening the weights-&-measures area. A weighing instrument used for trade must carry a CURRENT Legal Metrology verification stamp; selling by weight on a scale whose stamp has lapsed is an offence and every weighed price it produces is legally unsound. `weighingScaleVerification` reports whether verification is current as of a date and BLOCKS trading once it has lapsed — a DIFFERENT clock from the asset's AMC/warranty maintenance (`packages/facilities/src/assets.ts`): a perfectly serviced scale with an expired stamp is still illegal to weigh on. It warns inside a notice window so re-verification can be booked before it stops the counter, and the due date itself is the last valid trading day. The scale-register fields B7 names (approval no., capacity, last stamping, re-verification due) travel on the `WeighingScale` record; B7's alerting is already served by the generic `compliance/obligations expiryAlerts`. Runs on `GET /v1/facilities/scale-verification?reverificationDueOn=&asOf=&noticeDays=` (a new stateless facilities-metrology route), gated `facilities.asset.read` (owner/store-manager). `tests/unit/facilities-weighing-verification.test.ts` proves current/warn/expired + the due-date boundary; `tests/integration/facilities-scale-verification.test.ts` proves the route + RBAC (a cashier, lacking `facilities.asset.read`, is refused 403). Full gate green (typecheck, lint, secret-scan, **4,852 tests / 346 files**). No new dependency, no new permission. R2 progress: A1/A2/A4/A6/A8/A9/A10/A11/A12 (invoice+tax) + B2/B3/B4/B9/B24 (product/labelling) + B6 (weights & measures) = 14 ratified requirements built. Note: several remaining B/A items are already served by existing generic engines (B7/B10 → `compliance/obligations`; B1 → `price-change`/`snapshot-builder`; B25 → `self-checkout/price-integrity`; A29 → the `ledger-append-only` guardrail) or need a write path / owner decision (A5 GSTR-1 Table-12; the ten owner decisions). Context unchanged: CORE-04 pure-read seam complete and exhausted; work advances the owner-ratified R2 scope (#106), holding the gated items for pre-deployment.)
