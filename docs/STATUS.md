...
Last updated: 12 August 2026 (**★ GUARDRAIL — THE TILL SURFACES EVERY CRITICAL BREACH ★**: three sale-intake findings built across this session put a customer at risk or the shop in front of a regulator — a **recalled product sold** (M10-FR-04), a **charge above MRP** (B1), and an **expired batch sold** (B8) — and each must be banked (money's in the drawer, hard rule #1) yet surfaced **critical** and ranked **ahead of** any money variance. Three separate unit tests could each be weakened one at a time without the *contract* visibly eroding, so `tests/guardrails/the-till-surfaces-every-critical-breach.test.ts` locks it as ONE invariant: all three raise a critical finding, each ranks first over a tender mismatch, a **tripwire** proves a clean sale raises none (not passing vacuously), and a source read catches a silent `'critical'`→lower downgrade. Same family as the `the-pricing-screen-cannot-break-the-law` and `audit-trail-cannot-be-disabled` guardrails. Test-only, zero production change. Full gate green (typecheck, lint, secret-scan, full suite). NOTE: the self-contained R2 code vein is now genuinely exhausted — the remaining work needs an owner decision (e-invoicing GSP account; first GST filing; gold/OTC product lines) or is a deliberate offline-money-path change (B8-at-the-lane, B5). Prior: B8 cloud backstop (#160), M16 (#159), B1 (#158), batch-on-sale COMPLETE.)
