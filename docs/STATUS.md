...
Last updated: 10 August 2026 (**★ R2 v2.1 B2 — DUAL-MRP GUARD, ON THE CLOUD ★**: the thirteenth ratified-R2 increment. A pre-packed commodity carries ONE printed MRP; a pack showing two different MRPs is an offence and leaves the till unable to say which price is the legal ceiling. MRP is historised (effective-dated), so a price CHANGE over time is legitimate — what is not is two DIFFERENT MRPs both taking effect on the SAME date, where `mrpOn` (which tie-breaks on `effectiveFrom`) would pick arbitrarily. `checkDualMrp` REJECTS that second MRP at master-data commit, before a pack can carry a contradictory price; a later-dated change or an exact-duplicate re-entry is allowed, and a clash names the conflicting date + amounts. Runs on `GET /v1/catalogue/dual-mrp-check?effectiveFrom=&valueMinor=&existing=DATE~MINOR,…` (a new master-data guards route in the catalogue service, kept distinct from the labelling validators), gated `catalogue.pack.read`. `tests/unit/product-dual-mrp.test.ts` proves the same-date-clash rejection and the allowed cases (later date, empty history, exact duplicate); `tests/integration/catalogue-unit-price.test.ts` proves the route + RBAC. Full gate green (typecheck, lint, secret-scan, **4,846 tests / 344 files**). No new dependency, no new permission. R2 progress: A1/A2/A4/A6/A8/A9/A10/A11/A12 (invoice+tax) + B2/B3/B4/B9/B24 (product/labelling) = 13 ratified requirements built. Context unchanged: CORE-04 pure-read seam complete and exhausted; A5 (GSTR-1 Table-12) blocked pending invoice-line data on the ledger. Work advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
