...
Last updated: 10 August 2026 (**★ R2 v2.1 A1 — MANDATORY RULE 46 TAX-INVOICE FIELDS, ON THE CLOUD ★**: the seventh ratified-R2 increment (after A9/A8 #119, A10 #120, A11 #121, A12 #122, A6 #123, A4 #124) and the capstone of the tax-invoice work. `checkTaxInvoiceFields` validates an assembled tax invoice for every CGST **Rule 46** mandatory field and NAMES each one absent or malformed — the 'Tax Invoice' heading, a valid 15-char GSTIN, a ≤16-char consecutive invoice number + date, the HSN, the taxable value, the rate, the place of supply, and the CGST+SGST / IGST split MATCHING the place of supply (an intra-State invoice showing IGST, or an inter-State one showing CGST/SGST, is wrong, not merely incomplete). A silently-incomplete invoice is legally invalid — no input credit for the buyer, a penalty for the shop — so it is refused with its reasons rather than let out. Runs on `GET /v1/finance/tax/invoice-check` (the same back-office finance calculator, gated `finance.period.read`). `tests/unit/finance-invoice-fields.test.ts` proves a complete invoice passes and EACH dropped/malformed field fails (named), plus the split/place-of-supply mismatch; `tests/integration/tax-from-mrp.test.ts` proves the route + the RBAC gate. Full gate green (typecheck, lint, secret-scan, **4,802 tests / 337 files**). No new dependency, no new permission. **The R2 tax-invoice core is now a complete suite on one back-office calculator: A9/A8 MRP extraction+split, A10 rounding, A11 discount eligibility, A12 promo supplies, A6 time-of-supply rate, A4 HSN digits, A1 Rule 46 field validation.** Context unchanged: CORE-04 pure-read seam complete (5 increments) and exhausted; work advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
