...
Last updated: 11 August 2026 (**★ BATCH-ON-SALE COMPLETE — inc3b: HEAD-OFFICE BEST-ESTIMATE ATTRIBUTION ★** (M10-FR-03 / ADR-0006): the final piece. When a batch-tracked sale reaches head office with **no captured batch**, the cloud attributes the most-likely batch — `attributeSalesFifo` (`packages/fefo/src/attribute-sales.ts`) replays a product's receipts + its batch-tracked sales in date order, drawing **FIFO by receipt date** (the ADR-0006 proxy for FEFO, since the cloud ledger has no batch expiry): a captured-batch sale consumes its OWN batch (so it doesn't distort the estimate), an un-captured one draws FIFO and is attributed those batches, and units no recorded batch can cover are reported (`unattributedQty`) not invented (P-08). `lotTraceAdapter.soldOfBatch` computes this **at read time** (nothing written back onto the sale — hard rule #2) from the InventoryMoved receipts + the catalogue `batchTracked` flag + the banked sales, returning estimates labelled **`source: 'fifo_receipt_estimate'`** next to the captured lines (`source: 'captured'`); `/sold` now reports `capturedCount` vs `estimatedCount` so the two are never confused. **7 engine unit + 8 lot-trace integration tests.** Full gate green (typecheck, lint, secret-scan, traceability-integrity, services-run-on-their-engine, no-test-doubles, pos-offline, one-surface, full suite). **Batch-on-sale is now COMPLETE end-to-end** (inc1 capture-contract + flag, inc2 FEFO engine, inc3a real-sales fold, ADR-0006 decision, inc3b head-office estimate) — a till-captured batch is traced exactly, an un-captured one gets a labelled estimate, and the offline sale path was never touched. Prior: ADR-0006 (#157), inc3a (#156), inc2 (#155), inc1 (#154).)
