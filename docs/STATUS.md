...
Last updated: 11 August 2026 (**★ WRITE-PATH increment — GST-RETURNS OUTWARD-SUPPLY STORE + GSTR-1 TABLE 12 (A5) ★**: the owner chose the write-path increment. Built the first genuine WRITE path for fiscal reporting: `POST /v1/finance/outward-supplies/:documentId` (API-09, `services/finance/src/gst-returns.ts` + `gstReturnsAdapter`, event-sourced) PERSISTS a sale/invoice's tax-relevant lines — HSN, taxable value, the CGST/SGST/IGST split — as append-only outward-supply facts, and `GET /v1/finance/gstr1/table-12?from=&to=` folds them into GSTR-1 Table 12 (HSN-wise summary, B2B/B2C split) over the period. Discipline: a **free-text or malformed HSN is REJECTED before storage** ('closed master, no free text' — `validateOutwardLine` reuses the A4 digit rule and throws on non-numeric); a B2B document needs a valid recipient GSTIN; an inconsistent tax split (both CGST/SGST and IGST, or CGST≠SGST) is refused; the record is idempotent per document (a correction is a new document/credit note, never an overwrite, hard rule #2); and the tax facts are captured AS AT the supply so a later HSN change never rewrites a filed month. New engine `packages/finance/src/gstr1.ts` (`validateOutwardLine` + `gstr1Table12`), gated new permissions `finance.gstr.generate`/`.read`. **3 unit (`tests/unit/gstr1.test.ts`) + 4 integration (`tests/integration/gst-returns.test.ts`)** assertions. Full gate green (typecheck, lint, secret-scan, **4,930 tests / 364 files**). This is the read/report side of A5; the remaining GSTR-1 work (B2B invoice-level tables, JSON export to the portal, and feeding sale commits' lines into outward-supplies automatically) can be follow-ups. Prior: EINV inc1+inc2 (#145/#146), CORE-05 complete, owner decisions ratified.)
