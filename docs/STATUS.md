...
Last updated: 10 August 2026 (**★ R2 v2.1 A4 — HSN DIGIT COUNT BY TURNOVER, ON THE CLOUD ★**: the sixth ratified-R2 increment (after A9/A8 #119, A10 #120, A11 #121, A12 #122, A6 #123). A tax invoice must state the HSN to the number of digits the shop's SIZE requires: **6 digits above ₹5 crore aggregate turnover, 4 at or below** (`requiredHsnDigits`; the ₹5-crore threshold is pinned in paisa by test — 5,000,000,000 — so a mis-grouped literal cannot slip through, which one nearly did). `validateHsnForTurnover` then FAILS a code with fewer digits than required (a too-short HSN is a non-compliant invoice; a more specific 8-digit code satisfies a 6-digit rule) and refuses a malformed code rather than treat it as compliant. Runs on `GET /v1/finance/tax/hsn-digits?annualTurnoverMinor=&hsn=` (the same back-office finance calculator, gated `finance.period.read`). `tests/unit/finance-hsn.test.ts` proves the ₹5-crore boundary (4 at/below, 6 just above) + the too-short-fails rule; `tests/integration/tax-from-mrp.test.ts` proves the route + the RBAC gate. Full gate green (typecheck, lint, secret-scan, **4,797 tests / 336 files**). No new dependency, no new permission. The R2 fiscal core so far: A9/A8 MRP extraction+split, A10 rounding, A11 discount eligibility, A12 promo supplies, A6 time-of-supply rate, A4 HSN digits. Context unchanged: CORE-04 pure-read seam complete (5 increments) and exhausted; work advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
