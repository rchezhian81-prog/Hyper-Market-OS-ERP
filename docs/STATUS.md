...
Last updated: 11 August 2026 (**★ CORE-05 inc6 — NOTIFICATION SEND GUARD, ON THE CLOUD ★**: continuing the owner-chosen 'close the thin-services gap' workstream. Wires `packages/notifications` canNotify — a complete, tested consent-safe send gate nothing on the cloud fed — onto the live API (M31-FR-03). Before any notification goes out (a WhatsApp offer, an SMS receipt, a push), `POST /v1/notifications/can-send` (API-06, on `services/customer/src/notification-guard.ts`) runs `canNotify` — every gate must pass IN ORDER: the template is APPROVED (§28), the contact is not on the do-not-contact SUPPRESSION list (absolute), CONSENT for this purpose+channel is granted/not-withdrawn/within the frequency cap (reusing the M16 customer-consent engine), and the messaging BUDGET is not exceeded. A breach BLOCKS the send — never warned-and-sent — and the decision names the FIRST failing gate. Stateless: the caller supplies the contact's consent state and this send's facts; this is the ruling, not a store of contacts or sends. Gated new permission `notification.send.check`. `tests/integration/notification-guard.test.ts` proves the allow path and each block (unapproved_template, suppressed, no_consent, over_budget) blocking on the first failing gate in order, plus malformed-400 and RBAC-403. Full gate green (typecheck, lint, secret-scan, **4,888 tests / 355 files**). **Metric moved: test-only packages 10 → 9 of 77** (down from 35 at the audit). Prior CORE-05: inc1 `compliance` (#135), inc2 `quality` (#136), inc3 `self-checkout`/B25 (#137), inc4 `owner-control` (#138), inc5 `reversal` refund-exceptions (#139) — all merged. Remaining unused engines: notifications-queue (retry/dead-letter M31-FR-04), suspended-sales, ops, org, documents.)
