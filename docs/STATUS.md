...
Last updated: 11 August 2026 (**★ CORE-05 inc3 — PRICE-INTEGRITY AUDIT, ON THE CLOUD ★**: continuing the owner-chosen 'close the thin-services gap' workstream. Wires `packages/self-checkout` price-integrity — a complete, tested cross-surface reconciliation engine nothing on the cloud fed — onto the live API, delivering ratified R2 **B25** (shelf = scanned = billed; a mismatch surfaces as an exception, never a silent overcharge). `POST /v1/pricing/integrity/audit` (API-02, on `services/pricing/src/price-integrity.ts`) runs `auditPriceIntegrity`: **the till is the reference** (it is what the customer is charged), and the consequences are NOT symmetric — a shelf/ESL/app showing LESS than the till is an `overcharge_risk` ranked FIRST whatever its value (Legal Metrology: the displayed price is what the customer was offered — a ₹4 label error still draws a notice), a shelf showing MORE is `undercharge` (margin) ranked by value, a surface silent past its freshness window is `stale_surface`/`esl_unreachable` named by device+shelf (silence is never agreement), a required surface absent is `surface_missing`. Deliberately stateless — the caller holds the till prices and the ESL/shelf feeds, so this is the verdict not a new store (P-02). Gated new permission `price.integrity.audit` (owner/store-manager). `tests/integration/price-integrity.test.ts` proves the asymmetry (overcharge ranked first), stale-ESL, missing-surface, malformed-400 and RBAC-403. Full gate green (typecheck, lint, secret-scan, **4,879 tests / 352 files**). **Metric moved: test-only packages 13 → 12 of 77** (down from 35 at the audit). Prior CORE-05: inc1 `compliance` (M34-FR-03 + B7 + B10, PR #135 merged), inc2 `quality` cold-chain (M10-FR-02, PR #136 merged). Remaining unused engines: reversal, owner-control, suspended-sales, notifications, ops, org, documents. Each moves the runnable-shop % up.)
