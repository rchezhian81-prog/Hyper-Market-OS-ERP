...
Last updated: 10 August 2026 (**★ R2 v2.1 B4 — STATUTORY PACK DECLARATIONS GATE, ON THE CLOUD ★**: the eleventh ratified-R2 increment, closing the labelling group. Independently of any department's own attribute list, EVERY pre-packed commodity must carry a fixed set of Legal Metrology Rule 6 declarations, and four of them gate whether the pack may go on sale at all: `checkPackDeclarations` requires the **net quantity**, the **month/year of manufacture-pack** (YYYY-MM or -DD), a **consumer-care contact**, and the **country of origin** — a gap (or a blank/malformed date) makes the pack NOT SELLABLE and names the field, blocking activation rather than letting a non-compliant pack reach the shelf. This is a HARDER, always-on gate than the existing per-tenant completeness worklist (which measures how close a record is to finished, per the tenant's own category rules); the four are a legal floor under every pack. Runs on `GET /v1/catalogue/pack-declarations?netQuantity=&manufactureOrPackDate=&consumerCareContact=&countryOfOrigin=` (the labelling route in the catalogue service), gated `catalogue.pack.read`. `tests/unit/product-pack-declarations.test.ts` proves each missing/blank/malformed declaration blocks; `tests/integration/catalogue-unit-price.test.ts` proves the route + RBAC. Full gate green (typecheck, lint, secret-scan, **4,830 tests / 342 files**). No new dependency, no new permission. R2 progress: A1/A4/A6/A8/A9/A10/A11/A12 (tax-invoice suite) + B3/B24/B9/B4 (labelling) = 11 ratified requirements built. Context unchanged: CORE-04 pure-read seam complete (5 increments) and exhausted; work advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
