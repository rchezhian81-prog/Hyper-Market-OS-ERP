...
Last updated: 11 August 2026 (**★ CORE-05 inc8 — BRANCH OPEN/CLOSE LIFECYCLE, ON THE CLOUD ★**: continuing the owner-chosen 'close the thin-services gap' workstream. Wires `packages/org` branch-lifecycle — a complete, tested governance engine nothing on the cloud API could call — onto the live API (M01-FR-04). `POST /v1/platform/branches/transition/evaluate` (API-11, on `services/platform/src/branch-lifecycle.ts`) runs `evaluateTransition`: the decision to open / temporarily close / reopen / permanently close a branch is MEASURED not assumed — it reads what is actually true (stock held+valued, cash in tills/safe, open documents, items the edge never synced, unresolved exceptions) and returns EVERY blocker at once in plain English with the number. Every transition except reopening needs the owner's approval and the approver is NEVER the requester (§28); a permanent close over unsent sync items is REFUSED because it would destroy sales legitimately made (§31); a temporary close deliberately preserves state. Stateless and non-mutating — the caller supplies the request+state+readiness, this is the ruling and the caller applies it (auditable on its own). Gated new permission `branch.transition.evaluate`. `tests/integration/branch-lifecycle.test.ts` proves an allowed owner-approved open, a permanent close blocked on stock+unsent-sync+approval all at once, a self-approved closure blocked (§28), malformed-400 and RBAC-403. Full gate green (typecheck, lint, secret-scan, **4,895 tests / 357 files**). **Metric moved: test-only packages 8 → 7 of 77** (down from 35 at the audit). Prior CORE-05: inc1–inc7 all merged (#135–#141). Remaining unused engines: documents (M31/M36 template versioning), suspended-sales (M15 park/resume), notifications-queue (M31-FR-04 dead-letter), plus the ~4 shared libs/frontend (types/utils/config/ui/api-client) that are not service-wireable.)
