...
Last updated: 12 August 2026 (**★ D-3 — WE NEVER ORDER MORE PERISHABLE STOCK THAN CAN SELL BEFORE IT EXPIRES ★** (M09·M10): the ordering bookend to B8. Where B8 stops an already-expired item selling, D-3 stops the shop **buying** what it can't sell in time. `proposeReplenishment` (`packages/replenishment/src/replenishment.ts`) now takes an optional `remainingShelfLifeDays`; with the demand rate it caps the order-up-to at a **days-of-supply ceiling** (`avgDailyDemand × remainingShelfLifeDays`), so the order never lifts the holding above what can sell before the batch expires. A supplier pack / minimum-order that would breach the ceiling is fitted to **whole packs under** it — and if nothing compliant fits, NO order is placed and the item is surfaced as a visible `held_shelf_life` exception (a control, not a silent skip; P-03/P-08). It never guesses (demand unknown → no cap). Rides the existing advisory-only what-if `POST /v1/replenishment/propose` — no new route, no offline-till risk, no owner decision (a hard prudential bound). **8 new tests** (below-max cap, above-max no-op, over-order hold, expires-today hold, pack-fit-under-ceiling, MOQ-would-over-order hold, never-guess, end-to-end route). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). Prior: B8 auto-capture off a date-coded barcode (#163), B8 capture through the view (#162), B8 lane block (#161).)
