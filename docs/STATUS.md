...
Last updated: 10 August 2026 (**★ R2 v2.1 A27 — AUDIT TRAIL CANNOT BE DISABLED (GUARDRAIL), ON THE CLOUD ★**: the sixteenth ratified-R2 increment (a priority-10 item), and — see the scope note below — likely the last purely self-contained one. The FND-02 SHA-256 hash chain is only a control if it is ALWAYS ON; the kernel types `audit` as optional (so a unit test may omit it), so the ONLY thing between 'production audits everything' and 'silently audits nothing' is that `services/api/src/main.ts` wires a real `SqlAuditSink` unconditionally with no disable flag — a fact a refactor can erase in one line with the suite still green. `tests/guardrails/audit-trail-cannot-be-disabled.test.ts` now LOCKS it: the production composition must wire the sink, no `disableAudit`/`AUDIT_OFF`-style flag may exist, and the sink must stamp `recorded_at` on every row; a tripwire proves the guardrail bites when the wiring is dropped. No new production code — an existing invariant made machine-checked. Full gate green (typecheck, lint, secret-scan, **4,866 tests / 349 files**). No new dependency, no new permission. 
