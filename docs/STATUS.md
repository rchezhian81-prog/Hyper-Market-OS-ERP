...
Last updated: 12 August 2026 (**★ C4 — CHILDREN'S DATA IS PROTECTED IN LOYALTY & MARKETING (DPDP s.9) ★** (M16): a child is anyone under 18. `assessChildDataProcessing` (`packages/customer/src/child-data-guard.ts`) is a pure overlay on the consent rules that encodes both hard parts of DPDP Act 2023 s.9 — **(1)** a child's data may be enrolled/marketed/profiled **only with verifiable parental consent**, and **(2)** behavioural tracking and advertising **targeted at a child are prohibited outright**, which **parental consent cannot cure**. A transactional/service message is unrestricted; and it **never assumes an adult** — age unproven refuses a child-restricted activity. Runs on `POST /v1/customers/child-data/check` (API-06), gated `customer.consent.read` (the counter staff who enrol a customer hold it; the accountant does not). Commits nothing, stores no child's data. **17 new tests** (age-from-DOB boundary incl. impossible dates; adult permit, child-needs-consent, consent-cures-enrolment, s.9(3) tracking/targeted-ad prohibition even with consent, transactional pass-through, age-unverified refusal; end-to-end route + RBAC). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). This was the last self-contained R2 engine with **no persistence dependency and no owner decision**. Prior: D-3 shelf-life-constrained ordering (#164), B8 auto-capture (#163).)
