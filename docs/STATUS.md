...
Last updated: 10 August 2026 (**★ R2 v2.1 A12 — GST TREATMENT OF PROMOTIONAL SUPPLIES (BOGO / FREE SAMPLE / VOUCHER), ON THE CLOUD ★**: the fourth ratified-R2 increment (after A9/A8 #119, A10 #120, A11 #121), building the GST fiscal core. Encodes the CBIC clarifications, and the load-bearing point is the OPPOSITE input-tax-credit consequence of a BOGO vs a genuine free sample — the thing shops get wrong: **BOGO** (`bogoTreatment`) is a single supply for the paid consideration, the free units add NO extra tax and the ITC is NOT reversed (CBIC 92/2019); a **genuine free sample** (`freeSampleTreatment`) is not a supply (no output GST) but the ITC on it MUST be reversed (s.17(5)(h)); a **voucher's** time of supply (`voucherTimeOfSupply`) is the ISSUE date when the supply is identifiable at issue, else the REDEMPTION date (s.12(4)) — and it REFUSES to time an unidentifiable, not-yet-redeemed voucher rather than guess. Runs on `GET /v1/finance/tax/{bogo,free-sample,voucher-timing}` (the same back-office finance calculator, gated `finance.period.read`). `tests/unit/finance-promo-tax.test.ts` proves the CBIC truth tables including the BOGO≠free-sample ITC distinction; `tests/integration/tax-from-mrp.test.ts` proves the routes + the RBAC gate. Full gate green (typecheck, lint, secret-scan, **4,783 tests / 334 files**). No new dependency, no new permission. The R2 fiscal core so far: A9/A8 MRP extraction + split, A10 rounding, A11 discount eligibility, A12 promo supplies. Context unchanged: CORE-04 pure-read seam complete (5 increments) and exhausted; work advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
