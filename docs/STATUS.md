...
Last updated: 11 August 2026 (**★ BATCH-ON-SALE inc3a — THE RECALL TRACE NOW PULLS LIVE SALES ★** (M10-FR-03, cloud-side): the payoff of the batch-on-sale work. `bankSale` already stores every sale — with its batch-tagged lines (inc1) — as a `SaleCommitted` event, so `lotTraceAdapter.soldOfBatch` (`services/api/src/adapters.ts`) reads them back and returns every sale line that carried a batch as an OUTBOUND record. **`GET /v1/quality/lot-trace/:batchId/sold`** gives the "who bought this batch" list straight from live sales; **`POST /v1/quality/lot-trace/export`** now folds those real sales as its outbound by default (an explicit `outbound[]` still wins for a what-if), so **B11's recall trace pulls live data end-to-end** — bank a sale with a batch, ask for that batch, and the sale is there. A walk-in with no captured customer is kept and counted (customer names via loyalty/consent are a later M16 linkage). Gated `quality.lottrace.read` (owner / store-manager). **6 integration tests** (fold by batch, feed into the reconciled export, empty batch, RBAC). Full gate green (typecheck, lint, secret-scan, traceability-integrity, services-run-on-their-engine, no-test-doubles, one-surface, full suite). **The offline commit path is STILL untouched** (`pos-offline` guardrail green). REMAINING — inc3b (the one offline-path change): the POS shell runs `assignBatchesFefo` at commit and stamps the batch onto each line, so a real batch rides the sale (the field already syncs verbatim). Prior: batch-on-sale inc2 (#155), inc1 (#154), B11 (#153), B19 (#152), A7 (#151), B14 (#150).)
