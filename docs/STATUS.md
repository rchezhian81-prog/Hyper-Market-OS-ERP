...
Last updated: 11 August 2026 (**★ CORE-05 inc2 — COLD-CHAIN ASSESSMENT, ON THE CLOUD ★**: continuing the owner-chosen 'close the thin-services gap' workstream. Wires `packages/quality` — a complete, tested cold-chain engine nothing on the cloud fed — onto the live API. `POST /v1/quality/cold-chain/assess` (API-04, on `services/inventory/src/cold-chain.ts`) runs `assessColdChain` and returns the food-safety verdict: duration counts as much as peak (a freezer door open 90s is not a breach, four hours is), a cold-chain batch with NO reading at all fails rather than passes (an unmeasured cold chain is an unproven one), and a breach quarantines **automatically** — not a warning someone weighs while unloading. Deliberately STATELESS and deliberately NOT a second store of temperatures: facilities-monitoring already owns equipment-reading truth, and one temperature truth beats two (P-02). Gated new permission `quality.coldchain.assess` (owner/store-manager). `tests/integration/quality-cold-chain.test.ts` proves within-range / brief-excursion-within-grace / breach+quarantine / no-readings=breach / inverted-rule-400 / RBAC-403. This lands M10-FR-02 on the running surface. Full gate green (typecheck, lint, secret-scan, **4,875 tests / 351 files**). **Metric moved: test-only packages 14 → 13 of 77** (down from 35 at the audit). Prior: inc1 wired `compliance` (M34-FR-03 + B7 + B10), merged as PR #135. CORE-05 continues — remaining unused engines: suspended-sales, reversal, owner-control, self-checkout (B25), notifications, ops, org, documents. Each moves the runnable-shop % up.)
