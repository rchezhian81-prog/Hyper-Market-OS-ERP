...
Last updated: 12 August 2026 (**★ B8 — THE LANE'S EXPIRED-BATCH BLOCK IS NOW WIRED END-TO-END ★** (M10·M12): the capture side of B8, finishing the offline lane defence. `PosView.scanBarcode(code, batch?)` (`apps/pos/src/view-adapter.ts`) now threads the batch context (today + the scanned batch's use-by) into `CatalogueCache.scan`, so at the lane an expired batch is **refused and never reaches the bill**, while an in-date one scans normally — proven end-to-end in `pos-barcode-scan.test.ts`. Additive & backward-compatible (a plain `scanBarcode(code)` is unchanged); it never guesses (no date → no block, hard rule #1) and adds no I/O (`pos-offline` guardrail green). **2 lines of defence now live:** the lane hard-block (offline) + the head-office critical backstop. The one remaining piece is purely **device/UI**: how the cashier's screen OBTAINS the use-by date at runtime (a keyed field, or a GS1-128/label decode) — a deployment/UI integration on top of a live, tested block, not core logic. **1 new end-to-end lane test** (expired refused + nothing on the bill + in-date scans). Full gate green (typecheck, lint, secret-scan, recall + pos-offline + till-screen guardrails, traceability-integrity, full suite). B8 is now effectively COMPLETE bar the device date-source. Prior: B8 lane block (#161), critical-breach guardrail + cloud backstop (#160), M16 (#159).)
