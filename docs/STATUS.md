...
done for Store-Core (R2); Stage 5 has built 16 tested foundation units (133 tests). **D3/D4/D5/D8 were answered on 2 Aug 2026** (see
...
- **Stage 5 (foundation) — BEGUN** (`packages/contracts/`, all with tests; full `pnpm
  ...
  - `pnpm check` green: typecheck + lint + secret-scan + **133 tests**. Value-object
  ...
  - **Foundation engines now cover the core invariants** (exact money/quantity, append-only
    ...
    rule) plus compositions (line/bill pricing, tender settlement, and the end-to-end offline sale commit) — 16 tested units, 133 tests.
...
## In progress
- **Stage 5 foundation build** — 16 tested units done (`packages/` contracts, ledger,
  approvals, rbac, sync, numbering, calendar, pricing, tender, config, sale, tenant; 133 tests, `pnpm check` green). The
