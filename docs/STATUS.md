...
Last updated: 12 August 2026 (**★ D-1 — DEMAND FORECAST (BASELINE × DAY-OF-WEEK, JUDGED BY A BACK-TEST) ★** (M09): a forward-looking, per-SKU forecast that decomposes a product's own sales into a **baseline level × a learned day-of-week factor** and projects it ahead — the week is a hypermarket's steadiest signal, so `forecastDemand` (`packages/demand/src/forecast.ts`) gives a buyer a number they can read, not a black box (a day with no history is zero demand; an empty history forecasts zero, never a guess). **Judged, not asserted:** `backtestForecast` fits on the earlier history, forecasts the held-out tail, and scores it with **WAPE** — so the D-1 acceptance ("back-test error bounded on a fixture") is a real number in test (a clean weekly fixture < 0.05). Runs on `GET /v1/inventory/demand-forecast?productId=&from=&to=&horizonDays=` (per-SKU, `inventory.availability.read`, default trailing 8 weeks + a 7-day back-test), folding the sales-history read. **DEFERRED, NAMED (not dropped):** the roadmap's promo / festival / weather / cold-start signals layer ON this baseline and are follow-on. **9 new tests** (pattern learning, horizon projection, empty→zero, bad-input reject, the fixture back-test bound; end-to-end route + RBAC). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). NEXT on this vein: D-4 (expiry markdown ladder). Prior: replenishment uses real demand (#167), sales-history read (#166), C4 (#165), D-3 (#164).)
