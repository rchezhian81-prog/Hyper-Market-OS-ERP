...
Last updated: 11 August 2026 (**★ CORE-05 inc9 — VERSIONED DOCUMENT TEMPLATES, ON THE CLOUD ★**: continuing the owner-chosen 'close the thin-services gap' workstream. Wires `packages/documents` — a complete, tested template-versioning engine nothing on the cloud API could call — onto the live API as an EVENT-SOURCED service (M31-FR-01/M36-FR-02). The rule the whole package exists for: a template change is a NEW VERSION, never an overwrite, so July's tax invoice keeps July's address+branding after August's change. `POST /v1/documents/templates/:id/publish` (API-11, on `services/platform/src/documents.ts`) runs `publishTemplateVersion` — append-only (there is NO edit path), needs a change note (an invoice layout carries the shop's legal identity), needs a SEPARATE approver (§28: author ≠ approver), branding frozen into the version (M36-FR-02), one tenant's rebrand can never reach another's paperwork — a governance refusal is 422. `GET /v1/documents/templates/:id/current?at=` runs `currentVersion` — the version in force at a moment (v1 in July after v2 publishes in August). Event-sourced: versions folded from an append-only `TemplateVersionPublished` stream, keyed templateId+version. Gated new permissions `document.template.manage`/`.read`. `tests/integration/documents-templates.test.ts` proves append-only v1→v2, time-aware current (July=v1 w/ frozen branding), self-approved and no-change-note 422s, 404 unknown, malformed-400, RBAC-403. Issue/reproduce (needs a concrete renderer) is the deliberate follow-up. Full gate green (typecheck, lint, secret-scan, **4,898 tests / 358 files**). **Metric moved: test-only packages 7 → 6 of 77** (down from 35). Prior CORE-05: inc1–inc8 all merged (#135–#142). REMAINING: `suspended-sales` is the LAST genuine domain engine to wire (inc10); the other 5 test-only packages are shared libs/frontend (types/utils/config/ui/api-client) NOT service-wireable — after suspended-sales, CORE-05 is DONE.)
