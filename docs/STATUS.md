...
Last updated: 11 August 2026 (**★ CORE-05 inc7 — BACKUP VERIFICATION & RESTORE RECONCILIATION, ON THE CLOUD ★**: continuing the owner-chosen 'close the thin-services gap' workstream. Wires `packages/ops` backup verifier — a complete, tested disaster-recovery engine nothing on the cloud API could call — onto the live API (M35-FR-01/02, P-04 tested recovery). A backup nobody verified is a hope. `POST /v1/platform/backups/verify` (API-11, on `services/platform/src/backup-verification.ts`) runs `verifyBackup` — an EMPTY artefact (job reported success, produced nothing), a CHECKSUM that no longer matches (truncated/corrupted/altered), NO control totals (a restore could never be proven), NOT encrypted at rest, or NO off-site copy (a single-device backup is not a backup, §33) each make it not usable. And `POST /v1/platform/backups/reconcile-restore` runs `reconcileRestore` — proving a RESTORE correct: every row count AND every money total must match EXACTLY (there is no 'close enough'), a single difference failing it and naming the table and what it is worth. Stateless — the backup/restore job supplies the manifest + the file/DB facts; this is the ruling on whether recovery can be trusted. Gated new permission `backup.verify.read`. `tests/integration/backup-verification.test.ts` proves verified/empty/checksum-mismatch/not-encrypted verdicts, a whole restore reconciling, a short restore failing and naming the missing rows, malformed-400 and RBAC-403. Full gate green (typecheck, lint, secret-scan, **4,891 tests / 356 files**). **Metric moved: test-only packages 9 → 8 of 77** (down from 35 at the audit). Prior CORE-05: inc1–inc6 (`compliance` #135, `quality` #136, `self-checkout`/B25 #137, `owner-control` #138, `reversal` #139, `notifications` #140) all merged. Remaining unused engines: suspended-sales, org, documents, notifications-queue.)
