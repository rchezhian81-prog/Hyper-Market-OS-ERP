...
Last updated: 11 August 2026 (**★ TEN OWNER DECISIONS RATIFIED + CORE-05 COMPLETE ★**. **(1) CORE-05 done & merged (#144):** all ten domain engines are now on the live API (compliance, quality, self-checkout, owner-control, reversal, notifications, ops, org, documents, suspended-sales); test-only packages 15→5 of 77 (down from 35 at the 7-Aug audit); the 5 remaining are shared libs/frontend not wireable by design. The 'thin services' gap is closed. **(2) The ten owner-only decisions from the gap analysis are RATIFIED** (see `docs/requirements/OWNER_DECISIONS_2026-08-11.md`): D1 entity = proprietorship/partnership (so A27 audit-trail is best-practice, not law); D2 turnover = ₹5–10 cr → **e-invoicing IRN+QR now MANDATORY, new R2 build (A20)**, HSN 6-digit (A4, config), 30-day IRN (A21) N/A <₹10cr; D3 regulated goods = tobacco + gold + OTC pharmacy, **liquor REMOVED** (TN TASMAC monopoly); D4 own-brand = yes → EPR (R3/R4); D5 card = standalone → **smallest PCI scope**; D6 attendance = biometric+card/PIN → consent step (R3); D7 payroll = in-house → payroll/PF/ESI module (R4); D8 ONDC = defer; D9 wallet/BNPL = no; D10 online store = **wanted soon** → B23/A26/DPDP into near-term scope (R3/R4). **Sequencing = PHASE IT** (open with day-one essentials, schedule the rest — nothing dropped). The decision record maps every consequence to adopt-now-R2 / deferred-with-target / N-A-with-reason (CLAUDE.md governance). **Next natural build = the biggest new R2 item: e-invoicing (A20) IRN+QR generation on the M32 integration gateway**, OR the write-path increment (persist invoice lines → unlocks GSTR-1 + real price-integrity on stored data). Prod rmc-pilot-migrate-1 log still open if the owner wants the pilot fixed. Awaiting owner direction on which to build next.)
