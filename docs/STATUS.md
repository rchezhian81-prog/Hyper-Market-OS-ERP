...
**Then (revised per OA-9):** finish the M09 warehouse BACKEND — **FR-02 replenishment** and **FR-04
counts** — then build the offline-first **Warehouse PWA** and the **Web ERP** supervisory surfaces on the
authoritative warehouse/inventory services (the first CHANNEL/UI increment, following the
`tests/integration/the-screens-are-fed.test.ts` pattern, split honestly: data path first, screen shell
second). Other Phase 3 assembly (one authoritative engine per domain, up the completion ladder) continues
alongside. Owner-only blockers stay in `docs/OWNER-ACTION-REGISTER.md`.
