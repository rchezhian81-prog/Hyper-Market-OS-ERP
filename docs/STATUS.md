...
Last updated: 12 August 2026 (**★ M16 — THE RECALL TRACE NOW NAMES CUSTOMERS ★**: B11's recall trace could count identified vs anonymous recipients but every sale read anonymous, because a sale carries no customer. `lotTraceAdapter.soldOfBatch` (`services/api/src/adapters.ts`) now resolves the customers it can by **joining** the outbound sales to the loyalty/stored-value ledger's existing `{saleId, customerRef}` links (recorded when a member redeems/earns on a sale) — so a recall names the members who bought an affected batch and a walk-in stays anonymous. **NO new data collection** — only linkage the system already records; only the minimal `customerRef` is surfaced in the trace, and **actual contact stays consent-gated** (`packages/customer/src/consent.ts`, the recall runbook), PII minimised (M16-FR-03). Cloud-side, read-only, offline path untouched. **1 integration test** (a stored-value redemption links a sale → the trace names that customer, `identifiedRecipientCount` 1, the un-linked sale stays anonymous). Full gate green (typecheck, lint, secret-scan, traceability-integrity, no-test-doubles, services-run-on-their-engine, one-surface, full suite). The recall trace is now: exact batch where scanned, best-estimate where not, and named customers where a loyalty link exists. Prior: B1 (#158), batch-on-sale COMPLETE (#154-#157), B11 (#153).)
