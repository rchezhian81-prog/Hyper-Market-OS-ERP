...
Last updated: 10 August 2026 (**★ R2 v2.1 A2 — GAP-FREE ≤16-CHAR INVOICE SERIES WITH FY RESET, ON THE CLOUD ★**: the twelfth ratified-R2 increment, opening a fresh area (document numbering, M01) after the labelling group. A GST tax-invoice number carries three Rule 46 duties the generic gap-free series did not encode: ≤16 characters, unique for the whole FINANCIAL YEAR, and a fresh series each year. `allocateInvoiceNumber` puts the Indian FY (Apr–Mar, via `financialYearOf`) INSIDE the number (`INV/2627/000001`) so the sequence is consecutive/gap-free within a year and RESETS to 1 each April without ever colliding with last year's — a number is never reused (the allocator is strictly increasing within a FY) — and `assertInvoiceNumberValid` REJECTS a number over the 16-char limit BEFORE it is issued, not on a filed return. Pure/immutable-state (the caller owns persistence; no hidden clock). Runs on `GET /v1/finance/tax/invoice-number?prefix=&padTo=&seq=&fyOfSeries=&date=` (the finance tax calculator), gated `finance.period.read`. `tests/unit/numbering-invoice-series.test.ts` proves the FY boundaries (incl. Jan–Mar belonging to the prior April), the FY-reset, gap-free allocation, and the >16-char rejection; `tests/integration/tax-from-mrp.test.ts` proves the route + RBAC. Full gate green (typecheck, lint, secret-scan, **4,840 tests / 343 files**). No new dependency, no new permission. R2 progress: A1/A2/A4/A6/A8/A9/A10/A11/A12 (invoice+tax) + B3/B24/B9/B4 (labelling) = 12 ratified requirements built. Context unchanged: CORE-04 pure-read seam complete and exhausted; A5 (GSTR-1 Table-12) is blocked pending invoice-line data on the ledger (a write path first). Work advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
