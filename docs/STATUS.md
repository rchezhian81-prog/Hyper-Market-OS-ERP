...
Last updated: 11 August 2026 (**★ R2-B19 — SINGLE-USE-PLASTIC & CARRY-BAG GATE ★**: the till now refuses what the Plastic Waste Management Rules bar from leaving the store in a bag. `POST /v1/pos/single-use-plastic/check` (API-05, `checkSingleUsePlastic` in `packages/restricted-sales/src/single-use-plastic.ts`) enforces three rules: a **banned single-use-plastic SKU** (CPCB list) cannot be sold at all; a **plastic carry bag** must be **≥ 120 µm** (the threshold since 31 Dec 2022) — thinner, or a thickness the master data cannot prove, is refused, while cloth / paper / certified-compostable bags are exempt from the micron rule; and **a carry bag is a separate priced line** — a bag at zero price is refused (no free plastic bags). Names every failing line at once, commits nothing (a decision, not a write), reads the plastic attributes from the **offline price pack** so it works with the internet down (hard rule #1). Minimum thickness is configuration (OB-03). It **reuses** the `pos.restricted.check` permission the cashier already holds (no role change), added as a second route on the same B14 gate. **8 unit + 2 integration**. Full gate green (typecheck, lint, secret-scan, traceability-integrity, one-surface, full suite). OWNER CHECK in the store: try to sell a thin (<120 µm) plastic bag — refused; give a bag free (₹0) — refused; a proper 120 µm bag at a price — allowed. Prior: A7 (#151), B14 (#150), WRITE-PATH inc1-3 (A5 end-to-end), EINV inc1+inc2, CORE-05 complete.)
