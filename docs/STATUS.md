...
Last updated: 10 August 2026 (**★ CORE-04 inc4 — THE STORED-VALUE LIABILITY RECONCILIATION + REDEMPTION-VELOCITY FLAG ARE NOW READABLE ON THE CLOUD (M17-FR-03 / M23) ★**: the fourth CORE-04 increment, same method — two tested `packages/loyalty` engines (`reconcileLiability`, `flagVelocity`) that ran nowhere on the cloud, now wired onto API-06 as pure reads. Two NEW routes in `services/customer/src/stored-value.ts`: `GET /v1/stored-value/liability?posted=<minor>` folds the outstanding stored-value liability **tenant-wide** from the movement ledger (every unspent rupee on a gift card is money the shop OWES) and compares it **EXACTLY** against what the accounts have POSTED, naming any gap as unrecorded debt **with its sign** — the same discipline as a period-close control total, never 'about right'; and `GET /v1/stored-value/velocity` flags an instrument redeemed unusually fast for a person to look at — **detect-only**, blocking nothing, because a genuine customer spending a large gift card across a big shop looks identical. Decisively, the data was ALREADY on the cloud ledger: both fold the `StoredValueMovement` events the wired issue/redeem write path already produces and three wired reads already fold (balance, household pool, double-spend) — the only new adapter code is `allMovements`, a tenant-wide fan-out copied from `movementsForOwner` minus the owner filter. No new events, no write-path change, no migration, no owner input, no external auth, no edge data; `postedLiabilityMinor` is a supplied comparison figure on a READ (a query param, like `?asOf=` on the ageing report or `bankedForThemMinor` on concession settlement), not a write field. **RBAC least-privilege (P-04):** both are loss/books oversight surfaces, so both are gated one rung ABOVE the cashier's `loyalty.value.read` balance, on **`lp.case.read`** (owner / manager / accountant) — the SAME gate as the double-spend report in the same file. `tests/integration/stored-value-liability.test.ts` proves them over the real pipeline + real per-tenant RBAC (reconciles; a signed gap named unrecorded debt; refuses without a posted figure; velocity flags a fast card and nothing below threshold; refuses a malformed `?at=`; the cashier refused BOTH 403 while owner/manager/accountant read them; per-tenant isolation) AND end to end against real PostgreSQL (a `describe.skipIf(!DATABASE_URL)` e2e in the Stage-gate CI job). M17 stays PARTIALLY WIRED — nothing in FR-03 remains engine-only now; only coupons/referrals (M17-FR-02) do. Full gate green (typecheck, lint, secret-scan, **4,741 tests / 329 files**, plus the real-PG e2e). Held for pre-deployment as before: finance period-close second source, chart of accounts, real provider credentials, the ten owner decisions.)
