...
Last updated: 12 August 2026 (**★ B8 AT THE TILL — THE LANE NOW BLOCKS AN EXPIRED SCAN, OFFLINE ★** (owner-authorised offline-path change, M10·M12): the PRIMARY defence B8 asks for, on top of the cloud backstop already shipped. `CatalogueCache.scan(code, { asOf, batchExpiry })` (`packages/catalogue/src/catalogue.ts`) now throws **`ExpiredItemError`** — the sibling of the recall block, refused **even offline**, after the recall/status check — when the lane knows the scanned batch's use-by and it is **strictly before** today (the expiry is the LAST sellable day, so a sale ON it is fine). **Additive & backward-compatible:** a plain `scan(code)`, or one it cannot judge (no date / bad date), never blocks — it never guesses (hard rule #1), and the cloud backstop still catches what slips. **5 new unit tests** (expired-blocks, last-sellable-day-allowed, still-in-date, four backward-compat non-blocks, recall-checked-before-expiry). The `pos-offline` guardrail (no network in the sale path) stays green — the block adds no I/O. Full gate green (typecheck, lint, secret-scan, recall + offline guardrails, traceability-integrity, full suite). STILL TO WIRE (the capture side): the POS **shell passing the scanned batch's expiry into `scan`** — the gate is live and tested; the shell feeding it is the next step (same shape as the batch-on-sale shell capture). Prior: critical-breach guardrail + B8 cloud backstop (#160), M16 (#159), B1 (#158).)
