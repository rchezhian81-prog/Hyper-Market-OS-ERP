...
Last updated: 12 August 2026 (**★ D-2 — FORECAST-DRIVEN, CONSTRAINT-AWARE ORDER PROPOSAL ★** (M09·M06): the buyer's real question — given how much we expect to sell, and when the supplier next delivers, how big should this order be? Where D-3's `proposeReplenishment` answers "are we below the line," `proposeConstrainedOrder` (`packages/replenishment/src/constrained-order.ts`) sizes an order to the **supplier's own calendar** — an order placed now arrives at the **next** delivery and must last until the **one after**, so it covers exactly that window's demand (the D-1 forecast folded from the store's own banked sales), not a fixed max level. It nets **on-hand + on-order** still there at arrival, proposes nothing if that already covers the window (`covered`), rounds the shortfall **up to whole cases** (or `orderMultiple`), raises to the supplier `minOrderQty` still in whole cases, and reports the **pallet + loose-case** breakdown a buyer places. Fewer than two upcoming deliveries → `no_supplier_calendar` (never guesses a cadence); both constraints are DATA the caller supplies. **Advisory only** — a person commits the PO (hard rule #5). Runs on `POST /v1/replenishment/order-proposal`, gated `inventory.availability.read` (owner/manager, not the till), folding `soldLines` → `forecastDemand` → the engine end-to-end. **10 new tests** (7 unit: cover-window sizing, netting, covered, case+pallet breakdown, MOQ raise, no-calendar, malformed; 3 integration: end-to-end forecast-from-sales, covered/no-calendar, RBAC + malformed + past-date filtering). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). Prior: C3 retention-clock (#174), C1 consent-notice (#173), C2 breach-notification (#172), D-1 cold-start (#171), D-4 markdown (#169), D-3 (#164).)
