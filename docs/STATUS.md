...
Last updated: 10 August 2026 (**★ R2 v2.1 A6 — GST RATE IN FORCE ON THE TIME OF SUPPLY, ON THE CLOUD ★**: the fifth ratified-R2 increment (after A9/A8 #119, A10 #120, A11 #121, A12 #122). `resolveGstRate` picks the GST rate in force on the **time of supply** — not the invoice date, not today — from a per-HSN **effective-dated** schedule: the latest period whose effective-from is on or before the supply date, so a supply on the day a new rate takes effect gets the NEW rate and the day before gets the OLD one (the rate-change boundary, which is where this bites). Rates are DATA not logic — the schedule carries whatever applies (the 2017 slabs, the GST-2.0 0/5/18% + 40% demerit) — and it REFUSES a supply date before the earliest rate, an ambiguous duplicate effective date, or bad input rather than guess a rate. Runs on `GET /v1/finance/tax/rate-on-date?schedule=YYYY-MM-DD:bps,…&supplyDate=` (the same back-office finance calculator, gated `finance.period.read`). `tests/unit/finance-rate-on-date.test.ts` proves the boundary (18% the day before, 40% on/after) + the refusals; `tests/integration/tax-from-mrp.test.ts` proves the route + schedule parsing + the RBAC gate. Full gate green (typecheck, lint, secret-scan, **4,789 tests / 335 files**). No new dependency, no new permission. The R2 fiscal core so far: A9/A8 MRP extraction+split, A10 rounding, A11 discount eligibility, A12 promo supplies, A6 time-of-supply rate. Context unchanged: CORE-04 pure-read seam complete (5 increments) and exhausted; work advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
