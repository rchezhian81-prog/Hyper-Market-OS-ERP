...
Last updated: 12 August 2026 (**★ R2-B1 — NO SALE ABOVE MRP, NOW ENFORCED AT THE TILL TOO ★**: MRP is a legal ceiling in India (not a shop policy), and B1 closes the last gap. It was already refused at **price-setting** (`price-guard` returns `above_mrp`, `allowed:false`, no approval path) and at **pack build** (`buildCatalogueSnapshot` EXCLUDES an above-MRP product so a lane can never hold one), with the `the-pricing-screen-cannot-break-the-law` guardrail asserting no `overrideMrp`/`forcePrice` escape hatch exists. NEW: the **till** now catches it too — `acceptSale` raises a **CRITICAL `sold_above_mrp`** exception when a line arrives charged above the product's MRP, ranked with a recall and telling the manager to **refund the difference**; the sale still banks (money's in the drawer, hard rule #1) but the illegal overcharge is surfaced, not buried in an ordinary price variance (it subsumes `price_differs_from_catalogue` for that line). **6 new unit/route tests** (critical + ranked-first + subsumes-variance + at-or-below-MRP-allowed + no-MRP-no-breach + banks-and-records). Full gate green (typecheck, lint, secret-scan, the pricing-law guardrail, traceability-integrity, one-surface, full suite). OWNER CHECK: a cashier overcharging above MRP (however it happened) now shows on the day-end critical list, not just as a price note. Prior: batch-on-sale COMPLETE (#154-#157), B11 (#153), B19 (#152), A7 (#151), B14 (#150).)
