...
Last updated: 11 August 2026 (**★ R2-B11 — WRITTEN RECALL PLAN + ONE-UP/ONE-DOWN BATCH TRACE ★**: the document a food recall runs on. `POST /v1/quality/lot-trace/export` (API-04, `buildLotTrace` in `packages/quality/src/lot-trace.ts`) assembles, for a batch, the two lists FSSAI / Codex "one step back, one step forward" requires: **inbound** (which supplier + goods-receipt note + date + qty it came from) and **outbound** (each sale + date + qty + the customer *where identified*), and **reconciles** them. An anonymous walk-in sale is **kept** (counted, not dropped — a recall must know those units left the store), and a batch that dispatched **more than it received** surfaces as `reconciled:false` with negative remaining (a traceability gap is a safety failure, not a rounding note, P-08). Records sort by date so a re-run is byte-identical. Gated `quality.lottrace.read` (owner / store-manager, NOT the till). The **recall block itself** — a recalled batch cannot be sold, even offline — was already honoured (`tests/guardrails/a-recalled-tin-cannot-be-sold.test.ts`); this is the trace half. The **written recall plan** is the plain-English `docs/runbooks/product-recall.md` (stop-the-sale → trace → reach people → account for every unit → close with evidence). **6 unit + 3 integration**. Full gate green (typecheck, lint, secret-scan, traceability-integrity, one-surface, full suite). NOTE: real outbound sales feed in once **batch-on-sale** is captured at the till (the owner-blocked offline-core change); the reconciled export format + gap detection is the deliverable now. Prior: B19 (#152), A7 (#151), B14 (#150), A5 end-to-end.)
