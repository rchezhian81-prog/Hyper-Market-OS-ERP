...
document issue/reproduce write path, M31-FR-02). Working branch `claude/new-session-lw91i4`
is reset to that commit — no divergence, no lost work (verified `git diff --stat e49b4ee 052c192`
is empty, confirming #225 is integrated).

**What Phase 0 is:** a governance and honesty pass before any new functionality — verify the true
branch/baseline state, repair stale project documentation, and record the outstanding governance
decisions for the owner. No feature code changes in this phase.

- **Baseline gate re-run (full suite, 18 Aug):** typecheck ✓, lint ✓, `vitest run` = **5659 passed,
  262 skipped** across 450 files, **e2e 13 passed**, `pnpm run completion` = **40.3%**. Maturity
  counts: NOT_STARTED 1, ENGINE_ONLY 13, PARTIALLY_WIRED 78, WIRED 7, INTEGRATION_TESTED 3,
  E2E_VERIFIED 2.
- **Honest condition:** the application code is live and test-gated at **~40.3% weighted completion**
  (single reproducible model), full suite green. **This is NOT pilot-ready or production-ready.** Most
  modules are PARTIALLY_WIRED; the pilot/production gates (real-data migration, independent
  penetration test, certified-hardware + DR rehearsal, live provider credentials, explicit owner GO)
  are unmet. Completion is reported honestly and never inflated.
- **README repaired:** the stale "no application code yet, by design" claim is replaced with an honest
  snapshot pointing at this file and `docs/OWNER-ACTION-REGISTER.md`.
- **completion-status.json evidence reconciled** (descriptive text only, no label or % change):
  M06 count-review screen shipped; M18 substitution + channel-reconcile now wired; M25 workforce
...
Last updated: 13 August 2026 (**★ ITEM 2 e-invoice/e-way-bill RECONCILIATION COMPLETE (inc1–4); WP4 GST-RETURN SUBMISSION SAFETY COMPLETE (item 1); ITEM 3 (UI/UX + a11y + E2E) IN FLIGHT — inc1 shared design-system foundation `packages/ui`; inc2 FIRST SCREEN (GST e-invoice/e-way-bill reconciliation operator view) on it; inc3 made that screen a first-class OFFLINE screen the box serves, with real-browser offline+a11y E2E; COMPLETION MODEL baseline v1 = 40.3% ★**. **Item 2 inc1 BUILT + WIRED 13 Aug:** e-invoice reconciliation — `eInvoiceQueueCategory`/`isEInvoiceException` (operator vocabulary), `POST /v1/finance/e-invoice/invoices/:id/poll` (re-query the sandbox IRP for a submitted/pending_unknown invoice → **acknowledgement recovery**; terminal = safe no-op, no duplicate registration; never fabricates the IRN), and `GET /v1/finance/e-invoice/register?state=` (the **exception queue** folding a tenant-wide `EInvoiceIndexed` index appended atomically beside each submit; filter by category or `exceptions`). Tenant-isolated, RBAC-gated, restart/replay-safe. Sandbox only — live GSP EXTERNALLY BLOCKED. 2 unit + 5 integration; full gate green (5376 passed). **Completion % unchanged at 40.3%** (`pnpm run completion`): M23 deepened within PARTIALLY_WIRED, no maturity threshold crossed — reported honestly, not inflated. **Item 2 inc2 BUILT + WIRED 13 Aug:** the **durable e-way-bill register** — the e-way bill was stateless (eligibility/validity/sandbox-generate); now it has the transport twin of the e-invoice register (`services/finance/src/e-way-bill-register.ts` + `eWayBillAdapter`): `POST /v1/finance/e-way-bill/movements/:id/{submit,record-response,cancel}` + `GET …/:id`, one stream per movement folded by `foldEwayBill`, an atomic tenant-wide `EwayBillIndexed` index on submit, so an e-way bill **survives a restart**; never fabricates the 12-digit number; `generated` final; 24h cancel window. Plus the operator queue helpers `ewbQueueCategory`/`isEwbException` (for the inc3 queue). 2 unit + 4 integration; full gate green (5382 passed). **Completion % unchanged at 40.3%** (M23 deepened within PARTIALLY_WIRED, no threshold crossed). **Item 2 inc3 BUILT + WIRED 13 Aug — the e-way-bill reconciliation, closing the e-way-bill side of item 2:** the transport twin of the e-invoice reconciliation (inc1) — `POST /v1/finance/e-way-bill/movements/:id/poll` (re-query the sandbox portal for a `pending_unknown` movement → **acknowledgement recovery** via the never-fabricate `applyEwbResult`; a `generated`/`cancelled`/`rejected` movement is terminal so a poll is a **safe no-op** `polled:false`; forced sandbox `unknown`/`rejected` exercise the timeout/outage paths; gated `finance.einvoice.generate`, idempotent) and `GET /v1/finance/e-way-bill/register?state=` (the **exception queue** — every movement's operator category, `?state=exceptions` = `pending_unknown`+`provider_error`+`rejected`; read-gated + tenant-isolated; nothing auto-corrected). 2 unit + 3 integration added; full gate green (5385 passed). **Completion % unchanged at 40.3%** (`pnpm run completion`: M23 deepened within PARTIALLY_WIRED, no threshold crossed — reported honestly, not inflated). **Item 2 inc4 BUILT + WIRED 13 Aug — CLOSES owner directive item 2 (both documents):** MISMATCH DETECTION. A poll resolves a *stuck* document but leaves a *terminal* one alone; a stored IRN/EWB number can still silently drift from the portal. `POST /v1/finance/e-invoice/invoices/:id/verify` + `POST /v1/finance/e-way-bill/movements/:id/verify` re-query a registered/generated (or cancelled) document (in-flight → 422 pointing at poll) and COMPARE via the pure `detectEInvoiceMismatch`/`detectEwbMismatch`: agreement is a no-op; a disagreement (a DIFFERENT number, or the portal reporting REJECTED while a number is on file, or a cancelled doc the portal still shows live) records a `MismatchObserved` fact — an **ADDITIVE FLAG, never an overwrite** (hard rule #10: a conflict is a visible exception, not a silent last-write-wins). The fold applies the flag without touching the state or the number (replay-safe); a new **`mismatch`** queue category (`eInvoiceRowCategory`/`ewbRowCategory`) moves the doc into the `exceptions` filter for a human to reconcile — the software never picks a winner. Connector observation posted directly (`observed`) or sandbox re-query (`sandbox.forceOutcome`); new `EInvoiceMismatchObserved`/`EwayBillMismatchObserved` adapter facts; gated generate/read; tenant-isolated. +7 unit +6 integration; full gate green (5398 passed). **Completion % unchanged at 40.3%** (M23 deepened within PARTIALLY_WIRED, no threshold crossed). **★ OWNER DIRECTIVE ITEM 2 (e-invoice/e-way-bill reconciliation) IS NOW COMPLETE — poll + exception queue + mismatch, both documents. ★** **NEXT: directive item 3 — UI/UX + accessibility + connected E2E pass** over payroll, category policies, GST returns and e-invoice/e-way bill (reassess `apps/web-erp` shells + any `packages/ui` design-system first; bilingual English/Tamil; keyboard + screen-reader a11y; loading/empty/error/pending/locked states; maker-checker workflow; visual regression + automated a11y + role-based E2E). **Item 3 RECON done (read-only survey, 13 Aug):** `packages/ui` was an empty stub; each web-erp screen is a bespoke tested `src/*-session.ts` model + hand-rolled `web/*.js` shell with its own inline `WORDS={en,ta}`; `packages/a11y` (contrast/status/touch-target/focus, pure) exists but web-erp doesn't import it; E2E is Vitest+playwright-core (one offline spec, NO axe/a11y assertions, NO visual regression); **none of the four directive-3 domains has a screen yet** (backend only). **Item 3 inc1 BUILT 13 Aug — the shared design-system backbone (`packages/ui`):** three pure, framework-free primitives on the tested `packages/a11y` so a state is never a bare colour — `copy.ts` (`BilingualCopy<K>` with both languages required by construction, `translator` en-fallback-never-blank, and `bilingualGaps`/`isBilingualComplete` — the reusable "speaks both languages" check replacing each screen's regex), `states.ts` (`ScreenState` closed set loading/ready/empty/error/pending/locked/recovery + `presentScreenState`), `queue-status.ts` (`QueueCategory` from item 2 + `presentQueueCategory`/`isQueueException`; `mismatch` is its own attention state). 13 unit; full gate green (5411 passed). No completion-% change (net-new shared primitive; the consuming screens are the next increments). **Item 3 inc2 BUILT + WIRED 13 Aug — the FIRST screen, on packages/ui:** the GST e-invoice/e-way-bill **reconciliation operator triage view** (`apps/web-erp/src/gst-reconciliation-session.ts` + `web/gst-reconciliation.{html,js}`). Shows the item-2 queue with the ones needing attention first, each with a colour-never-alone status (icon + word via `presentQueueCategory`) and, in plain language, what to do (`recommendedAction`) and whether the user may. All rules in the tested DOM-free session model; the shell only renders. Read-gated + action-gated exactly as the server (`finance.einvoice.read`/`.generate`, default-deny; a reader who cannot act sees "needs the GST-portal role"; no rows leak to a non-reader). English+Tamil from ONE `BilingualCopy` a guardrail binds to via `bilingualGaps`. Nothing changes a document — a mismatch is surfaced, never auto-corrected (rule #10); no alert/confirm/prompt. Wired into `browser-entry.ts` (`window.gstReconciliationSession`) + `navigation.ts` (Finance group, `finance.einvoice.read`).

**Item 3 inc3 BUILT + WIRED 14 Aug — the screen is now a FIRST-CLASS OFFLINE screen the box serves.** Registered end-to-end in the edge pipeline (`edge/store-edge/src/screen-data.ts` `SCREENS`/`GLOBAL_FOR`/`BUILDERS` + `gstReconciliationPayload`; `store-pack.ts` new `gstReconciliationQueue`/`gstReconciliationPolicy` sections + `PackGstReconciliationPolicy`; `screen-server.ts` `APP_SHELL`): the box serves `/gst-reconciliation`, injects `window.gstReconciliationData` (last-synced queue + userId + permissions) at the `<!--SCREEN-DATA-->` marker, carries the pack-age badge, and **the service worker precaches the shell** so it opens with the cable out (P-01/§31). New pack sections are honestly absent until sync populates them → the screen shows its sample stand-in, never an empty queue that reads as "all settled". **Real-browser E2E** (headless Chromium, `screens-open-offline.e2e.ts`): loads → cuts network → reloads → still opens from cache AND is accessible (lang toggle + list labelled; every status carries a screen-reader announcement + a visible word, never colour alone; icons aria-hidden). **Browser-free static a11y guardrail** added so a11y holds where CI has no browser. Held to the same offline discipline as the other 11 web-erp shells. Full gate green: typecheck, lint, **5431 unit/guardrail/integration + 9 e2e**; ERP bundle builds. No completion-% change (screen surface; no module maturity threshold crossed). **Deferred:** wiring the poll/verify buttons to the item-2 API command path (via the box outbox), and cloud→box sync populating the real queue into the pack. **Item 3 → PAYROLL UI inc1 BUILT 14 Aug (owner directive: payroll UI security boundary):** the payroll operator screen, on the tested `packages/payroll` engine, to a HIGHER security bar than any other screen — the owner's explicit rule was NOT to copy the GST offline-caching model. `apps/web-erp/src/payroll-session.ts` (DOM-free, tested): least privilege (no `payroll.statutory.read` → the view leaks nothing); masking of bank/PAN/UAN/Aadhaar (new `packages/payroll/src/mask.ts`, last-4, Aadhaar never revealed); maker ≠ checker (submitter can't approve — enforced here and in the engine); a locked run is final (reason'd reversal, never a silent edit); online-first (submit/approve/lock/reverse refused offline); negative/zero net + missing bank block submission. The shell (`payroll.{html,js}`) is the FIRST deliberately **online-only** view — NO service worker (nothing sensitive cached), NO browser storage, sample data stamped **"DEMO DATA — NOT REAL PAYROLL"**, buttons disabled offline, English+Tamil, a11y. **NOT** on the offline store box (`SCREENS`) so shared floor devices never retain payroll. Nav item gated `payroll.statutory.read`. Governing spec recorded at `docs/design/screens/payroll.md`. 8 mask + 13 session + 11 guardrail tests; full gate green (**5462**). Live payroll BLOCKED pending CA/HR/legal sign-off; DEMO/simulator + tests proceed. **PAYROLL UI inc2 BUILT 14 Aug:** the locked-run outputs behind a re-auth gate. A LOCKED run surfaces the **bank-file summary** (record count + total net, reconciled against the run net — no account numbers in the browser; the file with full accounts is server-side under re-auth) and the **balanced accounting journal** (`buildPayrollJournal`, debits===credits; an unbalanced total set is a reconciliation FAILURE, never a silent pass). **Re-auth/MFA** now gates approve, lock, reverse, bank-file generation, bulk payslip download and export (`reauthAgeSeconds` port + freshness window; MFA is a step via `window.payrollReauth`, not a dialog; offline still beats a fresh re-auth; `submit` not gated). All inc1 controls kept. 33 payroll session/mask + 14 guardrail; full gate green (**5476** + 9 e2e). **PAYROLL UI inc3 BUILT 14 Aug:** statutory report (PF/ESI/PT/TDS employee+employer shares from the run totals, CONFIRM-WITH-CA); **full-and-final settlement** for a leaver on the tested `computeSettlement` — signed net where a **negative net is EXPECTED** (recoverable from the employee) and is NOT a pay-run blocking exception, release re-auth-gated (`releaseSettlement`); **arrears/loan/advance** shown as visible flag badges (never silent); **reversal** names its reason; a draft/**version delta** vs the previous version. All inc1+inc2 controls kept. 32 session + 8 mask + 18 guardrail; full gate green (**5488** + 9 e2e). **PAYROLL UI inc4 BUILT 14 Aug — PAYROLL UI COMPLETE (inc1–4):** the employee self-service payslip, a SEPARATE surface (own model `apps/web-erp/src/payroll-ess-session.ts` + own shell `payroll-payslip.{html,js}` + own nav item gated `payroll.ess.self`). **Strict own-record isolation** on the tested `ess.employeeSelfView`/`assertSelfScope`: the requester is the AUTHENTICATED principal (`config.requesterEmployeeId`), never a page value, so holding the permission cannot read a colleague — a cross-employee request is refused and shows NOTHING; a subject swap cannot widen access. The redacted view is the employee's own earnings/deductions/net + the employer's contribution shown separately, with no approver/cost-centre/bank/other-employee in the shape. Print/export is audited behind a fresh re-auth (offline-blocked). Same posture as the admin screen (online-first, no service worker, no browser storage, DEMO banner, English+Tamil, a11y). 9 session + 8 guardrail; full gate green (**5505** + 9 e2e). **Deferred (all payroll screens):** the audited API commit path (real MFA challenge + real bank-file/payslip generation) + the full browser E2E workflow; live payroll activation stays blocked pending CA/HR/legal sign-off.

**CATEGORY-RULES SCREEN BUILT + WIRED 14 Aug** (operator screen for the built WP1/WP1b category-policy engine; M03-FR-01·CAT-POLICY): a first-class OFFLINE screen like GST reconciliation. `apps/web-erp/src/category-policy-session.ts` (tested, DOM-free) on the tested `packages/product` `resolvePolicy`: resolves each category's rules on the box's own trading day and gives it a triage status that is never a bare colour — OK (grocery), CONTROLLED (gold/pharmacy-lite shipped OFF or an age/KYC/PAN/serial control), BLOCKED (prescription), or NO POLICY (attention). Attention-first; controls + behaviour flags shown as bilingual badges. Read-only (a rule change is an effective-dated catalogue entry, never here); read-gated `catalogue.pack.read`; English+Tamil. Full edge pipeline (SCREENS/GLOBAL_FOR/BUILDERS/payload + store-pack sections + APP_SHELL + sw.js precache CACHE v10) + nav (Catalogue group) + real-browser offline+a11y E2E. 9 unit + 8 guardrail; full gate green (**5521** + 10 e2e).

**GST RECONCILIATION ACTION-WIRING BUILT 14 Aug** (item 3 inc-e; the deferred piece from inc2/inc3): the operator can now ACT on a reconciliation exception, not only view it. The two portal-touching routes item 2 built — **poll** (chase up a stuck `unknown` document) and **verify** (re-check a settled one against the portal) — are now clickable, and acting **never calls the portal from the screen**: the button commits a deterministic, PII-free `GstPortalActionRequested` command to a **device-backed offline outbox** (`gst-reconciliation-session.ts` `requestAction`/`buildPortalCommand` + `browser-entry.ts` `openDeviceOutbox` over localStorage; survives a reload, §31), which the sync agent drains **idempotently** to the item-2 API command path — a user action never blocks on the network (hard rule #1). **`portalActionsFor` is the gate:** `unknown`→poll, terminal→verify, and **`mismatch`/`error`→NO button** (a disagreement is investigated by a person, never auto-corrected — hard rule #10); `rejected` is a reissue workflow, `processing` is waited on. Idempotency keyed on document+action+observed-category (double-click/reload collapses to one; a genuine re-request after the portal moves the doc is distinct). Action-gated as the server (`finance.einvoice.generate`; a reader without it sees the button disabled + "needs the GST-portal role"; a nobody-named box acts on nothing). Deterministic command — no AI (hard rule #5 does not arise). Queued state shown on the row so nothing is silent (P-08). New guardrail: the shell fires **no** raw network call (no fetch/XHR/WebSocket/sendBeacon/EventSource) — the action goes through the outbox only. 13 unit + 3 guardrail added; full gate green (**5534** unit/guardrail/integration + 10 e2e); ERP bundle rebuilt. No completion-% change (net-new action surface on an already BUILT+WIRED screen). **Deferred (thin follow-on):** the cloud-side drain handler that translates a drained `GstPortalActionRequested` into the internal poll/verify call, and cloud→box sync populating the real queue into the pack.

**GST-RETURNS SCREEN BUILT + WIRED 14 Aug — the 4th and FINAL item-3 UI domain (payroll ✓, category policy ✓, e-invoice/e-way-bill reconciliation+action ✓, GST returns ✓).** The operator screen for the WP4 (item 1) GSTR-1 submission-safety engine (`packages/finance/src/gstr1-submission.ts`). A first-class OFFLINE screen (like GST reconciliation): the box serves `/gst-returns`, injects `window.gstReturnsData` (every filing period folded to its current submission state + who is looking) at the `<!--SCREEN-DATA-->` marker; the SW precaches the shell (CACHE v11). **All rules in the tested DOM-free session model** (`createGstReturnsSession`) on the tested engine's `queueCategory`/`isSubmissionException`: every filing period with a triage status that is never a bare colour (icon + word + announcement via `presentStatus`) — Filed, Prepared→awaiting-approval, Approved→ready-to-file, Being-filed, Rejected (exception), Outcome-unknown→reconcile (exception), Withdrawn. **The exceptions (failed + unknown) come first**; each carries the next step in plain language (`recommendedAction`). **The one thing it must never do — bound by a guardrail + the model:** an UNKNOWN outcome is NEVER shown as filed; it reads as attention and says *reconcile with evidence* (hard rule #10). **Read-only** (nothing filed here — preview/approve/submit are the maker→checker API routes; wiring them is the follow-on). **Read-gated** `finance.gstr.read` (default-deny). English + Tamil from ONE `BilingualCopy` a guardrail binds to; no alert/confirm/prompt. New pack sections `gstReturnsQueue` + `gstReturnsPolicy` (honest absence → sample stand-in). Nav item in the Finance group. Real-browser offline + a11y E2E. The session imports the engine helpers from the specific `gstr1-submission` module, NOT the `packages/finance` barrel (which re-exports the `node:crypto` GSTN sandbox and would break the browser bundle). 11 unit + 9 guardrail; full gate green (**5554** unit/guardrail/integration + 11 e2e); ERP bundle rebuilt. No completion-% change (net-new screen surface on the already BUILT+WIRED M23 GSTR-1 path). **★ OWNER DIRECTIVE ITEM 3 (UI/UX + a11y + E2E) — all four named domains now have a tested, offline-first, bilingual, accessible operator screen. ★**

**GST-RETURNS ACTION-WIRING BUILT 14 Aug** (the deferred maker→checker piece from the GST-returns screen): an authorised person can now DRIVE the return-filing workflow from the screen, not only view it. The two one-click governance steps that move a return toward filing — **approve** (a PREPARED return → approved, by a checker who is NOT the maker — §28) and **submit** (an APPROVED return → filed) — are clickable. Acting **never calls the portal from the screen**: the button commits a deterministic, PII-free `GstReturnActionRequested` command to a **device-backed offline outbox** (`gst-returns-session.ts` `requestAction`/`buildReturnCommand` + `browser-entry.ts` `openDeviceOutbox`; survives a reload, §31), which the sync agent drains to the maker→checker API afterwards, where the engine re-checks **maker ≠ checker**, the **digest match** and the **live-portal gate** at the write boundary (hard rule #1 — a user action never blocks on the network). **`commandActionsFor` is the gate:** previewed→approve, approved→submit, and **failed/unknown→NO button** (a stuck return is corrected/reconciled with evidence by a person — hard rule #10); in-flight/terminal offer nothing. **Maker ≠ checker in the UI too:** the maker sees approve DISABLED on their own return with "you prepared this" (the engine refuses it again server-side). **Separate permissions** matching the server — approve `finance.gstr.approve`, submit `finance.gstr.submit`. Idempotency keyed on period+action+observed-state. Deterministic command — no AI (hard rule #5). Queued state shown per row (P-08). New guardrail: the shell fires **no** raw network call. Preview/reconcile/cancel (which need figures/evidence/reason) remain richer follow-on forms. 12 unit + 3 guardrail added; full gate green (**5569** unit/guardrail/integration + 11 e2e); ERP bundle rebuilt. No completion-% change (net-new action surface on the already BUILT+WIRED M23 path). **Deferred (thin follow-on):** the cloud-side drain handler that translates a drained `GstReturnActionRequested` into the internal approve/submit call.

**SYNC DRAIN REACHES THE PORTAL-ACTION COMMANDS BUILT 14 Aug** (the consumer for the GST-reconciliation action-wiring): the outbox commands the GST-reconciliation screen enqueues now REACH the API. The store-edge sync transport (`edge/sync-agent/src/http-transport.ts`) had no route for `GstPortalActionRequested`, so a drained command would dead-letter — the operator's poll/verify request never reached the portal. `EVENT_ROUTES` now accepts a **resolver function** (`EventRoute = string | (payload)=>path`) because the target depends on two payload fields; `gstPortalActionRoute` maps `{documentType,id,action}` → `/v1/finance/e-invoice/invoices/:id/{poll|verify}` or `/v1/finance/e-way-bill/movements/:id/{poll|verify}` — the **same door it would have online** (re-authorized `finance.einvoice.generate`, idempotent on the command's own key). **ONLY poll/verify** (idempotent, non-governance) are relayed under the store's own service token. An unrecognised action / unknown document / missing id is **rejected by name → dead-lettered** (hard rule #6). **Deliberately NOT routed:** `GstReturnActionRequested` (approve/submit) + the warehouse decisions — their routes decide maker≠checker from `ctx.userId`, so relaying under the store token would act as the store, not the operator (breaks §28); they need a dedicated relayed-actor "apply a synced governance command" route (a separate security-reviewed increment) and until then dead-letter honestly. 7 unit added; full gate green (**5575** unit/guardrail/integration + 11 e2e). No completion-% change (net-new transport route on the already-wired SYNC-01 drain).

**⚠ SECURITY FINDING (recorded, not yet actioned) — the relayed-actor governance-drain is NOT safe as designed.** Assessed option (A) (draining the GST-returns approve/submit commands to the API under the store token) and found the trust model UNCLEAN: the engine's approve gate is only `actor === previewedBy` (string inequality) — it does NOT verify the actor is a real user holding `finance.gstr.approve`; that authorization is entirely the ROUTE's job via the authenticated `ctx.userId`. So a relayed-actor route gated on the STORE token, passing a client-asserted `requestedBy`, would let the store box approve any return by relaying any id ≠ the maker — **downgrading §28** (it proves the edge *said* a different person approved, not that a distinct authenticated person did). Unlike a sale's `cashierId` (audit attribution), approve *is* the authorization gate. **The shipped GST-returns approve/submit action-wiring therefore has no safe consumer yet** — those commands dead-letter honestly and the operator sees "could not be sent — a person should check" (P-08; nothing wrong happens). **The correct fix is a follow-on for the owner's attention:** governance actions should be **online-first with the operator's own live session token** (like payroll's sensitive actions), NOT relayed via the store service identity — i.e. the GST-returns approve/submit buttons should be reworked to post directly online rather than through the offline outbox. Recorded here rather than rushed.

**WASTE & WRITE-OFF REVIEW SCREEN BUILT + WIRED 14 Aug** (M28-FR-01; the operator surface for the engine-only write-off engine, chosen as the safe next increment after the governance-drain finding). A first-class OFFLINE shrinkage-triage screen (like GST reconciliation / category rules): the box serves `/waste`, injects `window.wasteData` (recorded losses folded to review rows + who is looking) at the `<!--SCREEN-DATA-->` marker; the SW precaches the shell (CACHE v12). **All rules in the tested DOM-free session model** (`createWasteReviewSession`) on the tested `packages/waste` loss-type vocabulary: every recorded write-off with a triage status that is never a bare colour (icon + word + announcement via `presentStatus`) — loss type (wastage/damage/expired/donation/destruction), deliberate disposals idle, unplanned losses degraded. **The MATERIAL losses (needed a §28 approval) come first**, bumped to error/attention — shrinkage seen by exception (P-03), not buried in a total. Each row carries value (₹), quantity, evidence-on-file flag, raiser; the view totals the day's loss. **Read-only** (recording a write-off is the audited write path, never here — a guardrail asserts no fetch/XHR/enqueue). **Read-gated** new permission `waste.view` (owner + store_manager; default-deny). English + Tamil from ONE `BilingualCopy` a guardrail binds to. New pack sections `wasteWriteOffs` + `wastePolicy` (honest absence → sample stand-in). The existing `/waste` nav item now resolves to a real screen. Real-browser offline + a11y E2E. Session imports the loss-type vocabulary from the specific `waste.ts` module, NOT the barrel. 10 unit + 9 guardrail; full gate green (**5594** unit/guardrail/integration + 12 e2e); ERP bundle rebuilt. Advances M28 from PARTIALLY_WIRED (write-off engine-only) toward a wired operator surface. No completion-% change (net-new screen surface; M28 stays PARTIALLY_WIRED pending the write path + integration). **NEXT: the next dependency-priority incomplete roadmap module** (by dependency + impact) — OR, for the owner: the GST-returns governance-action online-first rework noted above. Continuing automatically per the owner directive. **Payroll maturity (owner-recorded):** implementation COMPLETE; integration-tested COMPLETE; **live payroll operation EXTERNALLY BLOCKED** pending CA/HR/legal confirmation; UAT + production verification PENDING. No live payroll activation and no hard-coded statutory rates before sign-off. **WP4 GST-return submission safety is now COMPLETE (inc1–3, item 1) in practice mode.** **WP4 inc1 BUILT 13 Aug (engine-first):** the pure GSTR-1 **submission-safety state machine** (`packages/finance/src/gstr1-submission.ts`) — event-sourced `previewed→approved→submitting→filed`(+`failed`/`unknown`); `foldGstr1Submission` is replay-safe (an event that does not fit is ignored, so a duplicate/out-of-order delivery cannot corrupt state); `evaluateGstr1SubmissionTransition` enforces **maker ≠ checker**, **duplicate-submission prevention** (a filed/in-flight period refuses a new submit or preview), **digest match on submit** (no silent change between approval and filing; period locked to the approved digest), **unknown → reconciliation not straight-to-filed**, and **reconcile-needs-evidence** (no silent rewrite); `classifyGstnError` gives operators a recovery class. 12 unit; full gate green. **WP4 inc2 BUILT + WIRED DURABLY 13 Aug:** the engine is now on the live surface — `gstr1SubmissionAdapter` (`services/api/src/adapters.ts`) appends each lifecycle step to the shared append-only `event_ledger` (one stream per filing period) and folds it with `foldGstr1Submission` (survives restart); `services/finance/src/gstr1-submission-store.ts` hosts `POST /v1/finance/gstr1/submission/:period/{preview,approve,submit,record-response}` + `GET …/:period`, every transition checked against the STORED state before the append (maker≠checker, dup-prevention, digest-match, period-lock at the write boundary). Maker-checker RBAC-separated: preview `finance.gstr.generate`, approve NEW `finance.gstr.approve`, submit NEW `finance.gstr.submit` (owner). LIVE path OFF by default + killable (`requireGstPortalLive(controls,'gst_return')` → 403 disabled/killed; 503 gate-open-but-no-certified-connector); the deterministic **sandbox GSTN provider** (`packages/finance/src/gstn-sandbox.ts`, provider-neutral, `SANDBOX-` non-fileable ARN, real dup detection, forced failed/unknown) runs otherwise; async/webhook path covered (`submit {async:true}` → submitting → `record-response`; unknown routes to reconciliation not straight-to-filed). 3 sandbox unit + 6 integration; full gate green (5357 passed). **WP4 inc3 BUILT + WIRED 13 Aug — CLOSES owner directive item 1:** RECONCILIATION + the exception queue. Adds a `cancelled` state + `cancel` transition and the operator-queue vocabulary (`queueCategory`→pending/processing/success/failed/cancelled/unknown; `isSubmissionException`→failed+unknown). Routes: `POST …/:period/reconcile` (resolve a stuck **unknown** to filed/failed WITH EVIDENCE — recorded `reconciled` fact, never a silent rewrite), `POST …/:period/poll` (re-query the sandbox for a submitting/unknown submission → **acknowledgement recovery**; safe to repeat), `POST …/:period/cancel` (gated `finance.gstr.approve`), `GET /v1/finance/gstr1/submissions?state=` (the **exception queue**, folding a tenant-wide period index appended on first preview, filterable by queue category or `exceptions`). Immutable audit, tenant isolation + RBAC, restart/replay proof. +4 engine unit +4 integration; full gate green (5364 passed). **WP4 remaining:** the credential-vault interface (a small stub increment), then the UI/E2E pass (directive item 3). NO live submission — the safe boundary is complete end to end in practice mode; live filing externally blocked pending CA/legal + production creds + owner GO. **NEXT WP: owner directive item 2 (e-invoice/e-way-bill reconciliation).** **REPORTING:** the reproducible weighted-maturity completion model (fixed weights, `Σ weights ÷ total controlling requirements`, six separate scores) is being added as the immediately-following documentation increment per the owner's reporting correction — this update stops quoting estimate ranges and reports categorical maturity (WIRED · INTEGRATION TESTED) instead. **Prior program (WP3) below:** ★ WP3 PAYROLL COMPLETE (inc1–9) — statutory deductions → payslip → TDS → approve/lock → bank file → journal → F&F settlement → ESS → DURABLE pay-run store (survives restart) ★: PR #182 (the uploadable GSTN portal file, below) is merged to `main`; local branch reset to merged main. The owner has ratified a large **continuation directive** (12 Aug) authorising autonomous multi-workstream development under a firm operating rule: _do not stop at a clean checkpoint for want of external credentials or future business confirmation — use safe configurable defaults, provider-neutral adapters and simulators, record the external-verification gap, and continue every unaffected workstream; ask only for paid contracts, production credentials, lawful data access, legal/CA sign-off, destructive production actions, or final pilot/production GO._ **Delivery sequence now in flight:** (1) reusable **category-policy engine** — category rules as effective-dated CONFIGURATION not hard-code, covering traceability level, valuation, tax, shelf-life, returns, controlled-sale, serial/batch/weight behaviour and approval requirements — then the high-risk category controls (grocery/FMCG, fresh produce, gold/jewellery [optional controlled vertical, disabled by default, CA/legal-gated], OTC pharmacy-lite [configurable, Rx/Schedule-H/H1/X blocked, not a licensed-pharmacy system], cosmetics, electronics, apparel); (2) **e-invoice/e-way-bill** provider-neutral GSP adapter + sandbox/mock (IRN lifecycle, signed QR, cancel/amend, idempotency, offline queue, feature-flag + kill switch) — disabled by default, live GSP creds are an externally-blocked verification item only; (3) **payroll** (India-ready, effective-dated, auditable); (4) then next highest-priority module; (5) UI/UX, accessibility, E2E, migration tooling, observability, simulated integrations wherever dependency-free. Work continues in small, individually-tested, PR-merged increments per hard rules #8/#9. Tasks #88–#91 opened. **WP1 category-policy engine + WP1b presets BUILT + WIRED & MERGED** (#183). WP1 (`packages/product/src/category-policy.ts` + `POST /v1/catalogue/category-policy/resolve`): category rules as effective-dated config — `resolvePolicy` (mirrors `mrpOn`/`resolveGstRate`), `CategoryPolicyRules` covering traceability/quantityMode/valuation/shelf-life/returns/controlled-sale/approvals/enabledByDefault, `categorySaleDecision` (composes with the till age gate, controlled verticals off until switched on, blocked categories short-circuit), `categoryReturnDecision`, `needsApproval`, `describePolicy`. **WP1b** (`packages/product/src/category-presets.ts`): the owner's categories A–G ship configurable, effective-dated default policies — grocery_fmcg, fresh_produce, perishable_packaged, gold_jewellery, otc_pharma_lite, prescription_blocked, cosmetics, electronics, apparel_footwear; gold + otc pharmacy-lite ship `enabledByDefault:false`, prescription/Schedule-H ships `controlledSale.blocked`; the resolve route accepts `kind` as an alternative to `history`. 25 unit + 5 integration total; RTM row `M03-FR-01·CAT-POLICY`; full gate green (5204 passed). **WP2 e-invoice/e-way-bill IN FLIGHT.** Recon: e-invoice (A20) was already built AND wired (`packages/e-invoice` engine — eligibility/build/apply/foldEInvoice/assessCancellation; `services/finance/src/e-invoice.ts` + `e-invoice-register.ts` submit→record-response→cancel→get lifecycle). **WP2 inc1 BUILT + WIRED (this session):** a deterministic **sandbox GSP** (`packages/e-invoice/src/sandbox-gsp.ts`) implementing the `EInvoiceProvider` port — `sandboxGspProvider` (hash-derived IRN, `SANDBOX.` QR, deterministic ack, forced unknown/rejected, real duplicate detection) + `registerViaProvider` closing register→`applyIrpResult` provider-neutrally; wired at `POST /v1/finance/e-invoice/sandbox/register` so the whole loop runs without live GSP creds. 8 unit + 3 integration; RTM A20 row updated. **WP2 inc2 (e-way-bill / A23) BUILT + WIRED (12 Aug):** a new `packages/e-way-bill` mirroring the e-invoice engine — `assessEwayBillRequirement` (inter-State >₹50k / **intra-TN >₹1L**, the two distinct thresholds as named constants, CA to confirm the TN figure), `ewayBillValidity` (Rule 138(10): 1 day/200 km), `buildEwayBillRequest` (Part-A, malformed refused), `applyEwbResult` (never fabricates the 12-digit EWB number, `unknown` first-class), `assessEwbCancellation` (24h), `foldEwayBill`, plus a deterministic `sandboxEwbProvider` + `generateViaProvider` closing build→generate→apply with no creds; wired `POST /v1/finance/e-way-bill/{eligibility,validity,sandbox/generate}`. 17 unit + 5 integration; RTM row A23; full gate green (5237 passed). **WP2 inc3 (GST-portal switch) BUILT + WIRED (12 Aug):** the feature flag + **kill switch** keeping LIVE e-invoice/e-way-bill portal calls off by default and killable — `assessGstPortalGate` (`packages/e-invoice/src/portal-switch.ts`): `enabled` (absent=off) + `killed` (overrides enabled); `requireGstPortalLive` throws `GstPortalDisabledError`; sandbox exempt; wired `POST /v1/finance/gst-portal/gate`; flags live in per-tenant config (M01-FR-03). 5 unit + 2 integration; RTM row; full gate green (5244 passed). **WP3 payroll inc1 BUILT + WIRED (12 Aug):** new `packages/payroll` — the India **statutory-deduction engine** (`statutory.ts`): effective-dated configurable rate tables (`DEFAULT_STATUTORY_SCHEDULE`, every value CONFIRM-WITH-CA) resolved on the pay date (`resolveStatutoryParams`, mirrors `resolveGstRate`); `computeStatutoryDeductions` = PF (capped ₹15k, nearest-rupee) + ESI (≤₹21k or covered-for-period, rounded up per ESIC) + apportioned PT → net; `professionalTaxTamilNadu` half-yearly slab; exact integer paise. Wired for review at `POST /v1/hr/payroll/{statutory-deductions,professional-tax-tn}`, gated the confidential owner-held `payroll.statutory.read`; commits nothing. 11 unit + 4 integration; RTM row; full gate green (5259 passed). **WP3 payroll inc2 BUILT + WIRED (12 Aug):** the **payslip builder** (`packages/payroll/src/payslip.ts`) that feeds inc1 — `resolveCompensation` (effective-dated structure, a raise never rewrites an earlier month), `buildPayslip` prorates every earning AND the PF wage by paid days (LOP reduces both), sums gross + PF wage, runs the tested statutory engine → earnings − deductions = net; `CompensationComponent.partOfPfWage`/`partOfGross`; wired `POST /v1/hr/payroll/payslip` (accepts `components` or `compensationHistory`), `confirmWithCa`. 6 unit + 4 integration; full gate green (5269 passed). **WP3 payroll inc3 (TDS) BUILT + WIRED (12 Aug):** the income-tax engine (`packages/payroll/src/tds.ts`) — `resolveTdsParams` (effective-dated) + `computeTds` (taxable = annual gross − standard deduction − old-regime declared deductions; regime slabs → gross tax; 87A rebate to nil within ceiling; 4% cess; balance after already-deducted spread over remaining months, never negative). New regime default, old regime honours declared deductions; `DEFAULT_TDS_SCHEDULE` = FY 2025-26, every value CONFIRM-WITH-CA. Folded into the payslip via `tdsMonthlyMinor` (backward compatible). Wired `POST /v1/hr/payroll/tds`. 7 unit + 3 integration; full gate green (5279 passed). **Payroll deduction side complete: PF + ESI + PT + TDS.** **WP3 inc4 (pay-run lifecycle) BUILT + WIRED (12 Aug):** the auditability backbone (`packages/payroll/src/pay-run.ts`) — append-only event-sourced state machine draft→submitted→approved→locked(+reversed); **maker ≠ checker** enforced in both `evaluatePayRunTransition` and `foldPayRun` (self-approval can't happen even via a crafted event stream); a locked run is final; correction = reversal + new run, never a rewrite; reject-to-draft; reversal needs a reason. Wired `POST /v1/hr/payroll/pay-run/evaluate`. 8 unit + 3 integration; full gate green (5290 passed). **WP3 inc5 (bank-transfer file) BUILT + WIRED (12 Aug):** `buildBankFile` (`packages/payroll/src/bank-file.ts`) turns a LOCKED pay run's net pay into the bank bulk-salary upload — refuses a non-locked run and any non-payable line (blank name/bad account/IFSC/non-positive/duplicate account), control total, CSV, `confirmWithBank`; wired `POST /v1/hr/payroll/bank-file`. 4 unit + 3 integration; full gate green (5297 passed). **WP3 inc6 (accounting journal) BUILT + WIRED (12 Aug):** `buildPayrollJournal` (`packages/payroll/src/journal.ts`) turns a LOCKED run's totals into a **balanced double-entry journal** (Dr salaries by cost-centre + employer PF/ESI; Cr net + PF/ESI/PT/TDS payables) with a **debits===credits invariant** that refuses an unbalanced run; wired `POST /v1/hr/payroll/journal`. 5 unit + 3 integration; full gate green (5305 passed). **WP3 inc7 (full-and-final settlement) BUILT + WIRED (13 Aug):** `computeSettlement` (`packages/payroll/src/settlement.ts`) itemises a leaver's earnings (pending part-month salary + leave encashment [capped days × per-day basic] + gratuity + other) minus recoveries (notice shortfall + advance/loan + other + tax) → a **signed net** (positive = payable to employee, negative = recoverable from them); `computeGratuity` = `15/26 × last-drawn basic+DA × completed years` for **≥5 years**, **capped ₹20,00,000**, via an effective-dated `resolveSettlementParams`/`DEFAULT_SETTLEMENT_SCHEDULE` (every value CONFIRM-WITH-CA); exact integer paise. Wired `POST /v1/hr/payroll/settlement`, gated `payroll.statutory.read`, `confirmWithCa: true`. 6 unit + 3 integration; full gate green (5316 passed). **WP3 inc8 (employee self-service / ESS) BUILT + WIRED (13 Aug):** the employee-facing side (`packages/payroll/src/ess.ts`) — an employee reads their OWN payslip (+ own settlement if leaving) and nothing else. Two separate controls: **self-scope** (`assertSelfScope`, refuses unless subject === caller; the route feeds the authenticated principal `ctx.userId` as requester → forge-proof; `employeeSelfView` also enforces it internally) and **redaction** (`employeeSelfView` returns only the employee's own earnings/deductions/net + employer cost shown separately as a company cost, never a deduction; no approver/cost-centre/bank/other-employee fields). Gated on a NEW narrow permission **`payroll.ess.self`** (granted to every employee role, NOT the confidential `payroll.statutory.read`). Wired `POST /v1/hr/payroll/ess/self` (subject must equal caller else 403). 7 unit + 4 integration; full gate green (5327 passed). **WP3 inc9 (durable pay-run store) BUILT + WIRED (13 Aug) — WP3 PAYROLL COMPLETE:** inc4's pure lifecycle fold is made durable by appending each step to the **shared append-only `event_ledger`** (one stream per run) via `payRunAdapter` (`services/api/src/adapters.ts`) + `payRunStoreRoutes` (`services/finance/src/pay-run-store.ts`) — `load` folds the stored stream, `append` writes one idempotent fact — so a run **survives a restart**. **No new table/migration:** durability rides the existing `SqlEventStore` exactly as the e-invoice register / GST-returns stores do (the "everything is an event" pattern; my earlier note about a bespoke SQL table was superseded once the codebase's shared-ledger architecture was confirmed — a parallel table would have violated hard rule #2's single append-only truth). Routes `POST /v1/hr/payroll/pay-run/:payRunId/append` (**maker ≠ checker enforced at the write boundary** — a self-approval is refused 422 before any append, and the fold refuses it again on read) + `GET /v1/hr/payroll/pay-run/:payRunId` (fold → current); stateless `…/pay-run/evaluate` retained. Gated `payroll.statutory.read`. 5 unit + 4 integration; full gate green (5336 passed). **Payroll now spans compute → payslip → deductions (incl. TDS) → approve/lock lifecycle → bank file → balanced accounting journal → full-and-final settlement → employee self-service → DURABLE pay-run store — the full employee lifecycle, both HR/finance and employee sides, now persistent across restarts (still preview/review; live GO is CA/HR/legal-gated). WP3 PAYROLL is DONE.** **WP2 remaining (deferred refinements):** enforce the portal gate on the live submit path, reconciliation of stuck-`unknown` documents. Externally-blocked GO items: live GSP/portal creds, CA-confirmed intra-TN e-way-bill threshold, and CA/HR/legal payroll sign-off.)

**WRITE-OFF WRITE PATH NOW ON THE LIVE API BUILT + WIRED 14 Aug (M28-FR-01).** Chosen per the check-in's escape hatch: the GST-returns governance online-first rework (option A) needs a foundational web-erp authenticated direct-POST client that does not exist yet (payroll's "online-first" is gating-only; the POST is deferred), so the online-first rework is larger/ambiguous than a single increment and stays flagged for the owner (see the security-finding block above). Instead I closed a real correctness gap on option B's territory: the tested M28 `commitWriteOff` engine (`packages/waste/src/waste.ts`, which reuses the M08 `commitAdjustment` §28 check) had **no live surface** — a loss could be *reviewed* (yesterday's offline waste screen) but never *committed* through the product. This wires the write path, mirroring the counts precedent (`services/inventory/src/counts.ts`): **`POST /v1/inventory/write-off/:writeOffId`** runs the engine over an ephemeral ledger + outbox and persists the committed write-off on its own append-only stream (`writeOffAdapter` folds `WriteOffCommitted`, idempotent append — hard rule #2, a correction is a new id never an overwrite). **The raiser is the AUTHENTICATED caller (`ctx.userId`), never a body value** — so this is a direct-POST authenticated write with **no relayed-actor ambiguity** (deliberately unlike the deferred synced-governance route that triggered the security finding; here the person acting IS the person authenticated). **§28 enforced at the boundary:** a **material** loss (value ≥ the tenant threshold) needs BOTH captured **evidence** (else 422 `write_off_needs_evidence`) AND a **separate approver** — the raiser naming themselves is refused (422 `write_off_needs_approval`); an immaterial loss commits with neither. Missing reason / non-positive qty → 400; a re-used id → 409 (append-only). Gated `inventory.movement.append` to write; the review list **`GET /v1/inventory/write-offs`** (with the day loss total) gated `waste.view` (owner + store_manager; default-deny — a cashier is refused both). Deterministic — no AI writes it (hard rule #5). 6 integration tests (immaterial commit, material-without-evidence 422, material-without/self-approval 422, material-with-evidence-and-different-approver 201, malformed 400 + idempotent 409, gated list); full gate green: typecheck, lint, **5600 unit/guardrail/integration + 12 e2e**. **Completion % unchanged at 40.3%** (`pnpm run completion`: previous 40.3%, change +0.0 pts; responsible IDs — M28 advances write-off from engine-only to WIRED but the whole M28 module stays PARTIALLY_WIRED as its other FRs — scrap/packaging/sustainability — are unchanged, so no module-level maturity threshold is crossed; reported honestly, not inflated). **NEXT: the next dependency-priority incomplete roadmap module** — OR, for the owner: the flagged GST-returns governance online-first rework (needs the web-erp direct-POST client foundation first). The offline capture screen (record a write-off from the shop floor, online-first with the raiser's own token, feeding this route) is the natural follow-on to this write path. Continuing automatically per the owner directive.

**STOCK-COUNT REVIEW SCREEN BUILT + WIRED 14 Aug (M09-FR-04).** The operator review screen for the blind-count reconciliation. Its write path (`services/inventory/src/counts.ts` — the very precedent the waste write-off was copied from) was already live but had NO operator surface, and a `/counts` nav item dangled pointing at nothing. This is the symmetric next step after the M28 waste pair (review screen + write path) shipped: the counts write path existed, so this adds its review screen. A first-class OFFLINE screen (like waste review): the box serves `/counts`, injects `window.countsData` (reconciled blind counts folded to review rows + who is looking) at the `<!--SCREEN-DATA-->` marker; the SW precaches the shell (CACHE v13). **All rules in the tested DOM-free session model** (`createCountsReviewSession`) on the tested `packages/counts` reconciliation: every reconciled count with a triage status that is never a bare colour (icon + word + announcement via `presentStatus`) — **Matched** (ok), **Short** (▼ shrinkage) / **Over** (▲ found stock) both degraded — with the signed variance, the value at variance (₹), and the counted-vs-system quantities. **The MATERIAL variances (needed a §28 approval) come first**, bumped to error/attention — shrinkage investigated by exception (P-03), not buried in a total; the view totals the day's value at variance. **Read-only** — reconciling a count is the audited write path; the expected quantity is computed **server-side** and the blind count is captured elsewhere, so blind-count integrity is structural and cannot be weakened by this surface (a guardrail asserts no fetch/XHR/enqueue). **Read-gated** the new permission `count.view` (owner + store_manager; default-deny), which the already-declared `/counts` nav item required but was undefined until now. English + Tamil from ONE `BilingualCopy`; no alert/confirm/prompt. New pack sections `countsQueue` + `countsPolicy` (honest absence → sample stand-in, never an empty list reading as "every count matched"). The dangling `/counts` nav item now resolves to a real screen. **Real-browser offline + a11y E2E** (headless Chromium). 11 unit + 9 guardrail; full gate green (**5620** unit/guardrail/integration + 13 e2e); ERP bundle rebuilt. Advances M09 count/adjustment from write-path-only to a wired operator review surface. **Completion % unchanged at 40.3%** (`pnpm run completion`: previous 40.3%, change +0.0 pts; responsible IDs — M09 deepens within PARTIALLY_WIRED, no module-level maturity threshold crossed — reported honestly, not inflated).

**Why this, not the flagged GST-returns online-first rework — a firmer diagnosis for the owner.** The last check-in preferred (A) reworking the GST-returns approve/submit buttons to be online-first with the operator's own live session token, *if* well-scoped. **On investigation it is NOT a small increment — it is a large new security foundation, and it needs an owner/deployment decision before any code.** Two independent findings: (1) **the browser holds no credential today** — every web-erp screen is served *pre-authenticated* by the store-edge box, which injects `window.*Data` (who you are + what you may see) on a **loopback-only** origin whose "entire security control is the bind address"; nothing in the browser could present a token to the API. (2) **There is no login or token issuance anywhere** — the identity service (`services/identity/src/token.ts`) is **verify-only by explicit design** ("it verifies; it never issues… until an identity provider is configured, nobody can log in"), the API authenticates a **signed bearer JWT** shaped for server-to-server callers (the sync-agent's per-store token), and payroll's "online-first" is UI-gating with a **client-side in-memory MFA stub** that calls no server. So a browser-held direct-POST client would require building, essentially from scratch: an **IdP/OIDC login flow + token issuance**, secure **browser token storage**, **CSRF** defenses for any cookie variant, and **real server-calling MFA** — a foundational epic partly in tension with hard rule #4 (no stored credentials in-repo). **This is a decision for the owner** (which deployment IdP; whether to introduce browser-held credentials at all), not something to build autonomously inside one increment. Until that foundation exists, the shipped GST-returns approve/submit commands remain on the **offline outbox and dead-letter honestly** with no unsafe consumer (P-08; the operator sees "could not be sent — a person should check"), exactly as recorded in the security-finding block above. **Recorded here for the owner to decide; not rushed.** Per the check-in's escape hatch ("if the online-first rework is larger/ambiguous than expected, do (B) and leave (A) flagged"), I built (B): the counts review screen, a clean, proven, well-scoped increment. **NEXT: the next dependency-priority incomplete roadmap module** (leading candidates with tested engines and no surface: M25 workforce roster/tasks/incentives; M31 documents issue/reproduce; M18-FR-04 substitution/refunds awaiting the finance surface). Continuing automatically per the owner directive.

**WORKFORCE ROSTER-GAP DETECTION NOW ON THE LIVE API BUILT + WIRED 14 Aug (M25-FR-01).** The M25 workforce engine (`packages/workforce/src/workforce.ts`) is a big tested engine (rosterGaps, canPerformTask, assessChecklist, computeIncentive, labourCost, sopStatus) that had **no cloud/API surface at all** — its only wiring was the edge-side checklist fold into the manager pack. This adds its first live route, chosen after investigation as the cleanest of the three standing candidates (M25 workforce / M31 documents-issue / M18-FR-04 refunds): the workforce functions are pure DECISIONS (no ledger, no events), so a **stateless what-if route** fits exactly — the payroll statutory-review pattern — with no new read-model or pack section to design (unlike a screen), and no renderer/finance-surface dependency (unlike M31/M18). `POST /v1/hr/workforce/roster-gaps` runs the tested `rosterGaps` over a caller-supplied proposed roster (shifts + assignments + employees) and returns **what is actually missing — per role per shift, with the hour** ("Sunday 06:00 has NOBODY rostered as opener"), never averaged into a "14 of 16 covered" figure nobody acts on. It surfaces a separate **unstaffed** count (roles with nobody rostered at all — the M25-FR-01 unstaffed-critical exception, seen by exception per P-03) distinct from a merely-short shift, and **a leaver still on the grid is not cover** (inactive employees are filtered before counting). Stateless — commits nothing, stores nothing (the durable roster/attendance store is a later increment); co-located with payroll under `/v1/hr`; manager-gated on a new `workforce.roster.read` (owner + store_manager; default-deny — a cashier is refused). Malformed roster → 400. 6 integration tests via the api-harness; full gate green (typecheck, lint, **5626** unit/guardrail/integration + 13 e2e). **Completion % unchanged at 40.3%** (`pnpm run completion`: previous 40.3%, change +0.0 pts; responsible IDs — M25 already counted PARTIALLY_WIRED via the edge checklist fold, so this first cloud route deepens it within PARTIALLY_WIRED without crossing a module threshold — reported honestly, not inflated). **NEXT: the next dependency-priority incomplete roadmap module** — the remaining workforce FRs (certification-gated tasks, checklist assessment, incentives, SOP acknowledgement) are natural follow-ons on this same surface; or M31 documents issue/reproduce (check the renderer scope first); or M18-FR-04 substitution/refunds (awaits the finance/refund surface). **STILL FLAGGED for the owner (unchanged):** the GST-returns governance online-first rework needs a browser auth foundation (deployment IdP + login + token issuance + browser token storage + CSRF + real MFA) — a large security foundation and an owner/IdP decision, not a small increment. Continuing automatically per the owner directive.

**WORKFORCE TASK CERTIFICATION-GATING NOW ON THE LIVE API BUILT + WIRED 14 Aug (M25-FR-03).** The second live route on the workforce surface (roster-gaps was the first), on the same proven stateless pattern. `POST /v1/hr/workforce/task-gate` runs the tested `canPerformTask` to decide whether a person may do a **gated task** today, given their certifications — the M25-FR-03 acceptance ("an expired certification blocks the gated task"). **The gate is on the TASK, never the person** (the engine's governing principle): someone whose food-handling certificate lapsed cannot work the deli counter but can still stack shelves, and the decision carries `stillAllowed` saying so — because a control people route around on a busy Saturday is not a control. An **expired**, a **missing**, or an **unverified** certificate all block the task (an unverified cert is never treated as cover); a task whose required role the person lacks blocks it (`not_assigned`); a leaver is blocked outright (`inactive`). Age-restricted / food-handling sales are exactly the gated tasks this protects (composes with the till's COTPA/age gate). Stateless what-if — the caller supplies the person + certs, the route commits and stores NOTHING (the durable certification register is a later increment); co-located with the roster-gaps route under `/v1/hr`. Manager-gated on a new `workforce.task.read` (owner + store_manager; default-deny — a cashier is refused). Malformed → 400. 6 integration tests (12 total on the workforce surface); full gate green (typecheck, lint, **5632** unit/guardrail/integration + 13 e2e). **Completion % unchanged at 40.3%** (`pnpm run completion`: previous 40.3%, change +0.0 pts; responsible IDs — M25 deepens within PARTIALLY_WIRED, roster-gaps + certification-gate now both wired, no module threshold crossed — reported honestly, not inflated). **NEXT: the next dependency-priority incomplete roadmap module** — the remaining workforce FRs on this same surface (M25-FR-02 checklist assessment `assessChecklist`, M25-FR-03 incentives `computeIncentive`, M25-FR-04 SOP acknowledgement `sopStatus`), or an offline workforce roster/tasks review screen; or M31 documents issue/reproduce (check the renderer scope first); or M18-FR-04 substitution/refunds (awaits the finance/refund surface). **STILL FLAGGED for the owner (unchanged):** the GST-returns governance online-first rework needs a browser auth foundation (deployment IdP + login + token issuance + browser token storage + CSRF + real MFA) — a large security foundation and an owner/IdP decision. Continuing automatically per the owner directive.

**WORKFORCE DECISION SURFACE FINISHED — checklist / incentive / SOP-ack now on the live API BUILT + WIRED 14 Aug (M25-FR-02/03/04).** The last three workforce DECISION functions get live routes, completing the M25 read/decision surface (roster-gaps + task-gate were the first two). Bundled as one coherent "finish the surface" increment rather than three near-duplicate PRs (each is the same trivial stateless pattern). Three routes: **`POST /v1/hr/workforce/checklist-assess`** (M25-FR-02, `assessChecklist`) — opening/closing/handover checklist where BLOCKING and non-blocking items are SEPARATED (a blocking item outstanding stops the shop; a signed checklist with only non-blocking items left is complete and carries them into the handover; an unsigned one is "a list, not a record"), gated `workforce.checklist.read`; **`POST /v1/hr/workforce/incentive`** (M25-FR-03, `computeIncentive`) — the EXACT integer payout (§29.1) where a MISSED target pays NOTHING not a proportion ("nearly" is a conversation, not a formula), with a BigInt accelerator on the excess + optional cap, gated `workforce.incentive.read`; **`POST /v1/hr/workforce/sop-status`** (M25-FR-04, `sopStatus`) — who has acknowledged the CURRENT version of each SOP for their role (acknowledging v3 is not acknowledging v5; other-role SOPs filtered; the outstanding count surfaced), gated `workforce.sop.read`. Each **stateless** — the caller supplies the inputs, the routes commit and store NOTHING (the durable checklist/incentive/SOP stores are later increments); each on its own manager-held permission (owner + store_manager; default-deny). Malformed → 400 with a route-specific code. 10 integration tests (22 total on the workforce surface); full gate green (typecheck, lint, **5642** unit/guardrail/integration + 13 e2e). **The workforce read/decision surface is now complete** — roster gaps, certification-gated tasks, checklists, incentives and SOP acknowledgement all on the live API; only the durable stores and an operator screen remain. **Completion % unchanged at 40.3%** (`pnpm run completion`: previous 40.3%, change +0.0 pts; responsible IDs — M25 deepens within PARTIALLY_WIRED, no module threshold crossed — reported honestly, not inflated). **NEXT: a NEW dependency-priority module** (the workforce decision surface is done — further workforce work would be the durable stores or an operator screen). Leading candidates: M31 documents issue/reproduce (check the renderer scope first); M18-FR-04 substitution/refunds (awaits the finance/refund surface); or an offline workforce roster/tasks review SCREEN on the now-complete decision surface. **STILL FLAGGED for the owner (unchanged):** the GST-returns governance online-first rework needs a browser auth foundation (deployment IdP + login + token issuance + browser token storage + CSRF + real MFA) — a large security foundation and an owner/IdP decision. Continuing automatically per the owner directive.

**ORDER SUBSTITUTION WRITE PATH NOW ON THE LIVE API BUILT + WIRED 14 Aug (M18-FR-04).** A NEW module (deliberately off the workforce surface, which is decision-complete). Chosen after investigation: a workforce review screen would be premature (the workforce routes are all stateless what-ifs — no stored roster/SOP data to fold into a pack, so it'd be sample-only), M31 documents issue/reproduce carries a renderer dependency, so M18-FR-04 substitution was the cleanest genuinely-new increment. The tested `applySubstitution` (`packages/orders/src/amendments.ts`) had no live surface; this wires it as a WRITE PATH mirroring the order lifecycle/cancel precedent in the same service. `POST /v1/orders/:orderId/substitute` runs the engine and RECORDS the picker's decision append-only on the order stream (`LineSubstituted` via `ordersAdapter`, idempotent on order+line — hard rule #2). The three safety properties, now live: **`no_answer` is NOT a yes** (silence short-picks, charges nothing — treating silence as consent is how somebody gets a product they cannot eat); **the customer never pays more for our shortfall** (a dearer substitute is charged at the ORIGINAL line price); and a **cheaper substitute leaves a refund DUE** — recorded as a fact (`refundDue`) and ISSUED downstream by the finance/refund surface, never kept here (P-08 — the money owed is visible, not silently dropped). A line is substituted once (a re-decision → 409); a substitution on a finished order (via the exported `isTerminal`) → 409; an unknown order → 404. Gated `order.lifecycle.manage` (the pick/pack authority; a cashier is refused). 7 integration tests; full gate green (typecheck, lint, **5649** unit/guardrail/integration + 13 e2e). **Completion % unchanged at 40.3%** (`pnpm run completion`: previous 40.3%, change +0.0 pts; responsible IDs — M18 already PARTIALLY_WIRED, cancel + substitution now both wired, deepens within it, no module threshold crossed — reported honestly, not inflated). **NEXT: the next dependency-priority incomplete module** — M18's `reconcileChannel` (channel-vs-ledger two-way reconciliation, self-contained stateless route) is a clean follow-on; returns/refunds processing awaits the finance/refund surface; M31 documents issue/reproduce needs a renderer decision; or re-scan for the next-highest-value engine-only FR. **STILL FLAGGED for the owner (unchanged):** the GST-returns governance online-first rework needs a browser auth foundation (deployment IdP + login + token issuance + browser token storage + CSRF + real MFA) — a large security foundation and an owner/IdP decision. Continuing automatically per the owner directive.

**CHANNEL RECONCILIATION NOW ON THE LIVE API — M18-FR-04 AMENDMENTS TRIO COMPLETE BUILT + WIRED 14 Aug.** The last engine-only piece of the M18-FR-04 amendments trio (cancel + substitution already wired) gets a live route — the clean follow-on the check-in flagged. `POST /v1/orders/channel-reconcile` runs the tested `reconcileChannel` **in both directions**, because a one-way check misses half the problem: an order the **channel** has and we do **not** is revenue we never fulfilled — a customer waiting for something nobody is picking (`in_channel_not_in_ledger`); an order **we** have and the channel does not is a **phantom** that will never be paid for (`in_ledger_not_in_channel`); and a matched order with a different **value** (`value_mismatch`) or **state** (`state_mismatch`) is surfaced too, with the total **at-risk value**. **Stateless** — the caller supplies both order lists (a marketplace export vs our own ledger export), the route commits and stores NOTHING (one commerce truth P-02, made checkable across channels; nothing at-risk silently dropped P-08). Gated `order.read` (owner + store_manager; a cashier is refused). Malformed → 400. 5 integration tests; full gate green (typecheck, lint, **5654** unit/guardrail/integration + 13 e2e). **Completion % unchanged at 40.3%** (`pnpm run completion`: previous 40.3%, change +0.0 pts; responsible IDs — M18 deepens within PARTIALLY_WIRED, the amendments trio now fully wired, no module threshold crossed — reported honestly, not inflated). **NEXT: the next dependency-priority incomplete module** — M18-FR-04 is now amendments-complete (only returns/refunds PROCESSING remains, awaiting the finance/refund surface). Candidates: M31 documents issue/reproduce (check the renderer scope first); or re-scan docs/traceability.md for the next-highest-value engine-only / PARTIALLY_WIRED FR with a tested engine and no live surface. **STILL FLAGGED for the owner (unchanged):** the GST-returns governance online-first rework needs a browser auth foundation (deployment IdP + login + token issuance + browser token storage + CSRF + real MFA) — a large security foundation and an owner/IdP decision. Continuing automatically per the owner directive.

**DOCUMENT ISSUE/REPRODUCE NOW ON THE LIVE API BUILT + WIRED 14 Aug (M31-FR-01/02).** The deliberate follow-up the documents service header explicitly named ("issuing/reproducing renders content and needs a concrete renderer; that is a deliberate follow-up"). Chosen after investigation: the M31 candidate turned out well-scoped, NOT renderer-blocked — `issueDocument` takes a **supplied `render` function** (the engine is layout-agnostic), so no PDF engine is needed. This surface supplies a **deterministic `{{field}}` text renderer** (an unknown field is left as its literal placeholder so a missing value is VISIBLE, never a silent blank), and the engine freezes whatever it returns — a real PDF/HTML renderer stays a production concern. `POST /v1/documents/templates/:id/issue` renders the content NOW under the version in force and **FREEZES it** (the document is the bytes, not a recipe to re-render), the issuer being the **AUTHENTICATED caller** (`ctx.userId`, never a body value); **idempotent on the document id** (a re-issue returns the SAME frozen bytes, never a second copy under a later version — hard rule #6); refuses `no_template` / `render_failed` (422); malformed → 400. `GET /v1/documents/issued/:documentId` runs `reproduceDocument` — the **EXACT frozen bytes**, never re-rendered from the current template, so **July's invoice still reads as July's after August's template change** (proved by test — after a v2 publish, reproduce still returns v1's content). Event-sourced (`DocumentIssued` folded by `documentsAdapter`); issue gated a new `document.issue` (owner + store_manager), reproduce `document.template.read`; a cashier is refused both. 5 integration tests; full gate green (typecheck, lint, **5659** unit/guardrail/integration + 13 e2e). **Completion % unchanged at 40.3%** (`pnpm run completion`: previous 40.3%, change +0.0 pts; responsible IDs — M31 already PARTIALLY_WIRED, template register + issue/reproduce now both wired, deepens within it, no module threshold crossed — reported honestly, not inflated). **NEXT: the next dependency-priority incomplete module** — re-scan docs/traceability.md for the next-highest-value engine-only / PARTIALLY_WIRED FR with a tested engine and no live surface (hunting grounds: M11 production yield/costing/packing/quality-release; M24 refusal-audit/probe-pattern; M05 abuse-caps/vendor-funding; M08 stockout-impact; M36-FR-03 tenant closure/retention). **STILL FLAGGED for the owner (unchanged):** the GST-returns governance online-first rework needs a browser auth foundation (deployment IdP + login + token issuance + browser token storage + CSRF + real MFA) — a large security foundation and an owner/IdP decision. Continuing automatically per the owner directive.

---

## PRODUCT COMPLETION — reproducible weighted model (baseline v1, 13 Aug 2026)

_Computed by `pnpm run completion` from `docs/completion-status.json`; model + governance in
`docs/COMPLETION-MODEL.md`; arithmetic locked by `tests/guardrails/completion-model-integrity.test.ts`.
No estimates, no ranges — same ledger → same number._

- **Denominator (total controlling requirements): 104** — roadmap v2.0/v2.1 top-level structure: M01–M36
  (36) + D01–D14 (14) + A01–A10 (10) + WF-01..20 (20) + QG-01..12 (12) + MG-01..12 (12).
- **Numerator (weighted maturity points): 4195 of 10400.**
- **PRODUCT COMPLETION: 40.3%** — **previous 40.3%** (baseline first run), **change +0.0 pts**. This
  increment (the reporting model itself) advances no product maturity; **requirement IDs responsible for
  the baseline value:** all 104, dominated by the 78 PARTIALLY_WIRED modules/extensions/workflows and the
  POS core sale path M12 / WF-10 (the only E2E-verified requirements).

**Six separate scores:**
1. Requirements / design completeness — **99.0%** (103/104 past NOT_STARTED)
2. Technical implementation (the weighted headline) — **40.3%**
3. Wired-and-integrated (≥ WIRED) — **11.5%** (12/104)
4. E2E verification (≥ E2E_VERIFIED) — **1.9%** (2/104: M12, WF-10)
5. UAT readiness (≥ UAT_VERIFIED) — **0.0%**
6. Production readiness (PRODUCTION_VERIFIED) — **0.0%**

**Exact counts per maturity category:** NOT_STARTED 1 · ENGINE_ONLY 13 · PARTIALLY_WIRED 78 · WIRED 7 ·
INTEGRATION_TESTED 3 · E2E_VERIFIED 2 · UAT_VERIFIED 0 · PRODUCTION_VERIFIED 0.

**Externally blocked (technical score retained, blocker shown separately): 2** — M23 finance
(live GST-return filing + e-invoice/e-way-bill live portal need production credentials + CA/legal sign-off)
and QG-06 security (independent penetration test not yet performed). These retain their achieved
PARTIALLY_WIRED weight; the blocker does not deflate the score.

**Governance:** the denominator (104) and the weights are fixed; a change needs a documented, owner-approved
baseline revision (bump `baseline.version`). The guardrail fails if the id set drifts, so no silent change.
Every future STATUS update copies the numbers from `pnpm run completion` and shows the change + responsible
requirement IDs — this is the single reproducible completion model that supersedes all prior estimate
ranges (e.g. the "≈22–30% wired" phrasing below is superseded by score 3 = 11.5% wired-and-integrated at
this stricter, per-requirement granularity).
