...
Last updated: 11 August 2026 (**★ BATCH-ON-SALE inc2 — THE FEFO AUTO-ASSIGN BRAIN ★** (M10-FR-03; owner ratified **FEFO auto-assign**, zero cashier effort): `assignBatchesFefo` (`packages/fefo/src/assign-batches.ts`) is the deterministic engine a lane runs **offline** to tag each batch-tracked line with its **earliest-expiry** batch(es) — First-Expiry-First-Out, reusing the tested `allocateFefo`. It DECREMENTS stock across lines so two lines of the same product never claim the same units, skips expired/recalled/quarantined batches, and where the batch records are behind the shelf it reports the shortfall as `untracedQty` (the sale still stands — hard rule #1 — but the untraceable units are flagged, P-08, the same honesty as inc1's cloud finding). Pure, no clock (caller passes asOf), no I/O. **7 unit tests** (earliest-first, spanning batches, cross-line decrement, the untraced remainder, non-tracked passthrough, whole-basket order, input refusals). Full gate green (typecheck, lint, secret-scan, traceability-integrity, one-surface, full suite). **The offline commit path is STILL untouched** — this is the pure brain; **inc3 wires it**: the edge commit calls `assignBatchesFefo`, threads the allocations through the local commit + sync onto the `IncomingSale`, and the cloud folds stored sale lines into the lot-trace outbound (so B11's recall trace pulls live sales and A5 GST returns can draw on real data). Prior: batch-on-sale inc1 (#154), B11 (#153), B19 (#152), A7 (#151), B14 (#150).)
