...
Last updated: 11 August 2026 (**★ EINV inc1 — GST E-INVOICING (A20) DETERMINISTIC CORE ★**: the owner chose to build e-invoicing next — mandatory now because the ratified turnover is ₹5–10 cr (> ₹5 cr). New package `packages/e-invoice` + `services/finance/src/e-invoice.ts` (API-09), on the live API. Three stateless steps: `POST /v1/finance/e-invoice/eligibility` decides if an IRN is mandatory (gated on the SAME ₹5 cr threshold as the HSN rule A4 — imported not restated so they cannot drift; B2C and unregistered-buyer supplies excluded); `…/build` assembles the canonical IRP request from a Rule-46-VALID invoice only (a malformed one is refused, not burned at the portal) and returns the IRP idempotency key (GSTIN|docType|number|FY); `…/apply-irp-result` turns whatever the IRP answered into the record to store and **NEVER fabricates the government's signature** — a malformed IRN or empty QR is a `provider_error` not a stored registration (the e-invoice equivalent of 'never store a card number'), and an `unknown` answer is `pending_unknown` (the invoice is not e-invoiced until the IRP confirms — the same discipline as the payment-reversal `uncertain` state). Gated new permission `finance.einvoice.generate`. 8 unit (`tests/unit/e-invoice.test.ts`) + 4 integration (`tests/integration/e-invoice.test.ts`) assertions. Full gate green (typecheck, lint, secret-scan, **4,914 tests / 361 files**). Test-only packages 5 of 78 (new e-invoice package wired from day one). **NEXT (EINV inc2): the LIVE IRP/GSP submission — the network call + credentials-from-vault + event-sourced IRN store + the real `EInvoiceProvider` adapter (a certified GSP), the same shape as the payment-reversal provider (a deployment step needing GSP credentials).** A21 30-day IRN stays N/A (< ₹10 cr). Prior: CORE-05 complete (#135–#144), owner decisions ratified (#145).)
