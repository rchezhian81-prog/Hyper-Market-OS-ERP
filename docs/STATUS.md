...
Last updated: 10 August 2026 (**★ R2 v2.1 A10 — NEAREST-RUPEE ROUNDING PER TAX COMPONENT, ON THE CLOUD ★**: the second ratified-R2 increment, composing directly with A9/A8 (#119). `roundToNearestRupeeMinor` rounds a paisa amount to the nearest whole rupee — **≥50 paisa UP, <50 DOWN** — and `roundToNearestRupee` applies it to the taxable value and EACH tax component of an extracted breakdown, stating the resulting round-off **explicitly** (P-08 — the invoice 'Round Off' line, which may be ±), never hiding a rounding that moves money. The exact paisa breakdown (A9) is left untouched and still reconciles to the MRP; the whole-rupee view is served ALONGSIDE it as `nearestRupee` on the same `GET /v1/finance/tax/from-mrp`, so an invoice can print either without re-deriving the rounding. `tests/unit/finance-inclusive-tax.test.ts` proves the exact ≥50p-up/<50p-down boundary and the per-component rounding (₹9.99 @ 18% intra → exact 847/76/77 → rounded 800/100/100, round-off +₹0.01); `tests/integration/tax-from-mrp.test.ts` proves the route serves it. Full gate green (typecheck, lint, secret-scan, **4,765 tests / 332 files**). No new dependency, no new permission. Prior context unchanged: the CORE-04 pure-read seam is complete (5 increments, #114-#118) and confirmed exhausted; work now advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
