...
done for Store-Core (R2); Stage 5 has built 12 tested foundation units (111 tests). **D3/D4/D5/D8 were answered on 2 Aug 2026** (see
...
- **Stage 5 (foundation) — BEGUN** (`packages/contracts/`, all with tests; full `pnpm
  ...
  - `pnpm check` green: typecheck + lint + secret-scan + **111 tests**. Value-object
  ...
  - **Foundation engines now cover the core invariants** (exact money/quantity, append-only
    ...
    rule) plus the first composition (line pricing) — 12 tested units, 111 tests.
...
## In progress
- **Stage 5 foundation build** — 12 tested units done (`packages/` contracts, ledger,
  approvals, rbac, sync, numbering, calendar, pricing; 111 tests, `pnpm check` green). The
