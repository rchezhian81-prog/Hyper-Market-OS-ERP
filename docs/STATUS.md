...
Last updated: 10 August 2026 (**★ R2 v2.1 B3 — UNIT SALE PRICE ON THE LABEL, ON THE CLOUD ★**: the eighth ratified-R2 increment and the first OUTSIDE the tax cluster — a Legal Metrology / Packaged-Commodities requirement (M03/catalogue). `unitSalePrice` computes the ₹-per-standard-unit a pre-packed label must show from the MRP + net quantity — grams normalised to a KILOGRAM, millilitres to a LITRE, count to a PIECE (a 500 g pack at ₹100 → ₹200/kg), rounded to the paisa so a shopper can compare a 480 g pack against a 500 g one without arithmetic. The two exemptions the rules allow — a very small package (principal display panel ≤ 100 cm²) and a low-value item (≤ ₹35) — are SURFACED (still computed, flagged display-not-mandatory), never silently omitted (P-08). It runs on `GET /v1/catalogue/unit-price` (a stateless labelling route in the catalogue service), gated `catalogue.pack.read` (owner/manager/cashier — a label reads the catalogue, not the accounts, so NOT a finance permission). `tests/unit/product-unit-price.test.ts` proves the g→kg / ml→l normalisation, the rounding, and BOTH exemptions; `tests/integration/catalogue-unit-price.test.ts` proves the route + the RBAC (the accountant, lacking `catalogue.pack.read`, is refused 403). Full gate green (typecheck, lint, secret-scan, **4,813 tests / 339 files**). No new dependency, no new permission. R2 progress: A1/A4/A6/A8/A9/A10/A11/A12 (tax-invoice suite) + B3 (labelling) = 8 ratified requirements built. Context unchanged: CORE-04 pure-read seam complete (5 increments) and exhausted; work advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
