...
Last updated: 10 August 2026 (**★ R2 v2.1 B9 — LOOSE-FOOD ALLERGEN + VEG/NON-VEG LABEL, ON THE CLOUD ★**: the tenth ratified-R2 increment, extending the labelling work (B3/B24) into food safety. Food packed at the counter still carries statutory declarations, and two are non-negotiable under the FSSAI Labelling & Display Regulations 2020: the major-allergen information and the veg/non-veg symbol. `checkLooseFoodLabel` requires BOTH — a veg/non-veg mark AND an allergen declaration (a list of the groups present, or an explicit 'none' — silence is not a declaration) — and rejects an allergen outside the 8 FSSAI major groups (`MAJOR_ALLERGENS`). A missing/malformed declaration BLOCKS the label; an undeclared allergen on prepared food is a safety failure, not a formatting one. Runs on `GET /v1/catalogue/loose-food-label?vegClass=&allergenDeclarationProvided=&allergensPresent=` (the labelling route in the catalogue service), gated `catalogue.pack.read`. `tests/unit/product-loose-food-label.test.ts` proves each missing declaration blocks + the unknown-allergen rejection; `tests/integration/catalogue-unit-price.test.ts` proves the route + RBAC. Full gate green (typecheck, lint, secret-scan, **4,825 tests / 341 files**). No new dependency, no new permission. R2 progress: A1/A4/A6/A8/A9/A10/A11/A12 (tax-invoice suite) + B3/B24/B9 (labelling) = 10 ratified requirements built. Context unchanged: CORE-04 pure-read seam complete (5 increments) and exhausted; work advances the owner-ratified R2 scope (#106), holding the gated items (turnover-triggered e-invoicing, chart of accounts, payroll ownership, product-line decisions, the ten owner decisions) for pre-deployment.)
