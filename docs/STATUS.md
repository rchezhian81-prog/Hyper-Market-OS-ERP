...
Last updated: 11 August 2026 (**★ R2-B14 — TOBACCO AGE-18 GATE + NO LOOSE SINGLE-STICK (COTPA 2003) ★**: the till now has a hard restricted-sale gate, unlocked by the owner's ratified decision (11 Aug) that tobacco is stocked. `POST /v1/pos/restricted-sale/check` (API-05, `checkRestrictedSale` in `packages/restricted-sales/src/restricted-sales.ts`) rules a basket before it commits: an **age-restricted line cannot clear** until the customer's age is confirmed **at or above** the store's minimum (18 by default, OB-03) — no confirmation, or a confirmed age below it, BLOCKS the line (hard, not a warning); and a **tobacco line below one whole pack** is refused (COTPA s.7 bars the loose single cigarette). It names EVERY failing line at once, commits nothing (a decision, not a write), and reads the flags from the **offline price pack** so it works with the internet down (hard rule #1). Gated `pos.restricted.check`, held by owner/store-manager AND the **cashier** who works the lane. **5 unit + 4 integration** (18+ clear, two age refusals, loose-stick refusal, both-at-once, configured minimum age, malformed, RBAC). Full gate green (typecheck, lint, secret-scan, traceability-integrity, one-surface, full suite 4944 passing). `packages/restricted-sales` is a new package wired the same increment — never test-only (now 79 packages, same 5 unwired-by-design). OWNER CHECK in the store: put a real cigarette pack through a lane — it must demand the age-18 confirmation, and a below-pack quantity must be refused. Note: **liquor is NOT sold** (TASMAC state monopoly), so this gate is tobacco-only today; the engine already carries `liquor`/`other_age_restricted` kinds for the day a licensed line appears. Prior: WRITE-PATH inc1-3 (A5 end-to-end), EINV inc1+inc2, CORE-05 complete.)
