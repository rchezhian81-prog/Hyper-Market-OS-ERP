...
Last updated: 12 August 2026 (**★ D-1 inc2 — THE FORECAST NOW KNOWS ABOUT FESTIVALS, PROMOTIONS & WEATHER ★** (M09): the weekly-pattern forecast learns nothing about a one-off Diwali or a promo; this teaches it. `forecastDemand`/`backtestForecast` (`packages/demand/src/forecast.ts`) take optional **exogenous signals** — each a **date range × a multiplier** with a label. Festivals, promotions and weather are the SAME shape ("on these days demand is × this factor"), so one mechanism covers all three: overlapping signals multiply, each forecast day reports its uplift + which signals applied, and the calendar is **data the caller supplies** (a compact `?events=FROM~TO~MULT~LABEL,…` on the route — public festival dates, the promo diary, a weather feed), never hard-coded. Threaded through the back-test, so a known event in the holdout is scored fairly (a festival spike blind ≈ WAPE 0.5; told about it, < 0.05 — proved on a fixture). **8 new tests** (uplift on signal days, overlap composition, back-test improvement, malformed-signal reject; route `?events=` end-to-end + malformed 400). **STILL DEFERRED, NAMED:** new-item cold-start. Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). Prior: D-4 markdown ladder (#169), D-1 forecast (#168), replenishment uses real demand (#167), sales-history read (#166), C4 (#165), D-3 (#164).)
