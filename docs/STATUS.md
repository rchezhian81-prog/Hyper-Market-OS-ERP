...
Last updated: 12 August 2026 (**★ C2 — DATA-BREACH NOTIFICATION WORKFLOW (DPDP s.8(6)) ★** (M16·M34): if personal data is breached, the law puts the response on a clock — this builds that workflow. `assessBreachNotification` (`packages/customer/src/breach-notification.ts`) turns a breach event into three obligations: the Data Protection **Board's immediate intimation**, its **72-hour detailed report** (deadline = discovery + 72 h, marked **overdue** once past), and the notice to **every affected person** — each with the **prescribed content still missing** (the field lists are named constants — a legal fact). The clock runs from **discovery** (when the shop became aware), not when it happened. **It drafts and tracks; a person sends** — advisory-only, no function transmits a notice (accountability stays human). Runs on `POST /v1/privacy/breach/assess`, gated `customer.consent.write` (owner + store-manager, not the till). **10 new tests** (72h deadline, three obligations, missing-content lists + clearing, overdue transition, all-satisfied, malformed reject, RBAC). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). **Note:** I had said the self-contained vein was exhausted — that was too quick; the DPDP C-series (C1 consent-notice validator, C2 done, C3 retention clock) is genuine self-contained R2 work needing no owner decision. Prior: D-1 cold-start (#171), D-1 signals (#170), D-4 markdown (#169), D-1 forecast (#168), replenishment uses real demand (#167), sales-history read (#166), C4 (#165), D-3 (#164).)
