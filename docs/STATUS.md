...
Last updated: 10 August 2026 (**★ CORE-04 inc5 — THE PERIOD CREDIT-NOTE RECONCILIATION (GSTR-1) IS NOW READABLE ON THE CLOUD (M23-FR-02) ★**: the fifth CORE-04 increment. `reconcileNotes` — a tested `packages/finance` engine no route called — now runs on API-09 as a pure read. One NEW route in `services/finance/src/credit-notes.ts`: `GET /v1/finance/credit-notes/reconciliation?period=YYYY-MM&ledgerTaxable=&ledgerTax=` folds the credit notes ISSUED for a return period from the wired `CreditNoteIssued` ledger (grouped by each note's own `declareInPeriod` — the month ISSUED, never the invoice's) and compares the adjustable credited taxable+tax totals against what the accounts POSTED for that period (supplied), reporting the difference **EXACTLY and with its sign** — the number the buyer's GSTR-2B finds before we do — plus the value issued OUTSIDE the s.34(2) 30-November window (commercially real, fiscally not). The only new adapter code is a `notes` fold of the events the credit-note write path (CORE-02 inc2) already produces. No new events, no write-path change, no migration, no owner input, no external auth; `ledgerTaxableMinor`/`ledgerTaxMinor` are supplied comparison figures on a READ (query params, the SAME pattern as `?posted=` on the stored-value liability and the B2B AR reconciliation), not write fields. **RBAC least-privilege (P-04):** reconciling GST credit notes to the general ledger is a finance-controllership act, so it is gated on the tier that ISSUES the notes — **`finance.creditnote.issue`** (owner + accountant), NOT the store_manager (who holds `finance.period.read` but cannot reconcile the notes) — the same one-rung-above split as the B2B sub-ledger reconciliation. `tests/integration/credit-note-reconciliation.test.ts` proves it over the real pipeline + real per-tenant RBAC (reconciles; a signed GSTR-2B gap; the s.34(2)-outside amount surfaced separately from an old-FY note; refuses without a period or ledger figures; the manager/cashier refused 403 while owner/accountant read it; per-tenant isolation) AND end to end against real PostgreSQL (a `describe.skipIf(!DATABASE_URL)` e2e in the Stage-gate job). **Process note:** the round-3 investigation had declared the pure-read seam exhausted, marking `reconcileNotes` NOT VIABLE because its ledger side 'is not cloud-derivable' — but that is exactly `reconcileLiability`'s shape (inc4): a supplied comparison figure on a read is the established pattern, not a blocker. Caught on verification and shipped. M23 stays PARTIALLY WIRED — the month-end CLOSE control totals still refuse by design (they need the owner's independent second source; held). Full gate green (typecheck, lint, secret-scan, **4,747 tests / 330 files**, plus the real-PG e2e). Held for pre-deployment as before: finance period-close second source, chart of accounts, real provider credentials, the ten owner decisions.)
