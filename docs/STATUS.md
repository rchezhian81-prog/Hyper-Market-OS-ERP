...
Last updated: 12 August 2026 (**★ SALES-HISTORY DEMAND READ — THE FOUNDATION FOR FORECASTING & MARKDOWNS ★** (M09, owner-directed "do the sales-history storage"): the demand history the reorder engine needs, and the base D-1 (forecast) and D-4 (markdown ladder) will build on. **The key finding: no new write path was needed.** Every sale is ALREADY kept as an append-only `SaleCommitted` event carrying the whole bill, lines and all (the events ARE the record, hard rule #2) — what was missing was the READ. `salesHistory` (`packages/demand/src/sales-history.ts`) folds sold lines over a window into **per-product demand**: total sold, the day-by-day series, and an **average daily demand** (Σ qty ÷ window days — the rate a reorder needs, exactly the `avgDailyDemand` D-3 consumes). Runs on `GET /v1/inventory/sales-history?from=&to=&productId=`, gated `inventory.availability.read`, default trailing 28 days; a **windowed** fold (cost of a window, not the whole history), the sale-commit path untouched (hard rule #1). **11 new tests** (per-product fold, window-days averaging, in-window filtering, malformed-skip, grams unit, empty window, bad-window reject; end-to-end route + RBAC). Full gate green (typecheck, lint, secret-scan, traceability-integrity, full suite). NEXT on this vein: D-1 (forecast decomposition + back-test) and D-4 (markdown ladder) now have their demand input; the replenishment route can later derive avgDailyDemand from this read. Prior: C4 children's-data guard (#165), D-3 shelf-life ordering (#164), B8 auto-capture (#163).)
