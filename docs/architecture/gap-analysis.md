...
| `apps/` — everything a person touches | 11,300 | **All six have real screens, and the ERP now has eleven** — the manager's, the buyer's, the product and price screen, merchandising and space, reporting, the service desk, expiry and recall, finance, admin and security, AI control, and migration. The gap that dominated this document is closed |
| Tests | 4,430 + 31 | Unusually thorough on rules; thin on assembly until today |
...
| M36 + A01–A10 AI | ✅ | ✅ | ✅ | ~~Kill switch defaults **on**~~ — and **stopped nothing**, which is the defect found building this screen: `callModel`, the one place every model call passes through, consulted neither the kill switch nor the budget, so the switch could be pulled and the gateway carried on calling the provider. An admission decision is now **required at the gateway and its absence refuses**, checked before the transport is touched. **AI control is now a screen** — the switch first, with what the shop does without each assistant beside it; what no assistant may ever do, shown as a list; everything drafted waiting for a named person; and what it has cost against the owner's own ceiling (D3). A second defect found by driving a box a month old: spend had **two sources** and the two tabs disagreed by an entire budget. No provider chosen (owner decision) |
| MG-01–MG-12 Migration | ✅ | ✅ | ✅ | Strongest non-POS area, and now **a screen the owner can drive** (OB-06: he extracts his own data, so nobody else will run this). **The defect found building it is the one that mattered most in the whole project:** `decideCutover` — the gate on the night the shop stops running on the old system — has refused GO on eight checks since it was written, and every call site, *including the Stage 11 gate test*, had always handed it those eight as booleans typed in by hand. The gate had never once been asked about the state of the migration. `cutover-checklist.ts` now DERIVES all eight from the real producers, and an unanswerable check fails rather than defaulting. A second defect, found by driving the page: a signature was computed, shown, and dropped — it now commits locally and queues (hard rule #1). **No real data yet** — the pipeline is proven against a synthetic legacy dataset (Stage 11), and the live extraction waits on the store |
