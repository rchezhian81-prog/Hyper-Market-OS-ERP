...
| GAP-SEC-06 | ~~Recent re-auth / step-up for sensitive actions enforced in web-erp, not API tier~~ **SUBSTANTIALLY CLOSED** — API-tier step-up mechanism added (`services/kernel/src/step-up.ts`, pipeline check on the SIGNED token's `auth_time`/`amr`) and enforced on privilege-grant + erasure-execution (fresh MFA ≤300s); direct-call bypass refused 403. **Follow-on:** payroll release + bulk product publish action-level gating | ~~Medium~~ Low (follow-on) | `services/kernel/src/step-up.ts`; `tests/{unit/step-up,integration/step-up-reauth}.test.ts` |
