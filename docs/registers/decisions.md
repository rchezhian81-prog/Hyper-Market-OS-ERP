...
| OB-04 | **Fresh/production departments: cafe only** (owner, 3 Aug 2026) — closes **AVR-12**. | M11 is built as the shared production foundation (recipes/BOM, raw-material issue, batch output, yield and process loss, packing and shelf life) **plus the `cafe` department only**. Bakery, deli, meat/fish, cut fruit, pharmacy and concession are **not built and not enabled** (roadmap §2.2 / Annexure H Stage 16: do not build a module for a department you do not have). Adding one later is a **change record**, never a silent gap (OD-02). Cold-chain scope (M10) and FSSAI obligations (M34-FR-03) are sized to a cafe. | Accepted |
