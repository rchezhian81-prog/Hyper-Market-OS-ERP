...
**Outside-party clocks started — owner, 24 September 2026.** For the narrow-and-deep store pilot
(`docs/runbooks/pilot-plan-narrow-deep.md` §6), the owner has **initiated** the onboarding/procurement
for the outside-party items that gate full production. **"Started" means the process is under way** — an
application filed, a vendor contacted, a quote requested — **not delivered**: each row below keeps its
current Type and moves to **Satisfied** only when the actual credential, contract, hardware or
certificate is in hand and wired. Marked **⏳ Started 2026-09-24**: EX-03 (payment provider), EX-07
(GST / e-invoice credentials, incl. engaging the CA), EX-08 (FSSAI / Legal Metrology / local licences),
EX-09 (store hardware), EX-13 (independent penetration test). None blocks the **test-mode** pilot; they
run in parallel because of their lead times.
...
| EX-03 | **Payment provider (RBI-authorised, tokenising)** | POS card/UPI tender, refunds, settlement reconciliation | **Blocking at Stage 9 for live tender** | **⏳ Started 2026-09-24.** Commercial agreement + credentials | Tender engine is provider-agnostic; card data never stored (hard rule #3); sandbox/mock provider used |
...
| EX-07 | **GST / e-invoice portal credentials** | Statutory filing and e-invoicing | Deferred (Stage 10) | **⏳ Started 2026-09-24.** Credentials + engage the CA | Tax computed exactly in-system; filing is an adapter |
| EX-08 | **FSSAI, Legal Metrology and local licences** | Lawful trading; M34-FR-03 alerting | **Owner data** (AVR-11) | **⏳ Started 2026-09-24.** Provide the certificates and a named owner each | Register built; alerts cannot fire until entered (OC-19) |
| EX-09 | **Store hardware** — lanes, scanners, printers, scales, cash drawers | Pilot | Deferred (Stage 12) | **⏳ Started 2026-09-24.** Purchase / confirm existing | All device access is behind ports; ESC/POS implemented |
...
| EX-13 | **Independent penetration test** | QG-06 before customer launch | **Blocking at Stage 14** | **⏳ Started 2026-09-24.** Engage a tester (paid) | Threat model, guardrails and security tests in place |
