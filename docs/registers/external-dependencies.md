...
| EX-02 | **Previous-system export access + lawful extraction rights** | Real-data extraction and final cutover **only** — the MG-01…12 rehearsal is complete against a synthetic dataset (`docs/evidence/stage-11-the-old-shop-arrives-whole.md`) | **Owner action pending** (letter drafted, `docs/discovery/legacy-data-access.md`) | Sign and send the drafted letter | **Does NOT block:** architecture, UX, modules, APIs, tests, simulator work, security controls, offline design, synthetic migration testing or AI implementation — **all of which are now built and gated**. What remains dependent on it: the real fault profile, the real volume and therefore the true cutover window, and the real control-total figures for the CA to sign. **Raise again ONLY when:** the vendor responds · a planned response date is materially missed · the real-data migration gate is reached (owner instruction, 4 Aug 2026) |
