...
| OC-44 | How long a shelf count stays worth acting on | store pack `merchandisingPolicy.countStaleAfterMinutes` | **120 minutes.** Past it a facing raises **no** refill task rather than sending somebody on an old reading — enough wasted walks and the whole list stops being believed | UAT |
| OC-45 | How empty a facing must be before it is worth a trip | store pack `merchandisingPolicy.refillAtBp` | **5,000 bp (half empty).** Refilling at 90% wastes the staff's day; refilling at 0% means the customer already found the gap | UAT |
