...
| In-store production — cafe (M11) | M11-FR-01/02/03/04 / OB-04 (AVR-12 closed: cafe only) / §2.2 / §9.3 / P-02 / P-08 — department enablement refusing counters the store does not operate; recipe/BOM scaling; issue blocked before anything is consumed when short; output into quarantine so release gates sellability; input cost carried by surviving output; yield drift and zero-output as valued exceptions; label refused without use-by/net quantity/packer/allergen declaration; release refused on failed QC, unnamed releaser or expired batch; repack inherits source expiry and stays traceable | `packages/production/src/` | `tests/unit/production.test.ts` (23 tests) |
...
| M11-FR-02 | 2 | `docs/requirements/M11.md` | `packages/production/src/recipe.ts` | `tests/unit/production.test.ts` | R2 | Foundation built (yield in basis points against the recipe, input cost carried by the surviving output so process loss lands in margin, yield drift and zero-output raised as valued exceptions) — catch-weight capture applies only to weighed departments, which SRE does not operate (OB-04) |
| M11-FR-03 | 2 | `docs/requirements/M11.md` | `packages/production/src/packing.ts` | `tests/unit/production.test.ts` | R2 | Foundation built (label refused without use-by, net quantity, packer details or an allergen declaration; quality release gates sellability and is refused for failed QC, an unnamed releaser or an expired batch; repack inherits its source expiry and stays traceable) — physical scale integration is D14 hardware |
| M11-FR-04 | 2 | `docs/requirements/M11.md` | `packages/production/src/departments.ts` | `tests/unit/production.test.ts` | R2 | Foundation built (**cafe only** per OB-04 closing AVR-12; every other department refused with what the store does operate; catalogue exists so another tenant can enable one — OB-01) |
