...
| `production` | In-store production (all departments) | Every department is built — cafe, bakery, deli, meat/fish, kitchen — and each tenant enables its own (SRE: cafe, OB-04/OB-05); a department not enabled is refused; a run cannot issue more than is on hand; output lands in **quarantine** and is sellable only after a named quality release; cost follows the food that survived so process loss shows in margin; yield drift and zero-output are valued exceptions; labels refuse to print without use-by, net quantity, packer and allergen declaration; a repack inherits its source's expiry; catch-weight costing in exact grams puts the whole carcass cost on the meat that survived, with yield measured against a standard; scale labels are generated from the same barcode rule the till parses, proven by round-tripping a printed label through the real catalogue (M11-FR-01/02/03/04, §9.3, §29.1, P-02) |
| `stock` | Stock states, availability & health | Availability derived from state — reserved/quarantine/damaged/expired/in-transit are visible but not sellable; expired and quarantine can never be made sellable by any tenant; negative stock blocked or raised as a visible exception; ageing/turns/GMROI/stockouts exact in basis points (M08-FR-02/04, §6.2, P-08) |
